...
- A metric can read the wrong source and report zero forever. Revenue was counted from Stripe checkout sessions until 2026-09-10; every session on this account is unpaid and expired, so it reported zero while a real charge sat in the account. Check that a measurement can see the thing it measures.
