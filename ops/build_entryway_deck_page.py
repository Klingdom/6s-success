#!/usr/bin/env python3
"""
Build the page that ships the (corpus-accurate) Entryway deck: 57 cards,
typeset, unillustrated, built straight off content.json's real five zones.

WHY THIS FILE EXISTS ALONGSIDE deck.html, NOT INSTEAD OF IT
-------------------------------------------------------------
BACKLOG-2026-09-07.md B7/B9. The free deck already live at `site/deck.html`
(`DECK-ENTRY`, 88/89 cards, `ops/build_deck_gallery.py`) teaches twelve
Entryway "Micro Zone" cards, a model transcribed off finished cards years
before the Manual's own diagnosis layer existed. The Manual, the zone
pages, the Home Quest and the $19 print pack all teach FIVE Entryway zones.
A household owning both the free deck and the paid pack is taught two
different zone lists for one room.

Retiring or rewriting the old deck is a real product decision (a live SKU,
a different photographed-art pipeline, years of prior work) and not one
this generator makes on its own. This page ships the corpus-accurate deck
at its own URL, `entryway-deck.html`, the same way
`ops/build_kitchen_deck_page.py` shipped the Kitchen deck: free, unillustrated,
alongside the existing product, each disclosing the other rather than
pretending only one exists. This file mirrors that generator function for
function, and imports its shared, room-agnostic rendering helpers directly
(`esc`, `band`, `art_panel`, `card_html`, `print_tile`, the CSS) rather than
forking a second copy of logic that has nothing Kitchen-specific in it.

Run:  python ops/build_entryway_deck_page.py
Out:  site/entryway-deck.html
"""
...
import build_entryway_deck as ED                                 # noqa: E402
# Shared, room-agnostic rendering helpers: none of these read anything
# Kitchen-specific, they read only the card dict shape both generators
...
OUT = os.path.join(SITE, "entryway-deck.html")

ZONE_ORDER = ED.ZONE_ORDER
...
def build_body(deck: dict) -> str:
    ...
    parts = [f'<section class="kzone kzone-room"><h2>The Entryway</h2>'
    ...
    # Names what the three whole-entryway cards are about, not just how
    # many: a count alone cannot prove the naming below still matches.
    whole_ids = [c["id"] for c in whole]
    assert whole_ids == ["EYA-011", "EYA-012", "EYA-013"], (
        "whole-entryway action cards changed (%s); the 'seasonal swap/"
        "fourteen day paper rule/full rebuild walk' sentence in "
        "build_body() no longer describes the real three and must be "
        "rewritten by hand" % whole_ids)
    parts.append('<section class="kzone"><h2>Whole entryway</h2>'
                  f'<p class="klead-p">{num_word(len(whole))} cards that are '
                  'not one zone’s job: the seasonal coat and shoe swap, '
                  'the fourteen day rule that keeps the paper folder '
                  'honest, and the safety walk to do before any '
                  'rebuild.</p>'
    ...
    parts.append('<section class="kzone"><h2>Root causes, the shared '
                  ...
                  f'entryway points at one of these '
                  f'{num_word(len(causes)).lower()}, the same vocabulary '
                  f'the Kitchen deck uses. Pull one when a friction card '
                  f'sends you here.</p>'
...
<title>The Entryway Deck, Five Zones: __N__ cards, free to read</title>
<meta name="description" content="The Manual's real __NZONES_LOWER__ Entryway zones, the friction each one causes, the root cause, the fix, and the standard to keep. __N__ cards, typeset, free.">
<!-- SEO:BEGIN -->
<link rel="canonical" href="https://6s-success.com/entryway-deck.html">
...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    {"@type": "ListItem", "position": 2, "name": "The Entryway Deck (Five Zones)", "item": "https://6s-success.com/entryway-deck.html"}
  ]
}
</script>
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Game",
  "@id": "https://6s-success.com/entryway-deck.html#deck",
  "name": "The 6S Success Entryway Deck (Five Zones)",
  "url": "https://6s-success.com/entryway-deck.html",
  "inLanguage": "en",
...
<section class="hero">
  <div class="wrap">
    <div class="hero-copy">
      <p class="eyebrow on-deep">The deck, five zones</p>
      <h1>The <em>Entryway</em> Deck</h1>
      <p class="sub">__N__ cards: the Manual's real __NZONES_LOWER__ Entryway zones, the frictions each one causes, the __NCAUSES_LOWER__ root causes underneath, the actions that fix them, and the standard each zone keeps. Typeset and free. No illustrations yet, so every card reads as text, not a photograph.</p>
      <div class="cta-row">
        <a class="btn btn-primary btn-lg" href="#entryway-cards">Read the deck</a>
      </div>
      ...
      <p class="fulfil-note">This is a second, newer Entryway deck. The original illustrated one, twelve "micro zones," photographed: <a href="deck.html" style="color:inherit">deck.html</a>.</p>
    </div>
  </div>
</section>
...
<section class="section">
  <div class="wrap narrow">
    <p class="eyebrow">Two Entryway decks, on purpose, for now</p>
...
<section class="section" id="entryway-cards">
  <div class="wrap">
    <p class="eyebrow">The deck</p>
    <h2>Read it here, zone by zone</h2>
    <p>Tap a card's "How it works" to see its back. Start at the door: it is the shortest zone and the one the room's own guidance says to work first.</p>
__BODY__
  </div>
</section>
...
def main() -> int:
    deck = ED.build()
    src = json.load(io.open(ED.SRC, encoding="utf-8"))
    intro = [r for r in src["rooms"] if r["room"] == "Entryway"][0]["intro"]
    ...
    for c in deck["cards"]:
        by[c["type"]] = by.get(c["type"], 0) + 1
    print(f"  entryway deck page  {deck['count']} cards")
    for t in TYPE_COUNT_ORDER:
    ...
    # re-runs on its own output (ops/build_kitchen_deck_page.py's own
    # comment explains why: issue #26's shape, a hardcoded fingerprint that
    # cannot go stale if it is never hardcoded in the first place).
