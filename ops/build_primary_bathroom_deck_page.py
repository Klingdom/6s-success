#!/usr/bin/env python3
"""
Build the page that ships the Primary Bathroom deck: 76 cards, typeset,
built straight off content.json's real seven zones.

WHY THIS FILE EXISTS
---------------------
BACKLOG-2026-09-07.md B9, the fifth room built this way after Kitchen,
Entryway, Laundry Room and Home Office. Like Laundry Room and Home Office,
there is no old, mismatched free Primary Bathroom deck to disclose against:
no free Primary Bathroom product exists on the site yet, so this page is
the first, at its own URL. This file mirrors
ops/build_home_office_deck_page.py function for function, and imports the
same shared, room-agnostic rendering helpers (`esc`, `band`, `card_html`,
`print_tile`, the CSS) rather than forking a second copy of logic that has
nothing room-specific in it.

Run:  python ops/build_primary_bathroom_deck_page.py
Out:  site/primary-bathroom-deck.html
"""
...
import build_primary_bathroom_deck as PB                         # noqa: E402
...
OUT = os.path.join(SITE, "primary-bathroom-deck.html")

ZONE_ORDER = PB.ZONE_ORDER
...
def build_body(deck: dict) -> str:
    ...
    parts = [f'<section class="kzone kzone-room"><h2>The Primary Bathroom</h2>'
    ...
    assert whole_ids == ["PBA-015", "PBA-016", "PBA-017"], (
        "whole-room action cards changed (%s); the 'standing-moisture "
        "walk/open-chemistry check/sleeping-dates sweep' sentence in "
    ...
    parts.append('<section class="kzone"><h2>Whole room</h2>'
                  ...
                  'are not one zone’s job: a moisture walk through every '
                  'zone that dries wet surfaces before mold gets a start, '
                  'a check that no two chemicals in the room are stored '
                  'close enough to mix, and a sweep to read every expiry '
                  'date in one sitting before it quietly passes.</p>'
                  + "".join(card_html(c, by_id) for c in whole)
                  + '</section>')
    parts.append('<section class="kzone"><h2>Root causes, the shared '
                  'deck</h2>'
                  f'<p class="klead-p">Every friction card in the primary '
                  f'bathroom points at one of these '
                  f'{num_word(len(causes)).lower()}, the same vocabulary '
                  f'the Kitchen, Entryway, Laundry Room and Home Office '
                  f'decks use. Pull one when a friction card sends you '
                  f'here.</p>'
...
<title>The Primary Bathroom Deck: __N__ cards, free to read</title>
<meta name="description" content="The Manual's real __NZONES_LOWER__ Primary Bathroom zones, the friction each one causes, the root cause, the fix, and the standard to keep. __N__ cards, typeset, free.">
<!-- SEO:BEGIN -->
<link rel="canonical" href="https://6s-success.com/primary-bathroom-deck.html">
...
<script type="application/ld+json">
{
  ...
  "itemListElement": [
    ...
    {"@type": "ListItem", "position": 2, "name": "The Primary Bathroom Deck", "item": "https://6s-success.com/primary-bathroom-deck.html"}
  ]
}
</script>
<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Game",
  "@id": "https://6s-success.com/primary-bathroom-deck.html#deck",
  "name": "The 6S Success Primary Bathroom Deck",
  "url": "https://6s-success.com/primary-bathroom-deck.html",
  "inLanguage": "en",
  ...
  "abstract": "A __N__ card deck for the primary bathroom, built from the Manual's real __NZONES_LOWER__ zones: the frictions each one causes, the __NCAUSES_LOWER__ root causes underneath (shared with the Kitchen, Entryway, Laundry Room and Home Office decks), the actions that fix them, and the standard each zone keeps. Typeset, free, no illustrations yet."
}
</script>
<!-- SEO:END -->
...
<section class="hero">
  <div class="wrap">
    <div class="hero-copy">
      ...
      <h1>The <em>Primary Bathroom</em> Deck</h1>
      <p class="sub">__N__ cards: the Manual's real __NZONES_LOWER__ Primary Bathroom zones, the frictions each one causes, the __NCAUSES_LOWER__ root causes underneath, the actions that fix them, and the standard each zone keeps. Typeset and free. No illustrations yet, so every card reads as text, not a photograph.</p>
      <div class="cta-row">
        <a class="btn btn-primary btn-lg" href="#primary-bathroom-cards">Read the deck</a>
      </div>
      ...
      <p class="fulfil-note">This is the first Primary Bathroom deck on the site: built straight from the same Manual as the Kitchen, Entryway, Laundry Room and Home Office decks, with the same rules.</p>
    </div>
  </div>
</section>
...
<section class="section" id="primary-bathroom-cards">
  <div class="wrap">
    <p class="eyebrow">The deck</p>
    <h2>Read it here, zone by zone</h2>
    <p>Tap a card's "How it works" to see its back. Start at the vanity counter: it is the surface you stand in front of every morning and every night, and clearing it changes what the whole room looks like from the doorway.</p>
__BODY__
  </div>
</section>
...
def main() -> int:
    deck = PB.build()
    src = json.load(io.open(PB.SRC, encoding="utf-8"))
    intro = [r for r in src["rooms"]
             if r["room"] == "Primary Bathroom"][0]["intro"]
    ...
    for c in deck["cards"]:
        by[c["type"]] = by.get(c["type"], 0) + 1
    print(f"  primary bathroom deck page  {deck['count']} cards")
    for t in TYPE_COUNT_ORDER:
