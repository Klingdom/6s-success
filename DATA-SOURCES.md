...
| Web analytics | Umami (self-hosted, on the Hostinger VPS) | VERIFIED: real traffic figures (75 visitors/196 visits, `GOALS.md` O1) were read directly from the production Umami database via SSH + `docker exec` + `psql` (`ops/experiments.py`'s `umami_rows()`), confirmed on at least five separate dates (2026-09-02, 09-05, 09-07, 09-11, 09-14); needs `~/.ssh/6s_deploy`, absent in most operator sandboxes | Behavioral analytics | Daily or better | analytics-intelligence |
