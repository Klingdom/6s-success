...
| Web analytics | Umami (self-hosted, on the Hostinger VPS) | VERIFIED: real traffic figures (**re-measured 2026-09-25**: 57 visitors/144 visits/30 days, `GOALS.md` O1, measured 2026-09-25 01:17 UTC; the same night an unfiltered query said 239 because this Umami instance serves three sites, so `umami_rows()` now refuses a website_event query that names no website) were read directly from the production Umami database via SSH + `docker exec` + `psql` (`ops/experiments.py`'s `umami_rows()`), confirmed on at least eight separate dates (2026-09-02, 09-05, 09-07, 09-11, 09-14, 09-17, 09-21, 09-23); needs `~/.ssh/6s_deploy`, absent in most operator sandboxes | Behavioral analytics | Daily or better | analytics-intelligence |
...
| Crawler activity | **Primary: the Nginx Proxy Manager access log** (`nginx-proxy-manager:/data/logs/proxy-host-4_access.log*`), which survives container recreation, is rotated with archives, reaches back to 2026-08-19 (181,847 lines) and keeps client IPs so a Googlebot claim can be reverse-DNS checked. Secondary: this site's own IP-free log at `/var/log/6s-success/access.log`, which starts 2026-09-20. Both read by `ops/crawl_report.py` (`--source proxy|container`) | PARTIALLY_VERIFIED: the mechanism was proved in a test container on the VPS on 2026-09-20 (file written, survives the container being destroyed and recreated, no IP addresses in it); the production log only begins at the deploy that mounts it, so it holds no history before then. Needs `~/.ssh/6s_deploy`. **A user agent is a claim, not an identity**: the log keeps no addresses, so Googlebot cannot be verified by reverse DNS and every figure must be written as "requests from something calling itself Googlebot" | Which pages search engines and AI crawlers actually fetch, and what status they got | Near real time | seo-aeo |
