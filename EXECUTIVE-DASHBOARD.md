...
> state. **Corrected 2026-09-16, operator:** Issue #8 was about a
> different pair, this file versus the discarded `_review/EXECUTIVE-DASHBOARD-ALT.md`,
> resolved by keeping this one as canonical; its closing comment only
> mentions `EXECUTIVE-DASHBOARD-LIVE.md` in passing, as the separate live
> artifact this spec was never meant to duplicate. The names still collide
> badly enough that somebody could read the spec as the report, which is
> the real risk this note exists to head off.
...
**Corrected 2026-09-16, operator.** This section still read `exists: UNKNOWN` and
`last_verified: UNKNOWN` for the dashboard itself, six lines below this same
document's own banner stating the dashboard already exists and is regenerated
on every run. The mission this section exists to track (build 105.1-105.20) is
long done; the block below was never updated once it was.

The authoritative, currently-measured record for every field below is
`EXECUTIVE-DASHBOARD-LIVE.md`, regenerated by `python ops/dashboard.py` on
every operating cycle and re-read there rather than hand-copied here, so this
section cannot go stale the same way twice. Only the fields this operator
could directly confirm are set below; everything else stays `UNKNOWN` on
purpose, per this section's own rule.

Populate only from verified evidence:

```yaml
dashboard:
  exists: TRUE, EXECUTIVE-DASHBOARD-LIVE.md
  url: no separate hosted URL; read as a repository file (GitHub blob view,
    or ops/dashboard.html for the same content as a static page)
  authentication: none; it is a file in a private GitHub repository, not a
    hosted app
  mobile_ready: UNKNOWN
  last_verified: see EXECUTIVE-DASHBOARD-LIVE.md's own "Generated" line,
    stamped fresh on every run
