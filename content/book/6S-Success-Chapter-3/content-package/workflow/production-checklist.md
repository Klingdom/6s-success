# Production Checklist: Chapter 3 Content Package

Work top to bottom. Check each item before it goes live.

## Phase 0: Source confirmation
- [ ] Confirm the final HTML is the version you want to publish.
- [ ] Confirm the live chapter URL and slug (`the-six-steps-that-transform-any-space`).
- [ ] Confirm the book title, author name, and publisher for attribution lines.
- [ ] Replace all placeholders (author, publisher, URL, dates) in `web/schema-org-article.json`, the `pdf-ebook/` files, and `chapter-03-publishable.html`.

## Phase 1: Canonical lock
- [ ] Title, subtitle, and "One Idea to Keep" match the live book exactly.
...
- [ ] Wire up previous (Chapter 2) and next (Chapter 4) navigation.
- [ ] Confirm the og:image (the 6S Loop or the before/after cabinet) is produced and sized 1200x630.
...
- [ ] The fifteen-minute-lap CTA appears in the highest-traffic posts.
...
- [ ] B-roll shot list gathered or sourced (the cabinet, the towel sort, the leak wipe, the safety crouch).
...
- [ ] The 6S Loop infographic produced to spec, both portrait and landscape, Safety in slate.
- [ ] 10 quote cards designed, quotes verbatim, attribution correct.
- [ ] Diagrams produced or adapted from the book's existing SVGs (the loop, Sort+Straighten, Shine reveal, Safety swap, before/after).
...
- [ ] No em dashes anywhere (run a find for the character). "Transform" appears only in the title.
- [ ] No banned words used as filler.
- [ ] All facts trace back to the final HTML. Inferences are labeled.

## Phase 9: Publishable HTML
- [ ] Open `chapter-03-publishable.html` in a browser and confirm it renders (fonts, the 6S Loop SVG, all figures).
