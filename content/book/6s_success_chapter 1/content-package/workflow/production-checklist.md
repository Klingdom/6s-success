# Production Checklist: Chapter 1 Content Package

Work top to bottom. Check each item before it goes live.

## Phase 0: Source confirmation
- [ ] Confirm the final HTML is the version you want to publish (not the earlier "The Day Everything Went Missing" draft).
- [ ] Confirm the live chapter URL and slug (`why-some-homes-feel-effortless`).
- [ ] Confirm the book title, author name, and publisher for attribution lines.
- [ ] Replace all placeholders (author, publisher, URL, dates) in `web/schema-org-article.json` and `pdf-ebook/` files.
...
- [ ] Wire up next-chapter navigation (this is Chapter 1, so there is no previous).
- [ ] Confirm the og:image (calm entryway or two-Tuesdays split) is produced and sized 1200x630.
...
- [ ] The before-photo CTA appears in the highest-traffic posts.
...
- [ ] Diagrams produced or adapted from the book's existing SVGs (two Tuesdays, junk-drawer stages, shelf-life bars, six-S loop, find-the-friction).
