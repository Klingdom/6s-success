# Production Checklist: Chapter 4 Content Package

Work top to bottom. Check each item before it goes live.

## Phase 0: Source confirmation
- [ ] Confirm the drafted chapter (signature, manuscript, final HTML) is approved by the author. This chapter was drafted, not supplied. Review it before building anything for publication.
- [ ] Confirm the live chapter URL and slug (`how-to-choose-your-first-target-area`).
- [ ] Confirm the book title, author name, and publisher for attribution lines.
- [ ] Replace all placeholders (author, publisher, URL, dates) in `web/schema-org-article.json`, the `pdf-ebook/` files, and `chapter-04-publishable.html`.
...
- [ ] Wire up previous (Chapter 3) and next (Chapter 5) navigation. Note this closes Part 1.
- [ ] Confirm the og:image (the First Target Map or the overwhelming garage) is produced and sized 1200x630.
...
- [ ] The choose-and-write-it-down CTA appears in the highest-traffic posts.
...
- [ ] B-roll shot list gathered or sourced (the overwhelming garage, a humble drawer, a hand writing the target).
...
- [ ] The First Target Map infographic produced to spec, both portrait and landscape, sweet spot in green dots.
- [ ] The Target Scorecard produced, numbers matching (drawer +3, entryway +3, bathroom +2, linen closet -1, garage -2).
...
- [ ] Open `chapter-04-publishable.html` in a browser and confirm it renders (fonts, the First Target Map and Target Scorecard SVGs, the friction meter, all figures).
