    ...
    return (
      <SafeAreaView style={s.screen}>
        ...
        <ScrollView contentContainerStyle={s.pad}>
          ...
          <Text style={s.note}>{zonesHoldingLine(zonesHeld, CORPUS.zoneCount)}</Text>
  ...
  return (
    <SafeAreaView style={s.screen}>
      ...
      <ScrollView contentContainerStyle={s.pad}>
        ...
        {/* The zone picture, the one place the web Quest card allows one: a small
          * thumbnail in the row that names the zone, so the reader knows which
          * spot in the house this is. The working half of the card stays text.
          * Decorative (the zone is named right beside it) and absent for zones
          * with no approved illustration, generated by build_mobile_corpus.py. */}
        <View style={s.zrow}>
          {ZONE_HEROES[card.zone.img] ? (
            <Image source={ZONE_HEROES[card.zone.img]} style={s.thumb}
                   accessible={false} importantForAccessibility="no" />
          ) : null}
          <Text style={s.whereInRow}>{card.zone.room} &gt; {card.zone.zone}</Text>
        </View>
...
const s = StyleSheet.create({
  ...
  zrow: { flexDirection: "row", alignItems: "center", gap: 12, marginTop: 16 },
  thumb: { width: 74, height: 56, borderRadius: 9, backgroundColor: C.panel },
  whereInRow: { flex: 1, fontFamily: F.sansSemi, color: C.soft, fontSize: 13, letterSpacing: 0.4 },
