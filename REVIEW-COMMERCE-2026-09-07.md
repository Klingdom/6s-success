...
| C6 | ~~Retire the 6 Area Bundles, staged against production~~ | 84% of a $19 superset's price for 12-20% of its content, on one page nobody reaches | 2 struct / 8 judgement | 0.5 | **Delisted 2026-09-22, operator; deploy/live-verify/archive closed the same day.** New `RETIRED` exclusion in `ops/generated_products.py` (the file that actually controls what `ops/wire_generated_catalog.py` writes into `data.js`, not a hand edit of the generated block) drops all 6 by SKU with a cited reason each; `ops/wire_generated_catalog.py` regenerated `data.js` (159 → 138 entries, exact predicted count), `ops/build_product_schema.py` and `ops/prerender_shop.py` rerun, `shop.js`'s `CAT_ORDER` updated (the category now has 0 members so its filter button no longer renders). Full definitions plus a computed, re-verified price/card-count reason preserved in `ops/retired-skus.json` (one earlier miscalculation, `AB-WET-ROOMS` at 12% instead of the real 16%, caught before shipping); `DECISIONS.md` D-023 records the call. `ops/audit_catalog.py` confirms 0 pages/scripts still sell any of the 6, `check_sellable.py`/`check_urls.py`/`audit_pages.py`/`affiliate.py --check`/`fix_dashes.py --check` all clean after. **Deploy/live-verify/Stripe archival, done later the same day by a session holding a Stripe credential (see the account above, "Close the Stripe half of the C6/C7 retirement"):** deployed (live build `3da8341e30a4d1da`, 138 products, matching the repository), live-verified (192 live URLs, the sitemap plus `data.js`/`shop.js`, fetched and searched for all 21 SKUs and payment-link ids, 0 references, 0 pages unreadable), archived (21 payment links deactivated, 21 products archived, via the new `ops/retire_stripe_skus.py`, which refuses to write anything unless the live scan comes back clean and refuses outright on any `metadata.ledgerium_plan` object; `check_live_links.py` confirms every link the live site serves is still active afterward). The staged order was deliberate and held: repository first, live site proved clean second, Stripe archival last. | Independent |
| C7 | ~~Retire the 15 Situation Kits, with a written re-entry condition~~ | Same, at 74%; reachable only from `shop.html` | 2 struct / 8 judgement | 0.5 | **Delisted 2026-09-22, operator, same commit and same mechanism as C6; deploy/live-verify/archive closed the same day, same session and same evidence as C6 above.** Re-entry condition recorded in `DECISIONS.md` D-023: a kit returns when a page for it exists and has produced a measured organic entry. | Independent |
...
| C9 | ~~Capture preferred times on `thanks.html` after a service purchase~~ | `service_orders.py` can only send an invite if a time was named; nothing ever collects one | 2 | 0.5 | **Done 2026-09-22, operator.** `thanks.html` shows a scheduling block for `CN-VIRTUAL`/`CN-INHOME` completions only: best time, a second choice, and an optional note, composed into the same mailto handoff pattern `corporate.html` already uses. The placeholder shape ("14 October at 2pm") matches `ops/service_orders.py`'s own `find_time()` regex exactly, proven directly against the real function in the new `ops/tests/test_thanks_schedule.py` (headless Chromium), not re-derived. A submitted message reaches Phil by email and, once `find_time()` reads a time out of it, `service_orders.py` attaches a real `.ics`; a customer who ignores the block still gets the existing "we'll email you" path, unchanged. Found and fixed the same cycle, not part of C9's own scope: a live 1.13:1 contrast failure on C8's consult button across 160 pages (`ops/build_zone_pages.py`, `ops/wire_consult_cta.py`, `gate_consult_cta_current` extended to catch it permanently). | Below |
| C10 | ~~Add a free, capped, 15-minute "which zone first" call~~ | No rung between $19 and $250, and no service conversion event that is not a payment | 8 | 0.5 | **Done 2026-09-22, operator.** See `BACKLOG-2026-09-07.md` section 1's own C10 row for the full account: offered on `consulting.html` and in the zone/room CTA, cap stated ("up to 5 of these a week"), a submission composes a mailto to Phil, `intro-call-request` fires, "not a booking" language present. Gated by `gate_intro_call_current`, proved by both a static fail-then-pass test and a real headless-Chromium interactive test that fills in and submits the actual form. | Below |
...
| C20 | Escalated, GitHub issue #34: retire the 7 `ZP-KITCHE-*` and `RP-KITCHEN` | `DECK-SYSTEM.md` 9: a deck must retire at least as many SKUs as it adds | 2 | 0.5 | **Half done 2026-09-22, operator: option 2 (the real PDF) shipped.** `site/downloads/6S-Kitchen-Deck-PrintAndPlay.pdf` now exists, downloadable, gated for staleness; "downloadable" no longer reads as a technicality. Whether to actually retire the 8 SKUs is still Phil's, per the same row: fire now, or leave as-is (issue #34, updated). | Independent |
...
per `CLAUDE.md` 0.8. Genuinely remaining: **C1/C2/R1-R4** need Stripe
credentials no sandbox here holds; **C20** is escalated to Phil.
**C20, GitHub issue #34, updated 2026-09-22, operator: option 2 shipped, retirement itself still Phil's call.** Issue #34 put three options to
Phil (fire it as-is, ship a real Kitchen PDF first then fire it, or leave the
8 SKUs listed). `ops/build_deck_pdf.py` could not be reused directly (it
draws card art from photographed PNGs; the Kitchen deck has none yet, B1),
so a sibling, `ops/build_kitchen_deck_pdf.py`, renders the page's own
existing `@media print` sheet straight to a PDF through headless Chromium,
the same page `window.print()` already produced correctly. Verified live:
12 pages, both the deck's own opening and closing card titles present in
the extracted text, gated for staleness (`gate_kitchen_deck_pdf_current`,
a content hash of the print sheet recorded at build time, the same shape
`check_sitemap_current.py` already uses). The hero's old print-dialog
button is now a real `<a href="downloads/6S-Kitchen-Deck-PrintAndPlay.pdf">`
link. Verifying this also found and fixed a real, live, sitewide gap:
`measure.js`'s `free-download` tracking only matched an href starting
`/downloads/` or containing `print-and-play`; every real download link on a
ROOT-level page (deck.html, book.html, and now kitchen-deck.html) writes a
page-relative href with neither shape, so the Entryway deck's own PDF and
the book sample have likely never once been counted since that handler was
written. Fixed and proven fail-then-pass in
`ops/tests/test_measure_events.py`'s new probe F. "Downloadable" no longer
reads as a technicality either way it is judged. **Not done, and not this
operator's to do:** whether to actually retire the 8 `ZP-KITCHE-*`/
`RP-KITCHEN` SKUs. Issue #34 explicitly framed that as Phil's pick among
three options, not an economic judgement this repository has independent
evidence for the way D-023's retirement of the Situation Kits and Area
Bundles did; commented on the issue rather than deciding it.
