...
**Last measured:** 2026-09-23 12:50 UTC, traffic re-measured by a direct database read: 68 visitors/160 visits/30 days (2.3 a day; trailing week 12 after 10, 14, 18). Production deployed and current at build `5eba61fde231c1a7` (`ops/deploy-verdict.json`, confirmed `2026-09-23T19:00:39Z`, `8e4c8e33`); repository HEAD has since moved 47 commits further (this PM check-in), and the gap is no longer only internal consistency work: `a16788fa` fixed a second live, real mobile-nav-menu defect (`404.html`, `corporate.html`, `kit.html`, and 2 B2B articles all shipped a hamburger button wired to nothing, same defect class as `8e4c8e33`), and `b0166730` finished retiring the last of 65 dead/superseded Stripe SKUs. Both are sitting fixed in the repository and still live-broken/live-stale in production, over 8 hours now, because no session in this pipeline holds the VPS deploy key. This needs a real redeploy, not more sandbox verification.
