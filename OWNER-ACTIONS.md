...
| **1h** | Run `STRIPE_ALLOW_LIVE=1 python ops/retire_stripe_skus.py --apply` from a machine that holds the Stripe secret key, for the 44 SKUs still unconfirmed in Stripe | 2 min | This is not the 6 Area Bundles/15 Situation Kits: those 21 were already deactivated and archived in Stripe, live-verified, the same day they were retired (commit `34efb9a9`, your own session). The remaining 44 SKUs on `ops/retired-skus.json` (the 36 from the original 2026-08-21 batch: the hardcover book, the illustrated Entryway deck, the courses, and others, plus 8 more added 2026-09-23 by D-024, the 7 Kitchen zone packs and the Kitchen room pack, retired once the free Kitchen deck became a real downloadable PDF) have no equivalent confirmation anywhere in this repository, because the tool that checks and closes this did not exist until the first 21 shipped. **Corrected 2026-09-23:** this row said 36 for a day after D-024 quietly grew the same pending list to 44 and nothing checked the number written here against it; `gate_owner_actions_retired_sku_count_current` now re-derives it live from `ops/retired-skus.json` and `ops/retired-skus-stripe-status.json` on every run, so this cannot go stale again unnoticed. Same tool, same one command, same safety refusals. |
