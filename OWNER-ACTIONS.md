...
**Reopened 2026-09-15 (afternoon, PM check-in): one real commit behind again.** `ops/deploy-verdict.json` last confirmed production live at build `346c043b56385f64`, 11:17:08Z, from a session with real access. Since then exactly one commit touched `site/`: `790a5d05` (a phone review by Phil himself, fixing the "One session: 30-45 min. most of it in Sort" grammar on 113 of 114 zone pages and labelling the empty zone-picture slot on room pages), moving the repository to build `587d80befe8bd586`. No cloud sandbox since (this one included: no ssh key at `/root/.ssh/6s_deploy`, no egress) can redeploy or check production directly. If you already redeployed since your own `790a5d05` commit, nothing further is needed except letting a session with real access re-run `python ops/deploy.py --check` so the tracked verdict catches up; if not, the two small customer-facing fixes above are sitting in the repository, not in front of anyone yet.
