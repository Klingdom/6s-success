...
| 1.2 | ~~Wire the share URL or API key into the dashboard~~ | **Done 2026-09-08, operator, by the other route.** The deck now carries a Traffic row: people not events, with the automated share separated, and an explicit "not measured (reason)" when it cannot look, because a missing row reads as no traffic and a zero reads as a measurement. Both paths verified. **The share URL does not work and this is why**, so nobody repeats it: `/api/share/{id}` returns 200 with a websiteId and a 569-char JWT, and every call to `/api/websites/{id}/stats` with that token returns 401. Tried `x-umami-share-token`, `x-umami-token`, `Authorization: Bearer` and `Share`, with and without a cookie jar, with narrow and wide date ranges, and replaying byte for byte a URL that had just returned 200 in the browser. Same URL, 200 in the browser and 401 from Python, so the difference is a header the app adds that is not the documented one. Reading the page's own storage to find it is blocked by the browser extension's safety filter. The database read already works and is what the row uses. Accept when: `python ops/dashboard.py` prints real visitors, not "not measured" | 0.5 | operator |
| 1.3 | ~~Answer EXP-001: has a stranger ever clicked a buy button~~ | **Answered 2026-09-03: AMBIGUOUS, and permanently so.** 9 buy-clicks from 7 visitors out of 52 ever. Nothing distinguishing was captured at the time and no later query can invent it, so the nine can never be attributed. `observed_daily_visitors` is 1.7, read from the database. Ambiguous is a result; it is recorded rather than left open. | 0.5 | **done** |
| 1.4 | Answer EXP-002: does anyone reach the offer at the bottom of a zone page | **Read 2026-09-08, operator: still unanswerable, and the tool that said otherwise is fixed.** A live read reported "4 of 241 zone page views reached 70% or deeper (2%)" as ANSWERED. It was one session: 229 of the 241 events came from a single session_id in sixteen minutes, roughly fourteen zone pages a minute, which is a crawler or one of our own passes. The whole zone sample is 2 distinct visitors on the schema version that can answer this. The guard in `answer_exp002` counted EVENTS (`if v2 < 30`), and events are cheap. It now also requires 12 distinct visitors and refuses when any single visitor carries more than 40% of the sample; both guards proved independently. This mattered because 4.2 below is conditional on this answer, so a scripted crawl was one step from moving the offer on 114 pages. Also found: no event has ever carried the `who` key, so measure.js's internal-visitor label has never once been set and cannot currently separate our traffic from a stranger's. Accept when: scroll depth distribution recorded for 30 days, across at least 12 distinct visitors, no one of them over 40% | 0.3 | operator |
| 1.6 | ~~Report traffic as people rather than as events~~ | **Found and fixed 2026-09-08, operator.** The traffic block reported "878 pageviews, 64 visitors" unqualified. 441 of those 878, half the site's entire history, are two automated sessions running at 15.3 and 10.0 pages a minute. It now reports the split and prints two honesty notes: nothing is discarded, and the remainder is not "strangers" but traffic that is not obviously a robot, still including Phil and every check run from a real browser. Threshold is 10+ pageviews faster than 3 a minute, chosen because the flagged sessions sit at 15.3 and 10.0 and the fastest unflagged ones are effectively zero, so there is no borderline case. Also: `measure.js` now stamps who="automated" on headless and driven browsers, verified live. Accept when: traffic reported with the automated share separated | 0.3 | operator |
| 1.5 | Search Console: first impression data | 30 days of impressions exported and read | 0.5 | **blocked on Phil**, one verification paste, `OWNER-ACTIONS.md` item 1a |
...
**1.1, corrected 2026-09-02, re-corrected 2026-09-03.** The Umami API token is
expired (401 on every route) and no operator sandbox can read it
programmatically, but that is not the same as "unreadable": Phil read the
analytics database directly on 2026-09-02 and recorded a real baseline in
`GOALS.md`, later corrected 2026-09-03 once a second read showed the first
number was mislabelled (`session_id` in Umami is the visitor, not the
session): 52 visitors across 144 visits in 30 days, 21 sessions in the last
7, 1 organic visit from Bing, 0 from Google. This is a one-time manual pull,
not a live feed, so it will go stale the same way any hand-transcribed
...
| 2.7 | **One image generation route** (issues #1, #2, #18, #19, #20) | any route that produces a usable image without Phil pasting prompts by hand | 2.0 | **Route found and built 2026-09-04; the decision is now a payment, not a design.** `GEMINI_API_KEY` reaches three image models and `ops/generate_card_art.py` drives them from the frozen Style Bible. Every request returns HTTP 429 because Google does not offer image generation on the free tier at all. Whole backlog of 222 images costs $8.66 to $29.76. `OWNER-ACTIONS.md` 1b, one upgrade click. |
| 2.8 | ~~Stripe business website field still reads Ledgerium (issue #21)~~ | **Verified fixed 2026-09-06 by reading the live account, not the record.** acct_1U5rDs6OlZmKL8mF business_profile now reads name `6S Success`, url `https://6s-success.com`, support email `support@6s-success.com`, statement descriptor `6S SUCCESS`. Nothing on it points at ledgerium.ai. This row had been carried as open and blocked on Phil while it was already correct, which is the cost of a backlog nobody re-measures. | 0.1 | **done, no longer needs anybody** |
...
**2.10 correction, found and fixed 2026-09-05, this operator, Prompt 9's third cycle (5B.11), picked up as the exact candidate the second cycle's own retro named rather than a fresh sweep.** The fix above says it checked "either side was a real number." It did not: `mergeDone()` and `restore()` both validated only the incoming value; the value already in `existingDone`/`state.done` was still used unvalidated, and `Math.min(a, b)` with a corrupted `a` produces `NaN` exactly as it does with a corrupted `b`, even when `b` is perfectly good. Reproduced directly in `node` before writing anything: `mergeDone({ "A|Z|sort": "corrupted-legacy-value" }, { "A|Z|sort": 1700000000000 })` returned `NaN` for a card whose incoming value was completely valid. Not reachable from a fresh install (the app only ever writes `Date.now()` into this store), but reachable from a hand-edited local store or a value written by some earlier, already-fixed bug, exactly the case a restore feature exists to be resilient against. Fixed both files: derive a sanitised existing value (finite, positive number, else treated as absent) before it enters the merge, same rule already applied to the incoming side. Two new cases in `lib/importProgress.test.js` (`npm test` 29/29, was 27), proved to fail against the pre-fix code (the exact `NaN` reproduction above) and pass after. Widened `gate_quest_restore_validates_timestamps` in `ops/preflight.py` with a second check for the web side's existing-value guard, proved to fail on the pre-fix `quest.js` in an isolated worktree and pass on the fix, restored. Re-fingerprinted `quest.js` after the edit (`ops/fingerprint_assets.py`), verified `python ops/preflight.py`/`--deep` both clean, full `ops/tests/test_*.py` (23 files), `check_urls.py` (187/187), `audit_pages.py` (0 findings), `audit_catalog.py` and `affiliate.py --check` (162 documents) all pass, `EXPO_OFFLINE=1 npx expo export` clean on both platforms (551 iOS/550 Android modules, unchanged from the last recorded figures). Full account in `docs/future-state/OPPORTUNITY-BACKLOG.md` and `LEARNING-LOG.md` L-APP-005.
...
| 5B.11 | Prompt 9: continuous target state and autonomous improvement | the app's own improvement loop runs | 1 | **fifteenth run done 2026-09-06, operator, following the fourteenth run's own "try to break it" instruction, no theatre found this time, one real documentation staleness fixed instead.** Read five more preflight gates adversarially, the fourteenth run's own named next step: `gate_dashboard_live_links_carry_forward`, `gate_dashboard_deploy_carry_forward`, `gate_video_slug_single_source`, `gate_cardtext_corpus_integrity`, `gate_deck_art_withheld`, `gate_zone_heroes_stable`. All six already prove themselves against synthetic adversarial inputs or check real shipped output independent of how it was produced, the two shapes the fourteenth run's fix moved `gate_room_images_stable` into; none had the vacuous-comparison bug that gate had. Widened the pass rather than report a clean sweep with nothing shipped: checked `GOALS.md`'s O1 "Published videos" row against `ops/state-checkin.json` directly rather than trusting `gate_goals_published_videos_current`'s own numeric-only check. The count (12) still matches and the gate correctly passed, but the row's own citation date (2026-09-03 13:35) was three days behind the hourly pipeline that feeds it, which has kept running successfully since the permission fix and reconfirmed 12 again at 04:51 today. Fixed the citation and recorded the reconfirmation, the same "count is right, date lied about how current it is" shape CLAUDE.md 0.0 treats as a defect in the file. No new preflight gate: the substantive number was already gated, and gating a citation's own prose date is not a check that class of defect elsewhere in this document already needs. `preflight.py` clean (9 warnings), `check_urls.py` 187/187, `audit_pages.py` 0, `audit_catalog.py` 0, `affiliate.py --check` 162 documents, mobile `npm test` all three suites passing, full `ops/tests/test_*.py` suite run. **Next:** five more preflight gates from the untested tier (`gate_agents_in_sync`, `gate_sync_page_links_scans_js`, `gate_hero_prompt_budget_checked`, `gate_card_prompts_desktop_only`, `gate_card_corpus`), or a fresh `ops/*.py` cold read if that lane runs dry. **sixteenth run done 2026-09-06, operator, closed the fifteenth run's own named lead and found the lane itself is now exhausted.** Adversarially tested all five named gates in an isolated `git worktree`: planted a real regression for each (banned term in card copy, a removed `image_style.check()` call, a removed `require_desktop_sources()` call site, `.js` dropped from `sync_page_links.discover_files()`, a drifted and a missing installed agent file) and watched each fail naming the exact defect, then restored and reran clean. All five proven genuine, no theatre, no bug found. Widened the check past this one batch: grepped every one of the 105 `gate_*` function names in `preflight.py` against every name ever mentioned anywhere in this log. All 105 now appear at least once, so the untested-by-name method that found real defects on runs nine through fifteen (`gate_room_images_stable`, `gate_icons_current`, and others) cannot surface a sixteenth candidate; the next cycle needs a different angle, named in `ops/NIGHTLY-LOG.md`'s own entry for this run. `preflight.py` clean, all 24 `ops/tests/test_*.py` run individually (0 failures; `test_generator_ownership.py` genuinely takes several minutes, confirmed not a hang), `check_urls.py` 187/187, `audit_pages.py` 0, `audit_catalog.py` clean, `affiliate.py --check` 162 documents, mobile `npm test` all three suites, CI confirmed green on current HEAD directly. **Next:** re-verify a narrative-only gate proof with an actual worktree test, or resume the `ops/*.py` cold-read lane. **fourteenth run done 2026-09-06, operator, a real gate-theatre defect found continuing the ops/*.py cold-read lane.** Read the next five low-mention files (`build_kit_page.py`, `build_mobile_corpus.py`, `import_room_images.py`, `owner_inbox.py`, `stripe_setup.py`). `build_mobile_corpus.py --check` passes (114 zones, 684 cards). `owner_inbox.py` and its `preflight.py` wiring are correct. `import_room_images.py`'s own `gate_room_images_stable` in `preflight.py` claims to prove its safety net holds by calling `reconcile(figures(), committed)` directly, but `figures()` keys by chapter number with `(room, [(file, alt), ...])` values, not by room name with `[{"file":..,"alt":..}, ...]` the way `reconcile()` and `committed` both expect. Every comparison inside the gate's shrunk-check therefore read `manifest.get(room_name)` against a dict keyed by chapter number, always `[]`, which happened to still produce the right "preserve everything" answer for the wrong reason and could not have failed on a genuinely broken `reconcile()`. Proved this empirically before fixing: monkeypatched a `reconcile()` that does not preserve anything and the gate's `shrunk` list still came back empty. Fixed by adding `fresh_manifest()` to `import_room_images.py`, the one place the room-keyed, exists-on-disk-filtered shape gets built, used by both `main()` (no behaviour change, same MISSING-on-disk prints and same manifest written) and the gate (which now calls `mod.fresh_manifest(mod.figures())`). Proved the fix works the STEP 10b way: planted the same broken `reconcile()` in an isolated `git worktree`, watched `gate_room_images_stable` fail naming all nine rooms, restored, reran clean. `git diff -- ops/room-images.json` empty: the real committed manifest never changed. `python ops/preflight.py` clean (9 warnings, one fewer than last cycle since `core.hooksPath` was re-enabled), `check_urls.py` 187/187, `audit_pages.py` 0, `audit_catalog.py` 0, `affiliate.py --check` 162 documents, full `ops/tests/test_*.py` suite (run individually) all exit 0. Also read `stripe_setup.py` (150 lines: two deliverable consulting products, idempotent by lookup_key, correctly matches `STATUS.md`'s live-catalogue description, no gate attached to adversarially test) and confirmed `build_kit_page.py` is the tenth data point already fixed and wired into `gate_generator_ownership`'s own generator list, per that gate's docstring. Neither got the same fail-then-pass adversarial test as `import_room_images.py`, since neither has a safety-net gate of that shape to break. **Next:** apply the "try to break it" test to any other preflight gate whose docstring claims to prove a fix rather than just check current state. **twelfth run done 2026-09-06, operator.** Checked the eleventh run's own named next step: `PRD.md` Section 5's ten protected properties against `CURRENT-STATE-AUDIT.md`'s ten strongest parts, item by item; consistent, no drift. The "12 contrast pairs" claim is no longer a hand-typed number that can drift: `gate_mobile_badge_contrast` computes the real ratio from `App.js` on every run. Widened past the mobile-doc lane once it came back clean and reran the full customer-facing audit suite cold: `audit_pages.py` (191/0), `audit_visual.py --all` (193 pages, 0 findings in every category), `check_urls.py` (187/187), `audit_catalog.py` (0 findings against 159 live SKUs), `affiliate.py --check` (162 documents), `link_graph_report.py` (0 orphans). All clean. No new Phil commit on `ON-DEVICE-TEST.md`/5B.4 since 2026-08-31, so the on-device wall stands. No code or gate changed this cycle: nothing found to fix. **eleventh run done 2026-09-06, operator.** Picked the tenth run's own named next step: checked `README.md`/`package.json` against `App.js`/`lib/*.js`. "No network calls," "no account," "no streaks" and the corpus counts (20 rooms/114 zones/684 cards) all still hold, verified directly (grep for fetch/XHR/axios/http, `build_mobile_corpus.py --check`). The README's opening line did not hold: "1,131 packages... 539 modules, 1.73 MB," dated 2026-08-31, no longer reproduces, and the module figure cannot be pinned down at all here (two consecutive `npx expo export` runs, no source change between them, gave iOS 552 then 526 modules). A clean `npm install` against the same unchanged lockfile now resolves 1,133 packages, not 1,131, cause not established. Fixed by removing the fragile specific number rather than swapping in a new one that would go stale the same way: the README now records the reproducible package count with its own instability noted, and says the module/size figure needs re-measuring live rather than trusting the file. No new preflight gate: the number is not stable even within one session, so gating it would fail on noise, not a regression. `npm test` 36/36 unchanged, `preflight.py` clean (9 warnings, standing set). Full account in `docs/future-state/CYCLE-PLAN.md` and `LEARNING-LOG.md` L-APP-011. **ninth run done 2026-09-05, operator.** Ninth run: checked `lib/eventLog.js`'s `formatForDisplay()` against `ON-DEVICE-TEST.md`'s Diagnostics description now that `card_drawn` exists; clean (the display builds its summary generically from whatever is in the log, no hardcoded category list to go stale). Used the rest of the cycle to compare `site/assets/js/quest.js`'s `restore()` against `lib/importProgress.js`'s `mergeDone()` side by side, since the 2.10 fix and its correction had fixed the same NaN/silent-erasure bug class in both files without checking whether the two stayed equivalent afterward. Found `mergeDone()` alone is not safe against a corrupted incoming value: its existing-value validation lives in the function itself, but its incoming-value validation had been moved into `parseBackup()`, its only real caller today, so calling `mergeDone()` directly (bypassing `parseBackup`) reproduced the identical NaN bug the 2.10 fix and its correction had already closed twice on the existing-value side. Never reachable in the shipped app (the one call site always calls `parseBackup` first), confirmed by grep before treating it as latent rather than live. Fixed with a shared `sanitizeTimestamp()` helper applied to both sides inside `mergeDone()`, matching `restore()`'s self-contained shape; two new test cases call `mergeDone()` directly with a corrupted and a negative value, proved to fail on the pre-fix code in an isolated worktree and pass restored. `npm test` 36/36 (was 34), `preflight.py`/`--deep` clean, `expo export` both platforms unchanged (552 iOS/551 Android). No new preflight gate: `gate_mobile_js_tests` already runs every `lib/*.test.js` file and fails on any nonzero exit. Full account in `docs/future-state/CYCLE-PLAN.md` and `LEARNING-LOG.md` L-APP-009. **eighth run done 2026-09-05, operator.** Eighth run: per the seventh run's own named next step, re-checked `docs/product/PRD.md` Section 5 (properties the port must protect) against `App.js`. Found item 4, deep-linking into a zone or room, named a protected property in both the PRD and `docs/audit/CURRENT-STATE-AUDIT.md`'s "ten strongest parts" list, the same framing given to the recommendation engine and analytics, but unlike those two never carried into either document's actual gap list. Grepped `App.js`, every `lib/*.js` file and the Expo config for `Linking`, `scheme` or any URL-parsing code: none exist, confirming the gap is real, not a stale claim. Fixed both planning documents, adding it as gap 8 in the PRD's Section 6 build order (numbered last, not by priority, to avoid renumbering the gaps already cited by number in Sections 4 and 11) and as item 7 in the audit's own gap list. No code changed and no feature built: Section 6's own sequencing rule holds building any gap until 5B.4 (device verification) clears, and this cycle did not override it. Full account in `LEARNING-LOG.md` L-APP-008. **seventh run done 2026-09-05, operator.** Recurring by the prompt's own design, not a one-time completion. Fifth run: checked `ON-DEVICE-TEST.md`'s eleven checks against current `App.js`; a side-by-side text read found all eleven matching, but transpiling the source (not just reading it) showed check 6's expected line is not what the app actually renders, a JSX line break between plain text and a `{}` expression dropping the separating space. Fixed by extracting the sentence to a tested pure function and gated the whole defect class in `preflight.py`. Sixth run: extended the same transpile scrutiny to the four extra checks (12 to 15); no defect found, no code changed; closed the transpile-level review of all 15 checks against current source. Seventh run: checked `lib/eventLog.js`'s output against `ON-DEVICE-TEST.md`'s own Diagnostics promise ("cards drawn, done, skipped, zones finished, stops, and import attempts") and found `card_drawn` was never actually recorded anywhere in `App.js`, despite the app's own header comment repeating the same promise. `card` recomputes regardless of which screen sits on top of it (the finished-zone recap, the stopping screen), so it was never the same fact as a card being visibly drawn. Fixed with `isCardVisible()` in `lib/pickCard.js` and a `useEffect` in `App.js` recording `card_drawn` on the transition to visible; new `gate_mobile_diagnostics_promise_kept` in `preflight.py`, proved to fail on the exact regression in an isolated worktree and pass restored. `npm test` 34/34 (was 33), `preflight.py`/`--deep` clean, `expo export` both platforms clean (module-count noise across cache states noted and not treated as a defect, see `CYCLE-PLAN.md`). Full account in `docs/future-state/CYCLE-PLAN.md` and `OPPORTUNITY-BACKLOG.md`. |
...
| 6.56 | ~~`ops/stripe_brand.py`'s own usage docstring told an operator to run `--apply`, but the code checked `"--draw" in sys.argv`; every sibling Stripe tool uses `--apply`, so the one documented command silently did nothing~~ | the code checks `--apply`, matching the docstring, the runtime's own gap message and every sibling script; `KEY = secret_key()` (a module-level credential read that made the module uncheckably import-fails with no credential, the same anti-pattern `stripe_catalog.py`'s own `key()` docstring warns against by name) made lazy the same way, so a test or a future gate can import it without one | 0.1 | **done 2026-09-06, operator** |
| 6.57 | ~~`ops/owner_inbox.py`'s `main()` returned immediately whenever `unread_from_owner()` reported no credential, silently skipping `unread_needing_action()` (the third-party affiliate/dispute/chargeback scan) even when the mailbox was otherwise fully reachable and only `OWNER_EMAIL` specifically was unset, the identical "unchecked read as nothing to do" shape this file's own docstring names as an eight-day incident, moved one level up into the checker itself~~ | the two checks run independently regardless of which credential either is missing, `ops/tests/test_owner_inbox.py` proves a real third-party finding is never swallowed when only the owner-check credential is absent, fail-then-pass proven in an isolated worktree | 0.1 | **done 2026-09-06, operator** |
...
| 6.59 | ~~`ops/wire_legal_strip.py` (owns the "Privacy - How we make money - Affiliate disclosure - Terms - Accessibility - Safety notice" strip on all 188 pages) is never called by any generator, any gate, or any CI workflow, confirmed by grepping the whole repository for its name; its own docstring already names the gap this leaves, that `gate_footer_consistent` only proves every page's strip matches every other page's, so a strip wrong the same way on all of them, including a dropped FTC affiliate-disclosure link, would still read "consistent"~~ | new `gate_legal_strip_current` in `preflight.py` runs `wire_legal_strip.py --check` against the canonical link table itself, proved to fail on a planted regression (the affiliate-disclosure link removed from the table, 189 pages went stale) and pass clean restored, in an isolated worktree; currently clean on `main`, so this closes a coverage gap rather than a live defect | 0.1 | **done 2026-09-05, operator** |
...
| 6.70 | ~~`ops/build_deck_pdf.py`'s output (the free print at home deck PDF, linked from `deck.html` and `deck-gallery.html`) is not one file, it is two: the generator writes `build/6S-Entryway-Deck-PrintAndPlay.pdf`, and 5.8 copied that by hand into `site/downloads/`, the copy nginx actually serves. Nothing since checked the two stay in sync, and `gate_generator_ownership` cannot cover this generator at all, the same `build/heroes/`-shape gap as `build_zone_pages.py`: its own source renders live in `build/cards-rendered/`, gitignored and Desktop-only~~ | new `gate_deck_pdf_download_current` in `preflight.py`, comparing the two already-committed files byte for byte, no regeneration needed so it runs the same in every environment; proved to fail by appending a byte to the served copy in an isolated worktree, restored, confirmed clean. Both copies currently match (no live drift found) | 0.05 | **done 2026-09-04, operator** |
| 6.71 | ~~`gate_no_stale_session_label` (6.68) fixed three sibling documents but the regex it shipped with was narrower than the wording it was meant to catch: it only matched the exact lowercase phrase "sessions in the last 30 days", nothing else. Two more live instances of the same retired "47 sessions" traffic figure had survived past it, both stating it as current fact a full day after `GOALS.md` corrected itself: `STATUS.md` section 30's own "Business Data Knowledge" line, in ALL CAPS ("47 SESSIONS AND 328 PAGEVIEWS IN 30 DAYS"), and `BACKLOG-2026-H2.md` item 1.1's own note ("47 sessions/30 days"), a file the gate did not even check~~ | fixed both files' text (52 visitors / 144 visits / 30 days, matching `GOALS.md`). Widened the gate's regex (case-insensitive, matches "sessions" within 40 characters of "30 days" under any phrasing found) and added `BACKLOG-2026-H2.md` to the checked list. Adding that file first reintroduced a real false positive: its own 6.68 entry, and `STATUS.md`'s "Updated By" changelog line, both legitimately quote the retired phrase in double quotes while narrating that it was fixed. Fixed by stripping double-quoted spans before matching rather than excluding the file again, so a bare unquoted claim still fails while a quoted historical citation does not. Proved against the real committed text of both files (not synthetic strings): a planted unquoted regression in either file fails, the real fixed files pass clean, and the pre-existing quoted citations in both files' own changelog entries do not trip it | 0.1 | **done 2026-09-04, operator** |
| 6.72 | ~~`site/quest.html`'s `.zfig figcaption` (the "illustration, not a photograph of a real home" disclosure under the room-picker preview image) read `#8C8478` on the page's `#F7F2E9` background, 3.31:1 against the 4.5:1 WCAG floor, only found by running `preflight.py --deep` directly rather than citing a prior cycle's clean claim of it~~ | darkened to `#6A625A`, the exact color `site.css`'s `.zone-figure figcaption` already uses for the same disclosure on every zone page (5.37:1, real margin); no new gate needed, `gate_visual_audit` already covers this and caught it correctly the moment it ran; `test_quest_flow.py` reran clean, no functional regression | 0.05 | **done 2026-09-04, operator** |
| 6.73 | ~~`ops/build_thumbnails.py` (one mention in `NIGHTLY-LOG.md`, its own build commit) renders a designed YouTube thumbnail for all 114 zones, read directly by `ops/youtube_upload.py`, and nothing on the dashboard said this asset existed, the same hiding-finished-work shape already fixed for the four video cuts, the captions, the metadata and the social cards it sits beside. The same file also carried its own hand-copied slug function, agreeing with `video_zone.zone_slug()` only by coincidence since no current room or zone name has a slash or comma in it, the exact single-source-of-truth gap `gate_video_slug_single_source` already fixed for five sibling files~~ | new `thumbnail_line()` and a measurement block in `ops/dashboard.py`, wired into both dashboard tables; `build_thumbnails.py` now calls `vz.zone_slug()` directly instead of its own copy (all 114 already-built filenames confirmed to match the canonical slug exactly, so no renaming needed); new `gate_dashboard_thumbnails_live` in `preflight.py`, proved to fail on the real regression shape (a stale hardcoded line, matching how this exact bug looked every prior time) and pass on the fix, in an isolated worktree | 0.15 | **done 2026-09-04, operator** |
| 6.74 | ~~`audit_visual.py --all`'s widened crawl (6.41) found 36 real heading-level jumps in the free sample eBook, the site's primary lead magnet, noted in cycle twenty's own retrospective as found but left unfixed~~ | every chapter's "In this chapter" box (h1 to h4, 30 instances) and each of the six "Checklist" boxes (h2 to h4) retagged to h2/h3, matching the level of the section they actually sit beside or inside; `.what h4`/`.checklist h4` in `book.css` updated to match, verified pixel-identical against the old markup in headless Chromium before shipping | 0.2 | **done 2026-09-04, operator** |
| 6.75 | ~~The same audit's `landmark/h1 problems` row named two pages missing `<main>`: the sample eBook and the retired Entryway print-and-play notice, both deliberately outside `wire_landmarks.py`'s site-wide pass because they have no site header/footer to distinguish themselves from, a reason that covers header/footer but never covered main~~ | both wrapped in `<main id="main">`; the eBook's closing line also retagged from `<section class="bookend">` to `<footer class="bookend">` (no CSS targets it by tag); `landmark/h1 problems` 2 to 1, the remaining row is the eBook's inherent 31 chapter-level h1s, which the audit's own comment already treats as context once main exists, not a fixable defect | 0.1 | **done 2026-09-04, operator** |
| 6.76 | ~~Running preflight's own `gate_tests()` while a second, separately launched pass over `ops/tests/*.py` was still finishing produced a real-looking gate failure with no product defect behind it: `test_audit_catalog.py`'s scratch fixture lived at one fixed path, so two overlapping runs could share it, third instance of the `_visual_probe.html`/`test_affiliate.py` fixture-collision shape~~ | fixture renamed after its own pid so two runs cannot share a path; `gate_no_stray_probe_files` widened to also sweep for a leftover `_audit_catalog_fixture*.html`, gitignored so one can never be committed; proved the widened gate fails on a planted stray file and passes clean once removed | 0.1 | **done 2026-09-05, operator** |
| 6.77 | ~~6.76 named this class as three instances and fixed the two it already had names for. Checking every `ops/tests/*.py` that writes a scratch page under `site/` found six more, none swept by name: `test_audit_links.py`'s `_audit_link_fixture.html`, `test_gates.py`'s five `_gate_fixture_*.html`, `test_measure_events.py`'s five `zones/_measure_probe_*.html`, `test_mobile_overflow.py`'s three `_fixture_*.html`, `test_quest_flow.py`'s `_quest_flow_probe.html` and `test_web_to_mobile_import.py`'s `_import_probe.html`, every one cleaned up only in a `finally` block or a context manager's `__exit__` that a SIGTERM from an outer timeout can skip, same exposure as the two already fixed~~ | `.gitignore` and `gate_no_stray_probe_files` both widened from two literal filenames to the convention itself, `site/**/_*.html`, matched against a real check that no committed page anywhere in `site/` starts with an underscore (`git ls-files site`, zero matches); a script written next month needs no matching edit here as long as it keeps the prefix; proved the widened gate fails on a planted stray file (a fresh name, not one of the two it already covered) and passes clean once removed, in an isolated worktree | 0.1 | **done 2026-09-05, operator** |
| 6.78 | ~~`ops/send_questions.py`, the drafted "things only you can do" email to Phil, had never been read since it was written. Two of its four BLOCKING items were false: it still asked him to complete Stripe live onboarding because the payment links were "test links", months after Stripe went live and took a real sale on 2026-08-21; it still asked him to fill in book front matter, months after this operator answered every field itself on 2026-08-27 with nothing left for him to decide. A third asked for a Listmonk "list UUID" when the real blocker, diagnosed since, is a branding/SMTP identity decision (OWNER-ACTIONS.md item 7). A DECISIONS entry separately hardcoded a retired social-unit count months after `dashboard.py` fixed the identical hardcode in itself and said so in its own comment, never propagated here~~ | the two resolved items removed, the Listmonk ask corrected, and the social-unit figure now read live from `corpus_index.build_index()` instead of a frozen number, mirroring `gate_dashboard_social_units_live`'s own fix for the sibling copy; new `gate_send_questions_current` in `preflight.py`, proved to fail on the exact planted regression (the old "test mode" wording, and separately a reintroduced hardcoded figure) and pass on the fix, in an isolated copy | 0.2 | **done 2026-09-05, operator** |
| 6.79 | ~~`ops/check_ledgerium.py`'s own docstring for `gate_ledgerium` claims it "watches the four prices and the webhook," but the webhook check only lived in a branch reachable when the ambient Stripe key IS Ledgerium's own account, which never happens by design (that key lives only at `/docker/ledgerium/.env` on the VPS). Every realistic invocation instead delegates to `ops/ledgerium_price_check.py` on the VPS, which checked only the four prices, never the webhook: the one failure mode named in the file's own comment as worst case ("subscriptions would be paid for and never activated") could happen and this gate would still print "intact." Separately, `EXPECTED`/`WEBHOOK`/`WEBHOOK_EVENTS` were hand-copied into both files, the same single-source-of-truth gap already fixed five times this week for `video_zone.zone_slug()`~~ | `ledgerium_price_check.py` restructured into a testable `check(key)` that covers prices, products and the webhook, the function that actually runs with Ledgerium's real key on the VPS; `check_ledgerium.py` imports its constants instead of duplicating them and calls the same function on its own rarely-reached direct-key branch. New `ops/tests/test_ledgerium_check.py`, five cases, proving the webhook check fires (missing, disabled, missing events, all-healthy) and that no second copy of the constants exists; proved to fail on the exact regression shape (a webhook-check block stripped from an otherwise-working `check()`, returning `[]` with the real failure silently unreported) in an isolated worktree, restored | 0.15 | **done 2026-09-05, operator** |
| 6.80 | ~~`ops/NIGHTLY-LOG.md` states its own convention as "newest first," but the ninth cycle of 2026-09-05 appended its entry to the physical end of the file (after every 2026-09-04 entry) instead of prepending it, self-mislabelling as "first today" in the process; the tell is STEP 1's "read the last four entries," which a session can read as the literal end of the file (what `tail` gives) rather than the top a newest-first file actually wants~~ | moved the misplaced entry to the top and corrected its ordinal to "ninth"; new `gate_nightly_log_ordering` in `preflight.py`, checking only that the file's newest date never reappears after the sequence has moved on to an older one (deliberately narrower than "every date non-increasing," which fires 12 false positives on the legitimate pre-"newest-first" chronological section written across prior weeks); proved to fail on the real pre-fix file (named entry #389 exactly) and pass clean on the fix | 0.15 | **done 2026-09-05, operator** |
| 6.81 | ~~`gate_visual_audit` only ever parsed 2 of the 9 categories `ops/audit_visual.py` computes on the real rendered DOM (text contrast, image distortion); the other 7 (broken images, missing image dimensions, missing alt text, heading level jumps, unlabelled inputs, missing focus styles, landmark/h1 problems) were computed fresh every deep run and silently discarded, so a regression in any of them could ship with preflight still saying clean. Not theoretical: "landmark/h1 problems" was already 1, on the free 30-chapter sample eBook, the site's primary lead magnet, h1=31 (a cover `<h1>` plus one `<h1 class="title">` per chapter, each chapter authored as its own standalone document before being assembled into one combined download; to a screen reader, 31 same-level top headings carry no book/chapter hierarchy at all)~~ | `gate_visual_audit` now reads all 9 categories, proved to fail on the exact planted regression (the 30 chapter titles reverted to `<h1>`) in an isolated worktree and pass clean restored; the real defect fixed at the source, not the shipped copy: `content/book/.../Sample (Chapters 1-30).html`'s 30 per-chapter `<h1 class="title">` demoted to `<h2 class="title">`, `content/book/assets/book.css`'s two `h1.title` rules generalised to `.title` (checked first that no other element on the page carries that class), regenerated via `ops/build_sample_html.py --apply` (the file's own owning generator) and re-fingerprinted; verified with a real headless screenshot that the demoted title still renders at full display size and weight | 0.2 | **done 2026-09-05, operator** |
| 6.82 | ~~CLAUDE.md's "zero em dashes and en dashes anywhere including code comments" had two gates covering two slices (`fix_dashes.py` on root `*.md`/`claude/**/*.md`, `audit_pages.py` on shipped `site/**/*.html`) and nothing covering the code surface the rule names directly: `ops/*.py`, `mobile/**/*.js`~~ | new `gate_no_stray_dashes` in `preflight.py`, scoped to `ops/*.py` and `mobile/quest-app/**/*.{js,jsx}` with a checked-by-name exemption for the handful of files whose job is detecting the two characters (`fix_dashes.py`, `audit_pages.py` itself, `build_epub.py`, `dashboard.py`, `build_articles.py`, `import_chapter_svgs.py`, `merge_cardtext.py`); proved to fail on a planted comment in a non-exempt `ops/*.py` file and in a mobile `.js` file, in an isolated worktree, and pass clean restored | 0.2 | **done 2026-09-06, operator** |
| 6.83 | ~~`gate_roadmap_prices_current`'s page-count check called `all_pages()` with no filter, so a stray `site/**/_*.html` scratch file existing mid-flight (`audit_visual.py`'s own probe, or any of the seven sibling test fixtures `gate_no_stray_probe_files` already sweeps for) inflated the count by one and the gate reported a false drift against `ROADMAP-2026-2029.md`; hit for real by `preflight.py --deep`'s own sanctioned concurrency (it runs `audit_visual.py` while other checks proceed), reproduced directly with a planted `site/zones/_repro_probe.html` (191 became 192)~~ | filtered the underscore prefix locally inside this one gate's count rather than in `all_pages()` itself, since `all_pages()` is deliberately blind to that prefix everywhere else (three sibling `test_gates.py` cases plant `_gate_fixture_*.html` fixtures and rely on `gate_stale_claims`/`gate_unsourced_stats`/`gate_nav_current` finding them through this exact function, confirmed by first trying the global fix and watching those three break); new case in `ops/tests/test_gates.py`, proved to fail on the exact pre-fix shape (a stray fixture counted as a real page) and pass on the fix | 0.1 | **done 2026-09-06, operator** |
| 6.84 | ~~`dashboard.status_of()`, the single function that sets the executive dashboard's RED/YELLOW/GREEN headline, had only 2 of its 6 outcomes ever exercised anywhere (both RED, both the dead-links case, one plain and one carried-forward)~~ | the no-payment-route RED, the unreachable-GitHub YELLOW, the open-P0 YELLOW and the plain GREEN fallback all proved fail-then-pass in an isolated worktree, gated in `gate_dashboard_severity` | 0.2 | **done 2026-09-06, operator** |
| 6.85 | ~~`ops/build_zone_pages.py` imports `hazard_icons.icon()` inside a bare `try/except Exception: return ""`, so a broken `hazard_icons.py` or a moved `content.json` would silently blank the safety icon on all 114 zone pages' hazard lists with no error anywhere in the build; safety is the fourth S and P0 under CLAUDE.md's own priority order, and nothing checked this~~ | new `gate_hazard_icons_current` in `preflight.py` checks both that `hazard_icons.py` itself still imports and passes its own coverage assertion, and that the live `class="hz"` count across every `site/zones/*.html` file matches `content.json`'s total `watch_for` entries (252 of 252 today), so a fallback returning `""` for everything cannot pass silently either; proved fail-then-pass in an isolated worktree two ways (a missing `ICONS` entry, and icons stripped from a live page to simulate the fallback), restored | 0.2 | **done 2026-09-06, operator** |
| 6.86 | ~~`gate_stripe_one_product_per_sku` (added same day, commit `524bcd0d`) called `stripe_dedupe.duplicates()`, which reports a missing Stripe credential with `sys.exit(...)`; `SystemExit` is not an `Exception` subclass, so it walked past both the gate's own `except Exception` and `run_gate()`'s safety net and killed the whole preflight process with no summary at all, in the one credential state (no `.env.secrets`) every sandbox this project has run in for its entire history. Confirmed live, not just reproduced: CI's `checks.yml` run 308 on `524bcd0d` was red on exactly this~~ | the gate's own except now also catches `SystemExit` (restoring its documented warn-not-fail intent), and `run_gate()` catches it too so any other gate with this shape cannot take the single mandatory gate down again; proved in an isolated worktree in three states (gate-only fix: contained but reports FAIL instead of the intended WARN; both fixes reverted: reproduces the exact CI crash byte for byte; both restored: clean) | 0.2 | **done 2026-09-06, operator** |
| 6.87 | ~~`ops/stripe_invoice.py`'s own docstring promises "It refuses to run against a live key unless `STRIPE_ALLOW_LIVE=1`, the same guard the product and link scripts use", but `main()`'s actual guard was `if live and a.send and ...`. `stripe_catalog.py` and `stripe_dedupe.py`, the two scripts the docstring names as using "the same guard", gate on any write (`apply_it`), not on a final send step. A bare `--draft` run against a live key with no `STRIPE_ALLOW_LIVE` set still POSTs a real customer, a real invoice and a real invoice item to the live Stripe account; only the email send was ever gated, a copy-vs-control mismatch CLAUDE.md names as P0~~ | guard moved above the send check so it covers the whole function, matching the sibling scripts and the docstring; new `ops/tests/test_stripe_invoice.py`, four cases, proved fail-then-pass in an isolated worktree (pre-fix: a live `--draft` reaches the stubbed network call the guard should have stopped; post-fix: clean, and a live run with `STRIPE_ALLOW_LIVE=1` still works) | 0.1 | **done 2026-09-06, operator** |
| 6.88 | ~~`ops/shrink_sample.py` (compresses the free sample eBook PDF, the site's lead magnet) verified its own output was not corrupted with `assert chk.page_count == doc.page_count if not doc.is_closed else True`, written after `doc.close()` had already run two lines above. `doc.is_closed` was therefore always `True`, so Python's ternary always took the `else True` branch: the page-count comparison itself never executed, for any input. A shrink that silently dropped every page past 10 would have passed this assert; confirmed by reproducing the exact expression standalone before touching the file~~ | extracted the whole post-shrink check into `verify_shrunk(out, expected_pages)`, called with `expected_pages` captured before `doc.close()` (accessing `.page_count` on a closed pymupdf document raises `ValueError: document closed`, which is why the broken guard existed in the first place, just written wrong); new `ops/tests/test_shrink_sample.py`, built on real pymupdf documents (no PIL needed, so it runs in this sandbox), proved fail-then-pass in an isolated worktree by reintroducing the exact pre-fix expression and watching it accept a PDF with the wrong page count, naming the regression | 0.1 | **done 2026-09-06, operator** |
| 6.89 | ~~`ops/wire_breadcrumbs.py` (writes the `BreadcrumbList` JSON-LD for the 27 root-cause articles) incremented one `done` counter for every article that merely qualified for markup, whether or not the computed bytes actually differed from what was already on disk, then printed it under an action verb: "marked up N article breadcrumb(s)" for a real run, "would mark up N" for `--check`. Against the live tree, where all 27 already carry correct, current markup, this read "would mark up 27" and "marked up 27" on every single run, including runs that changed zero bytes, the same vacuous-count shape 6.88 fixed one file over. Not hypothetical: multiple nightly cycles read this exact line and logged the script as "still unrun" or "a real, unapplied SEO gap", and only a cycle that ran it for real and diffed the working tree ever caught that the claim was false~~ | split the single counter into `changed` (bytes actually written, or would be under `--check`) and `already` (qualified, byte-identical to what main() would produce), and changed the verb from "marked up"/"would mark up" to "changed"/"would change" so the two modes cannot disagree with each other; new `ops/tests/test_wire_breadcrumbs.py`, six assertions against a synthetic site tree, proved fail-then-pass by reverting to the single-counter version and watching a second no-op run on an already-correct page still report it as changed | 0.1 | **done 2026-09-07, operator** |
| 6.90 | ~~`ops/build_image_prompts.py`'s `main()` hardcoded the word "nine" into the committed `content/images/prompts/tier-0-prompts.md`'s own top heading and opening line, from when tier 0 was three before/after pairs plus three safety drawings. The safety-drawing prompts were removed later (five coded hazard icons replaced them) and tier 0 has been 6 images since, but the hardcoded text was never updated: the live file read "Start here: nine images" and "Nine images, one evening" two lines below a line that plainly said "6 images.", the exact copy-vs-control disagreement CLAUDE.md STEP 6 calls a P0 trust defect, sitting in the one file Phil actually opens to do the work~~ | both strings now read `len(group)` instead of a fixed word; new `gate_image_prompts_tier0_count_honest` in `preflight.py` reads the committed file and checks its heading and opening-paragraph counts against its own "N images. Style anchor" line, proved fail-then-pass in an isolated worktree against the original stale file, restored | 0.1 | **done 2026-09-07, operator** |
| 6.91 | ~~Issue #21 was still open and read as one undecided question, but `ops/stripe_brand.py`'s own `check()`, the tool that exists specifically to watch this account's public identity for Ledgerium's branding, never read `business_profile.name` or `.product_description` at all, only `.url` and `.support_email`. The issue's own finding was a product description reading "Ledgerium AI's... workflow documentation platform" in full, so the one field most directly named as the problem was also the one field nothing has ever checked, and nothing would notice if it drifted back or was never actually fixed~~ | `check()` extended to compare `.name` against "6S Success" and flag `.product_description` if empty or containing "ledgerium" or "set in order"; new `gate_stripe_brand` in `preflight.py`, warn-style like its sibling Stripe checks since no sandbox here holds a credential, proved fail-then-pass in an isolated worktree against a planted regression removing exactly these two checks; 5 new cases in `ops/tests/test_stripe_brand.py` (9 total, was 4). Posted a reconciling comment on issue #21: item 1 (business website) verified fixed 2026-09-06 per 2.8, item 2 (product description) now has tooling but still needs a live `--check` run to confirm, items 3 to 4 (industry, organization) are explicitly Phil's own call, not a diagnosis gap | 0.2 | **done 2026-09-07, operator** |
| 6.92 | ~~`ops/build_youtube_metadata.py` writes a title, description and tags for all 114 zone videos plus a room playlist grouping, committed under `build/video/youtube/` (not gitignored), from `video_zone.zones()` and `build_zone_pages.slug()`/`display()` alone, no Desktop-only input required. It was never in `gate_generator_ownership`'s list, the same coverage gap already closed thirteen times for other generators (issue #26): a future zone-content edit could leave 114 video titles, descriptions or the one written-guide link in each pointing at stale or dead text with nothing to say so~~ | added to the generator list; confirmed no live drift first by running it against the current checkout and diffing (byte-identical, so this closes the gap before it opens one, not a fix for a defect already shipped); proved fail-then-pass in an isolated worktree (planted a hand edit to one committed metadata file, gate failed naming it; restored, gate passed clean) | 0.1 | **done 2026-09-07, operator** |
| 6.93 | ~~`ops/prerender_shop.py` writes shop.html's 159 product cards into plain HTML so a crawler or slow phone sees real content, not an empty `<script>` shell, but nothing re-runs it when the catalogue changes, and `audit_catalog.py`'s existing `check_price_drift` cannot see a wrong price inside that snapshot: on a shop card the blurb, chip and phase/fulfil text all sit between the product name and its price, past the 60 character tail window the generic check reads. Confirmed live, not assumed: planted a wrong price ($99 for the $19 Whole House Pack) directly in the committed `site/shop.html` and ran `audit_catalog.py`, `preflight.py` and `--deep`; all three passed clean~~ | new `check_shop_prerender()` in `audit_catalog.py`, comparing the prerendered block to the catalogue positionally (one `<article>` per catalogue entry, in catalogue order, the same order `prerender_shop.py`'s own unfiltered render produces) rather than by proximity; wired into `main()` so it runs on every page automatically via the existing `catalogue` preflight gate, no new gate needed; proved fail-then-pass with the same planted $99 price, restored; three new cases in `ops/tests/test_audit_catalog.py` (a correct snapshot, a wrong price, a short card count), built from the real catalogue rather than fixed strings | 0.2 | **done 2026-09-07, operator** |
| 6.94 | ~~`ops/video_zone_photo.py`'s docstring hardcoded "102 of the 114 zones carry a photograph... approved" as a claim about the current state, but `ops/hero-verdicts.json` now holds 107 approved (checked directly, not assumed), and the field this file actually decides on is computed fresh from that file every run. A hardcoded number in a comment does not gate anything a customer sees, but it is the same claim-vs-control shape STEP 6 and 5c warn about, sitting in the one file that explains why the video format changed~~ | reworded to state the count moves and point at running the tool for the live figure, rather than repeating a number that will go stale again; also ran a full `ops/shoot_mobile.py` sweep across all 160 live pages (top-level, all 114 zones, all 20 rooms) at a true 390px phone viewport for the first time this week rather than the 6-page default sample, 0 overflow findings, real added mobile-safety coverage even though it found nothing | 0.1 | **done 2026-09-07, operator** |
| 6.95 | ~~`ops/deploy.py` (the script that does what the Redeploy click does, the one-time-key-install fix for the eight-day payment outage) had zero test coverage on its own verdict logic, the exact blind spot 2.9's addendum already closed for `check_live_links.py`: no sandbox in this project's history has ever held `~/.ssh/6s_deploy` (reconfirmed again this cycle), so every real run here takes the "no deploy key" exit before the product-count/build-id/stylesheet-stamp branching in `main()` ever executes. Writing a test to exercise it found a real, live bug rather than a hypothetical one: when the key exists but is not yet installed on the server, `main()` unconditionally opened `KEY + ".pub"` to print the install command, with no check that the file exists, an unhandled `FileNotFoundError` instead of the graceful BLOCKED message the script exists to give~~ | `main()` checks `os.path.exists(KEY + ".pub")` first and prints a clear next step (generate the key pair) instead of crashing; new `ops/tests/test_deploy.py`, 14 cases covering every verdict branch (blocked, docker failure, pull failure, unreadable/behind/mismatched product count, build-id mismatch or unreadable, stylesheet stamp mismatch or unreadable, no-op success, real success) plus the missing-`.pub` case itself, proved fail-then-pass against the pre-fix file (real `FileNotFoundError`, not a simulated one), rides the existing `gate_tests` discovery so no new preflight gate was needed | 0.1 | **done 2026-09-07, operator** |
| 6.97 | ~~`ops/verify_deploy.py`'s honesty check on the contact page tested `"does not send email yet" not in contact`, a phrase contact.html stopped carrying once its disclaimer was reworded to "Our mail pipe is not connected yet." Checked directly against the served page's own source rather than assumed: neither phrase existed anywhere in the repository, so the check was vacuous, always true regardless of what a customer actually saw, including a hypothetical regression that falsely claimed the message had been sent~~ | rewrote the check to test today's real wording and to fail on an explicit false-delivery claim rather than on the absence of an old one; new `ops/tests/test_verify_deploy.py`, three cases (honest copy passes, a false "message has been sent" claim fails, copy with no disclosure at all fails rather than passing by default), proved against the pre-fix logic directly (confirmed it would have passed the false-claim case) since the script itself needs a live URL this sandbox cannot reach, rides `gate_tests` discovery, no new preflight gate needed | 0.1 | **done 2026-09-07, operator** |
| 6.96 | ~~`OWNER-ACTIONS.md`'s "Zone hero gaps, measured" row (item 1b) read 4 since 2026-09-04. Checked `ops/hero-verdicts.json` directly rather than trusting it: 7 zones hold a "no" verdict, not 4. Two of the seven, `mudroom--family-hook-zone` and `nursery--crib-and-sleep-zone`, had no entry in `ops/hero-subjects.json` at all, so working the old four-item list would have left three zones permanently textless with nobody flagging it. No live defect: all seven pages already correctly withhold their hero image rather than showing a wrong one~~ | wrote both missing subjects from the zones' own `done_looks_like` sentences in `content/manual/source/content.json`; corrected `OWNER-ACTIONS.md`'s table to all seven rows and the recomputed cost line (225 images, not 222); reworded `generate_card_art.py`'s stale 332-image docstring and a misleading runtime cost estimate ("all 20 rooms at deck depth", implying a scope the roadmap explicitly defers) to point at `OWNER-ACTIONS.md` instead of repeating a number that goes stale in a second place; new `gate_zone_hero_rejects_have_subjects` in `preflight.py`, checks every rejected zone has a subject and the owner-facing count matches the verdicts file, proved fail-then-pass in an isolated worktree on both the missing-subject and the stale-count regression, restored | 0.1 | **done 2026-09-07, operator** |
| 6.98 | ~~`ops/owner_inbox.py`'s `unread_needing_action()`, the third-party (affiliate/payment/domain) check written for the Impact decline that sat unread eight days (d5bde67c), had its own passing test but no caller anywhere outside the file's own bare `main()`. Nothing automated ever invoked that: `ops/inbox_agent.py`, the tool STEP 8 of the operating runbook actually instructs a cycle to run, grew its own separate affiliate/billing classifier and never imports this module. The safety net built for a named incident could only ever fire if someone typed the old command by hand, which nothing has instructed anyone to do since~~ | wired `unread_needing_action()` into `gate_owner_waiting` in `preflight.py` as a warn, alongside the existing owner-mail fail check; caught a self-introduced regression while writing it, an early return on a missing `OWNER_EMAIL` would have reintroduced the exact "unchecked read as nothing to do" bug `test_owner_inbox.py` already guards one layer down, fixed before it shipped; new case in `ops/tests/test_gates.py`, proved fail-then-pass in an isolated worktree, restored | 0.2 | **done 2026-09-07, operator** |
| 6.99 | ~~`ROADMAP-2026-2029.md` corrected its own checkout-session count 2026-09-07 (the real figure is twenty, not seven; nineteen expired, one completed, seven of the nineteen quoted a phantom $18 duplicate price archived 2026-09-06), and nothing checked whether the sibling that repeats this as current fact had been told. `STATUS.md` still stated "Seven checkout sessions have existed in total; six were abandoned" in its funnel-status prose and "1 (7 checkout sessions started, 6 abandoned)" in its metrics table, plus described EXP-001 as still unanswered when backlog item 1.3 closed it permanently AMBIGUOUS on 2026-09-03, and named 1.1 (Umami read access, partly done) rather than 1.2 (the share URL/key, still open) as the standing blocker. Same one-document-corrected-sibling-never-told shape 6.68/6.71 already fixed for the visitor/session figure, not yet caught for this one~~ | corrected all four spots in `STATUS.md` (funnel-status paragraph, metrics table, Current Highest-Level Priority section, Workstream 1's blocked-on line). New `gate_no_stale_checkout_count` in `preflight.py`, checking STATUS.md/RISKS.md/GOALS.md for the retired "seven/7 checkout sessions" wording outside quotes; proved to fail on the exact pre-fix wording (planted via a monkeypatched `ROOT` rather than a real worktree, since the check is a pure text scan) and pass clean on the real, fixed repository | 0.1 | **done 2026-09-07, operator** |
| 6.100 | ~~CI red on `checks.yml`/`publish-image.yml`: three new control documents (`REVIEW-COMMERCE-2026-09-07.md`, `REVIEW-DISCOVERY-2026-09-07.md`, `PLAN-MEDIA-2026-09-07.md`) shipped with 109 em/en dashes, and `ops/fix_dashes.py --apply` itself mishandled three real shapes while resolving them~~ | CI green again, and the tool that failed to convert them correctly, fixed at the root rather than by hand-patching the output | 0.5 | **done 2026-09-07, operator** |
| 6.101 | ~~Generator-ownership drift, real and live: the concurrent Sustain rewrite (`ff9a5d0c`) updated `content.json` for all 114 zones but never reran the generators that read it, so `build/6S-Whole-House-Print-Pack.html` (the $19 product), 74 `build/video/youtube/*.json` files, `site/assets/js/quest-data.js` (the live Home Quest app's own data) and `mobile/quest-app/assets/quest-corpus.json` (the mobile app's copy) all still served the pre-rewrite text, found by `ops/preflight.py --own`, which only runs on request and had not been run since the rewrite~~ | every drifted file matches a fresh run of its own generator, gated | 1.0 | **done 2026-09-07, operator** |
| 6.102 | ~~`gate_stripe_price_claims` (`98bd3a4a`, same day) caught only `Exception` around `stripe_catalog.list_all()`; the credential check it depends on reports a missing `.env.secrets` with `sys.exit()`, raising `SystemExit`, a `BaseException` subclass the `except Exception` clause never sees, so the gate crashed `checks.yml`/`publish-image.yml` on its first real execution in every sandbox this project runs in. The third occurrence of the exact shape 6.86 and 6.48 already name and fixed elsewhere in the same file, reintroduced in a new sibling gate the same day 6.86 was written. `BACKLOG-2026-09-07.md` (`7b61ec6e`, same day) also carried one em dash the dashes gate correctly caught~~ | caught `(Exception, SystemExit)`, matching `gate_stripe_one_product_per_sku`'s established pattern; dash fixed with `ops/fix_dashes.py --apply`; new regression case proved fail-then-pass against the reverted except clause | 0.2 | **done 2026-09-07, two concurrent operator sessions independently, reconciled in the merge** |

**6.101.** Confirmed a customer-facing gap before fixing it, not assumed: the
regenerated `build/6S-Whole-House-Print-Pack.html` contains the new, longer
Sustain text ("The folder rides to the sofa with you after dinner...") that
the previously-committed version did not, so anybody who bought the $19 pack
between the Sustain rewrite and this fix received the thin, pre-rewrite
Sustain passes while the site itself already showed the new ones. Regenerated
in the documented, safe order (`gate_generator_ownership`'s own list):
`build_zone_pages.py` through `build_youtube_metadata.py`, then
`fingerprint_assets.py`, `build_pwa.py`, `build_avif.py --wire`, then
`ops/build_mobile_corpus.py` for the mobile side, the same tool
`gate_mobile_corpus_current` already names. Verified `build_zone_pages.py`
did not repeat the 2026-08-30 zone-hero-wipe defect (no source photographs
in this sandbox): `git diff --stat -- site/zones/*.html` is empty, confirming
`hero-fallback.json`'s 107 entries protected every already-approved image.
`preflight.py --own` clean on the committed result (checked in an isolated
worktree first, then on the real tree after committing). Full account in
`ops/NIGHTLY-LOG.md`, 2026-09-07.

**6.82, this operator.** Tried a whole-tree version first (every `*.py`,
`*.md`, `*.js`, `*.html`, `*.css`, `*.txt` in the repo) before scoping down,
per STEP 5d: verify before acting rather than assume the narrow version was
enough. It found 233 files, almost all `content/**` and `build/**`: card
prompt kits, editorial review notes, superprompt files and other
pre-existing source archive material that was never subject to this rule
and is not shipped. Gating those would fail permanently on day one over
history, not a live defect, which is exactly the kind of unpassable check
`CLAUDE.md` calls theatre from the other direction. A second false lead in
the same pass: the whole-tree scan first flagged 233 files that used `grep
-P` under this sandbox's POSIX locale, which silently no-ops on `\x{...}`
byte classes above a certain range rather than erroring loudly, so an
earlier manual check with that exact command had reported zero hits
repo-wide when the true count was 233. Caught only because Python's own
`io.open(..., encoding="utf-8")` count disagreed with it; worth remembering
next time a shell one-liner is trusted as verification. Scoped the real
gate to `ops/*.py` and `mobile/**/*.js`, the code-comment surface the rule
names by name, where a violation is genuinely cheap to prevent and the
current true count is zero.
