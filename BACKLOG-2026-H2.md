...
| 6.64 | Two concurrent sessions pushed to `main` while this cycle was running (`915adecc`, `4f30b950`), and CI went red on both: `ops/tests/test_generator_ownership.py`'s own "an untouched checkout must not be reported as drift" assertion failed for real, naming `site/standards.html`. Read rather than assumed: regenerating the page from its own generator (`ops/build_standards_page.py`) dropped `data-sku="PACK-HOUSE"` from the Print Pack buy button that the committed file carries, meaning somebody had hand-patched the analytics attribute onto the shipped HTML (`measure.js` reads `data-sku` to fire buy-click tracking) without ever fixing the generator that owns the page, the exact issue #26 shape CLAUDE.md 5b warns against: the next regeneration would have silently stripped that page's buy-click measurement with no error | added `data-sku="PACK-HOUSE"` to the generator's own template line, matching `build_resources.py`'s and every other buy button's own convention; regenerated and re-fingerprinted, output byte-identical to the already-committed page, so this closes latent drift rather than changing anything live; no new gate needed, `gate_generator_ownership`'s existing test is what caught this correctly and stays as the check | 0.1 | **done 2026-09-04, operator** |
