...
- **`9b8a41ea` (18:54):** a live overclaim fixed on `site/deck/entryway-print-and-play.html` (the retired-mockup redirect notice): it told visitors "88 cards, illustrated front and back" when `ops/card-hero-verdicts.json` shows 9 of 88 currently render text-only (no photograph). Rewritten to state the real split; new `gate_print_and_play_art_count_current` in `preflight.py` re-derives the count from the verdict file so this cannot drift silently again. **This is a new, real deploy gap: production was last confirmed current at `2026-09-16T15:06:13Z` (`ops/deploy-verdict.json`, build `628eb4520b6e9ded`), before this fix landed, so this one page is live with the old overclaim until the next VPS-side deploy.** `153b89f1` below (12:31) was already inside that confirmed build.
