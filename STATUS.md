...
**Production Knowledge, RE-DERIVED 2026-09-24 21:2x, scheduled operator cycle: a newer redeploy confirmation superseded the 18:47:16Z one below, then went stale again within the same cycle, same recurring shape `BLOCKER-001` now names by pattern rather than by one date.** `ops/deploy-verdict.json`, read directly, now records `verdict: "current"`, build `28ed2709194afab5`, `checked_at: 2026-09-24T21:10:12Z`, matching commit `d5b0d5c8`. One further site-affecting commit, `869d4e93` ("Home Office personalised"), landed after that confirmation and moved `site/build-id.txt` to `6e4992daa2791557` at HEAD; `git diff --quiet d5b0d5c8 HEAD -- site/ Dockerfile` is dirty again (1 commit). This session's own `f62cc3ea` (an internal `ops/`-only bug fix) added nothing to that gap. Production is therefore confirmed stale by exactly one commit again, not the earlier 20/29-file gap. NO OPERATOR SANDBOX HOLDS THE DEPLOY KEY'S PRIVATE HALF OR EGRESS TO THE VPS (CONFIRMED AGAIN THIS CYCLE), SO THIS IS READ FROM THE COMMITTED VERDICT, NOT RE-CHECKED LIVE; `VPS_DEPLOY_KEY` (`OWNER-ACTIONS.md` ITEM 0, ISSUE #35) IS WHAT WOULD STOP THIS FROM BEING REDISCOVERED EVERY CYCLE.
