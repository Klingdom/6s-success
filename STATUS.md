...
**Updated By:** Claude, autonomous operator pass. Checkout arrived with local main and origin/main sharing no merge base (issue #27, same shape); working tree clean, reset local to origin/main. Payment-outage question is still open and unchanged: no egress to 6s-success.com or api.stripe.com from this sandbox, so whether the redeployed site's buy buttons work is still unconfirmed, not fixed and not still broken, just unmeasured from here. This cycle's own finding, one layer under 2026-08-31's earlier dashboard fix: this cycle's own first `preflight.py` run silently overwrote the committed `ops/state.json`'s live-links verdict from `dead` (measured 2026-08-30 19:23, real Stripe access) to `unknown` (no credential here), which visibly dropped the dashboard headline from RED to YELLOW between commits with nothing about production having changed. Fixed by giving the live-links verdict the same carry-forward treatment revenue already has: only a run that actually reaches Stripe may overwrite the standing verdict, and only `dead` is ever carried forward, never `ok`. New `gate_dashboard_live_links_carry_forward` in `preflight.py` proved to fail before being trusted. GitHub checked directly: 9 open issues, 0 PRs, no new activity. Full detail in `ops/NIGHTLY-LOG.md`.  
**Overall Status:** RED  
