...
**Repository HEAD Build:** `b7a36ca1` (95 commits ahead as of this
scheduled operator cycle, re-derived fresh rather than repeating the
93-commit figure one merge earlier, which had gone stale in this section
specifically because no gate re-checks it here, unlike the build_id
citations in `BLOCKER-001` and "Production Knowledge" below; includes `a16788fa`, a live
...
**Production Knowledge, re-derived 2026-09-24, PM check-in: the 95-commit figure and the 914c2881 build citation below were both stale, and a newer, better confirmation than either already existed on GitHub unread.** LAST CONFIRMED CURRENT 2026-09-23T19:00:39Z (build `5eba61fde231c1a7`, per `ops/deploy-verdict.json`, a session with real production access that finished the Stripe SKU retirement and redeployed). `site/build-id.txt` at HEAD now reads `4ec571da81db3b34`: 127 commits have landed since that confirmation (`git log 8e4c8e33..HEAD`, re-derived directly this cycle), but the build side of that gap is closed, not open: `publish-image.yml` run 398 (`workflow_dispatch`-free, a plain push build, `https://github.com/Klingdom/6s-success/actions/runs/36032909521`) completed `success` at 2026-09-24T17:35:17Z against commit `accc9fff`, checked directly via the GitHub Actions API this cycle, and `git diff --quiet accc9fff HEAD -- site/ Dockerfile` is clean, so the GHCR image already carries every fix named in BLOCKER-001, HEAD included. Only Phil's Hostinger Redeploy click (or a session holding the VPS key) remains, per `OWNER-ACTIONS.md` item 0/1b. NO OPERATOR SANDBOX HOLDS THE DEPLOY KEY'S PRIVATE HALF OR EGRESS TO THE VPS, SO NEITHER A FRESH CHECK NOR A REDEPLOY IS POSSIBLE FROM HERE; TREAT VPS ACCESS AS PER-SESSION, NOT UNIFORMLY AVAILABLE.
