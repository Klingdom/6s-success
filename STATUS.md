...
**Prior (2026-09-24, scheduled operator cycle): Cold-read of `ops/crawl_report.py` found and fixed a real "unknown reported as zero" defect.** `main()` computed an `unparsed` count whenever a raw access-log line failed to parse, the exact log-format-drift shape this file's own docstring records happening once already (1,162 lines dropped), but when every line failed the `if not rows:` branch printed the same message a genuine empty traffic window produces, and the `unparsed lines (log format changed?)` diagnostic built to catch this never ran in the total-failure case. Fixed: 100% unparsed now reports `UNCHECKED`/exit 2, matching the file's own established contract; a genuine empty window still reports its honest zero. New `ops/tests/test_crawl_report_unparsed_reporting.py` (3 cases) proved fail-then-pass. Also cold-read `build_kitchen_deck_pdf.py`, `build_cover.py` and `build_corporate_asset.py` (the standing PM-twin handoff plus the next tier): all three confirmed clean, no defect. Full `preflight.py` reran clean after the fix (every gate passed, 23 warnings, none new); `check_urls.py` (190/190), `audit_pages.py` (0 duplicates), `affiliate.py --check` (165 documents) all clean. No backlog row newly unblocked; 7 GitHub issues confirmed live via the API, unchanged (2 P0 `blocked-on-art`, 5 `decision`). Only Phil's Hostinger redeploy click remains open; full detail in `BLOCKER-001` below and `OWNER-ACTIONS.md` item 1b.
**Older entries (62 of them, 2026-08 to 2026-09-24) live in `STATUS-ARCHIVE.md`.** Most recently added 2026-09-26, this cycle, moving the oldest of the four stack entries there to keep this stack at four (the same rotation practice this file has followed since 2026-09-17): the oldest archived each time a new one is added rather than left to grow unread. Nothing is deleted, and the gates that scan this file for stale claims (`gate_no_stale_session_label`, `gate_no_stale_checkout_count`, `gate_no_stale_listmonk_blocker`, `gate_corporate_buy_path_current`, `gate_critical_risks_escalated`) scan the archive too, so an archived claim is no less checked than a current one.
