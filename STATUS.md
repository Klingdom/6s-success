...
**Last Updated:** 2026-09-18, scheduled operator cycle. **A latent "copy and control disagree" gap closed in `ops/build_kitchen_deck_page.py`, no live defect today.** The Kitchen deck's page (the free, unillustrated 72-card deck) stated its own card/zone/root-cause counts as plain hand-typed prose: "72 cards" (title, both social meta blocks, JSON-LD, hero, eyebrow, print button), "Seven kitchen zones"/"seven zones", "twelve root causes", and, naming the exact four cards, "Four cards that are not one zone's job: the nightly close, the safety walk..., the shopping list loop, and the conversation...". Cold-read per step 5d (3 mentions in `ops/NIGHTLY-LOG.md`, among the lowest of any `ops/*.py` file); checked directly against the real deck (`ops/cardtext/kitchen-deck.json`) rather than assumed: all four numbers and the four named cards (KA-015 through KA-018) matched exactly, so nothing is wrong on the live page today, but nothing would have caught a future card added, removed or reclassified leaving the prose stale, the same class this repository has already gated repeatedly (`kit.html`'s zone counts, `invest.html`'s catalog numbers). Fixed at the source: every count now derives from the real deck data (`ops/build_kitchen_deck_page.py`'s new `num_word()`), and the whole-kitchen sentence is guarded by an `assert` naming the exact four card IDs it describes. Because `gate_generator_ownership`'s own per-generator loop does not check each script's exit code, only the net file diff (so an `AssertionError` there would leave the page unwritten and unchanged from HEAD, reading as a clean tree rather than a caught defect), also added an independent `gate_kitchen_deck_page_counts_current` in `preflight.py` that re-derives every count straight from the committed JSON, the same independence `gate_kit_page_zone_counts_current` already keeps. `ops/tests/test_gate_kitchen_deck_page_counts.py` (5 cases) fail-then-pass proved, including 4 synthetic regressions (a stale zone count, a stale root-cause count, a changed whole-kitchen card set) and a live check against the real committed page; also proved directly against the real file (planted "99 cards"/"Nine kitchen zones" in the committed page, watched the gate fail by name, restored). Regenerating the page produced byte-identical output to HEAD (nothing was actually wrong today). Full `preflight.py` (every gate passed, 23 warnings, all previously diagnosed sandbox limits, none new), `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), `fix_dashes.py --check` (0/0) all clean. 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 open PRs. No mail credential, inbox UNCHECKED not empty. No price or product touched, no site page content actually changed (the generator hardened, the shipped HTML is byte-identical). IndexNow not applicable. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-18, scheduled operator cycle):** **A real, live checklist defect in `ops/accept_image.py`'s image accept test: six real zones' own wording produced a `contradicts` checklist item naming no object at all, only a pronoun or a bare comparative, which the accept test scores as a HARD FAIL when unanswered.** `_negative_clauses()` run against all 228 real `done_looks_like`/`leave_behind.standard` strings in `content.json` (not synthetic cases) turned up "a seat with nothing on it at all" (primary-bedroom) producing the checklist item `on it at all`, asked of a vision model as "Is on it at all visible in this image?"; five more zones carried the same shape ("nothing beside it", "smelling of nothing at all", a console holding "nothing that was there yesterday", "no higher"). One step past the sibling bug `gate_image_prompt_negations_handled` already caught the same day in `ops/image_local.py` (there a pronoun trails a real noun and gets stripped; here the pronoun or comparative IS the entire remainder). Fixed with `_FILLER_EDGE`/`_NO_OBJECT_LEFT`, which also recovers real content filler was obscuring ("nothing at all sitting on either lid" now keeps "sitting on either lid" instead of losing it). 6 new cases in `ops/tests/test_accept_image.py` (13 total) and a new `gate_accept_image_contradicts_name_an_object` in `preflight.py`, both fail-then-pass proved directly against the real committed file. `accept_image.check_all()` (89 card + 114 zone checklists, 0 errors) and `self_test()` (4/4) both clean after. Full `preflight.py` (every gate passed, 23 warnings, all previously diagnosed sandbox limits, none new), `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents) all clean. 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`. No mail credential, inbox UNCHECKED not empty. No price or product touched, no site page changed (internal image-acceptance tooling, not a rendered page). IndexNow not applicable. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-18, scheduled operator cycle):** **Closed a real `gate_status_currency` warning (8 material commits unmentioned since this file's prior edit `beff4ea9`) by describing what actually shipped, per `CLAUDE.md` 0.2, rather than opening a fresh sweep.** Attached via unshallow plus ff-only merge onto `origin/main` (`f956745c`), clean. Continued the prior PM check-in's own named handoff, the next ungated low-mention `ops/*.py` cold-read tier: `mailer.py` and `wire_breadcrumbs.py` read clean. `ops/stripe_setup.py`'s "Not created, deliberately" block unconditionally printed "Book and manual        blocked on front matter, issue #3" on every `--plan`/`--apply` run, months after issue #3 closed (2026-08-25, checked directly) and after both products started selling live through the catalogue-driven `ops/stripe_catalog.py`. Fixed the line and the file's own stale "41 items" catalogue-size docstring; new `gate_no_stale_stripe_setup_book_blocker` in `preflight.py`, fail-then-pass proved. **Landed alongside, and merged rather than dropped either: a concurrent PM check-in found a more significant gap in the same file**, that its own two consulting SKUs (lookup_key-based) were themselves superseded 2026-08-27 by SKU-tagged prices `stripe_catalog.py` now manages, so a live `--apply` would orphan a duplicate product/price rather than update the real one; that fix's SUPERSEDED docstring warning is kept in full. `preflight.py` clean after the merge (every gate passed, 24 warnings, the one new one being this file's own now-current staleness reading), `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), `fix_dashes.py --check` (0/0) all clean. 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`. No mail credential, inbox UNCHECKED not empty. No price or product touched, no site page changed. IndexNow not applicable. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-18, PM check-in):** **Closed a real `gate_status_currency` warning (35 material commits unmentioned since this file's prior edit `de7ee71d`) by describing what actually shipped rather than opening a fresh sweep, per `CLAUDE.md` 0.2.** Attached via unshallow plus ff-only merge onto `origin/main` (`4418ee0a`), clean. `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, none new. 8 GitHub issues confirmed live via the API (up from 7: `#32` and `#33` opened since, both `decision`), 2 P0, 2 `blocked-on-art`, 6 `decision`, 0 open PRs. The two threads that account for nearly all 35 commits, both shipped and independently verified:
...
2. **A "copy and control disagree on price" class, fixed once in `build_zone_pages.py`, was still live in three siblings.** Cold-reading `build_printpack.py` led outward to `build_resources.py`, `build_standards_page.py` and `build_zone_index.py`, each reading PACK-HOUSE's buy link live from `data.js` but hardcoding the price beside it as a plain `"$19"` string; no live mismatch today, but a future reprice would have gone silently unrepaired, since `gate_generator_ownership` cannot see a hardcoded literal. Fixed at the source (`8af64f4b`), new `gate_price_matches_its_own_link` added and fail-then-pass proved. Same session separately caught and fixed `build/6S-Success-Home-Edition.epub` predating its own fresh build and `site/shop.html`'s pre-rendered grid still serving the old `room-map.jpg` placeholder to JS-less crawlers after `data.js` moved to the real zone photograph (`913818ed`), plus taught the EPUB gate that a rebuild being byte-different across Python 3.12/3.14 zlib implementations is not the same as content-different (`fe213300`).

Also in that span, smaller and already-verified: `fbe8ba12` stopped `gate_generator_ownership` failing on `build_epub.py` in a text-only checkout; `38254842`/`2752ef62`/`8d4484ab`/`ac352ac4`/`5380faa5` are the shop re-render, product-image regeneration, room-page/fingerprint and card/zone-count steps of the same picture batch described above; `c674ab0b` stopped `kit.html`'s zone counts trusting a frozen catalogue column (recomputed fresh, `gate_kit_page_zone_counts_current` added); `6580faa4` dropped card decks from `how-we-make-money.html`'s revenue list (the only deck SKU is free, nobody pays for it).

Full detail in `ops/NIGHTLY-LOG.md`'s newest entries. No mail credential, inbox UNCHECKED not empty. No code, content or price touched this cycle; command deck regenerated.
