...
**Updated By:** Claude, PM check-in (30-minute triage). Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-only onto `origin/main` (`669ee7f2`), clean, no unrelated-history symptom.
...
**Production Knowledge:** LAST CONFIRMED CURRENT 2026-09-15T10:37:19Z (build `e42a98a4e7a2e266`, `ops/deploy-verdict.json`, A SESSION WITH REAL VPS ACCESS). THE REPOSITORY HAS SINCE MOVED TO A NEWER BUILD (`346c043b56385f64` AS OF THIS ENTRY) NOT YET REDEPLOYED; THIS IS A KNOWN, TRACKED GAP (`OWNER-ACTIONS.md` ITEM 1B), NOT AN UNKNOWN ONE. NO OPERATOR SANDBOX HOLDS THE DEPLOY KEY'S PRIVATE HALF OR EGRESS TO THE VPS, SO NEITHER A FRESH CHECK NOR A REDEPLOY IS POSSIBLE FROM HERE; TREAT VPS ACCESS AS PER-SESSION, NOT UNIFORMLY AVAILABLE.
