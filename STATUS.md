...
**Updated By:** Claude, autonomous operator pass. Unshallowed and fast-forwarded 53 commits onto `origin/main` cleanly, no issue #27 symptom. Every unblocked row in both backlogs was already done or Phil-gated, and the prior cycle's GitHub Actions incident had resolved (Checks and Publish site image both green at HEAD), so this pass checked whether a real distribution channel outside the site itself had drifted.

**This pass's real finding: the live MCP server had been answering real queries with stale content since 2026-08-31.** `mcp/content.json` is a committed copy of `content/manual/source/content.json`, bundled into the deployed MCP server's Docker image (watchtower-updated on push) so the image stays self contained. `publish-mcp.yml` only triggers on a push touching `mcp/**`, and its own drift check runs only inside that same job, so this week's real content work (the Sustain rewrite, median 28 to 94 words; new `diagnosis` root-cause blocks on 12 pilot zones) never touched `mcp/` and never re-triggered it. A structural diff, not the stale trigger alone, confirmed it: all 114 of 114 zones differed, each missing `diagnosis` entirely and several still on the old shallow Sustain text the site itself had already superseded. Re-copied the manual into `mcp/content.json` (`server.py` never reads `diagnosis`, confirmed inert there; every field it does read present for all 114 zones). Widened the workflow trigger to also fire on the manual's own path, and added `gate_mcp_corpus_current` to `preflight.py` so the invariant is checked locally on every cycle regardless of what triggered CI, since a workflow trigger is invisible to a local run. `ops/tests/test_gate_mcp_corpus_current.py` (4 cases), fail-then-pass proved. Pushed (`5a45a312c`); `publish-mcp.yml` confirmed to fire on this exact push, rebuild not yet confirmed complete from this session.

Preflight fast clean (0 gates failed, 15 warnings, all sandbox-environment limitations, one fewer than the prior cycle now that GitHub Actions itself has recovered). Page count 191. GitHub: 8 open issues unchanged (all art-blocked or decision-labelled), 0 open PRs. All 50 test files, check_urls (188/188), audit_pages (0 findings), affiliate.py (162 documents), mobile npm test (4 suites), generator-ownership `--own` (all checkable generators), all clean.  
