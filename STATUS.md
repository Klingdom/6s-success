...
**Last Updated:** 2026-09-26, scheduled operator cycle. **Step 2's own gate failed on first run, its own fix IS this cycle's work: the prior PM check-in (`ba73ec3c`) regenerated all 20 room pages to fix a false no-affiliate-link disclosure but its own `preflight.py` was still running in the background at close and never confirmed clean.** It was not: `ops/build_seo.py` had not rerun after the content change, so `site/sitemap.xml`/`ops/sitemap-content-hashes.json` were stale for 20 URLs, failing `gate_sitemap_lastmod_current` and, as a knock-on, `test_gate_sitemap_lastmod_current.py` itself (its own "restore, then confirm clean" assertion tripped on the real pre-existing staleness, not a defect in the test). Fixed by running `ops/build_seo.py` and restamping `site/build-id.txt` (`89a030a5`, `0f1641cb`); full `preflight.py` clean after (every gate passed). Two concurrent PM check-in cycles (`ac1af6e7`, `91aadd2c`, both folded into `Prior` immediately below) independently found and fixed the identical sitemap staleness plus an unrelated `gate_no_stray_probe_files` regression, and confirmed production is 3 material commits behind repository HEAD, including this same `ba73ec3c` disclosure fix: the repository has the honest text, production does not, until the next redeploy (not actionable from this sandbox, no VPS key). **Second finding, this cycle's own cold-read pass (`ops/cold_read_ledger.py --next`), a real gate gap rather than a live defect: `gate_stripe_one_product_per_sku` only ever checked duplicate Stripe PRODUCTS, never duplicate active payment LINKS, even though `stripe_dedupe.py`'s own module docstring names the link case as a real incident from 2026-09-23 (five SKUs, each with two active links) and the fix (`dedupe_links()`) already existed with no gate wired to it.** Added `stripe_dedupe.duplicate_active_links()` and a new `gate_stripe_link_dedup()` in `preflight.py`, mirroring the product gate's own warn-not-fail convention exactly (no credential reports UNCHECKED, never clean); fail-then-pass proved directly (stashed `preflight.py`, watched the new test fail with `AttributeError`, restored, reran clean); new `ops/tests/test_gate_stripe_link_dedup.py` (3 cases), existing `test_stripe_dedupe.py`/`test_stripe_dedupe_links.py` both still pass after the refactor (`22cec4f2`). 9 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, none pickable (issue #36, a dead-check finding in the payment-integrity file `ops/check_sellable.py`, was correctly left for Phil rather than re-attempted: the prior session's own edit there was already blocked by the harness's safety classifier and reverted, and this file is squarely CLAUDE.md section 52's YELLOW/RED checkout-adjacent tier). No mail credential; no backlog row newly unblocked (`BACKLOG-2026-09-07.md` sections 2-4 fully Done or CLOSED by decision, section 5 HOLD, section 6 owner-gated), so this cycle's own work is entirely epic 2 (broken or dishonest): finishing the prior cycle's unconfirmed fix, then closing a real, if quiet, gate gap in the same family.

**Prior (2026-09-26, PM check-in): Previous work confirmed finished via CI's own Preflight step and a clean full local `preflight.py` run; the cold-read lane continued, two more `ops/*.py` files cleared, and a real customer-facing honesty fix from earlier this same slot is confirmed genuine but still undeployed.** Attached cleanly (`git fetch`/`unshallow`/`checkout main`/`merge --ff-only`, 385 commits fast-forwarded onto `ac1af6e7`, no conflict). Rather than trust the prior PM cycle's own unfinished background `preflight.py`, checked CI directly: `checks.yml` run #1464 on `ac1af6e7` shows the **Preflight step completed successfully** (18 minutes, the normal range), confirming both of that cycle's gate fixes (a `__pycache__` false positive in `gate_no_stray_probe_files`, a stale post-fix sitemap) for real; this cycle's own full local `preflight.py` then finished clean too (every gate passed, 25 warnings, all previously diagnosed sandbox limits, none new). 9 open GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, none pickable; `BACKLOG-2026-09-07.md` sections 2 to 4 remain Done/CLOSED, section 5 HOLD, section 6 six owner gates, so the cold-read lane (the standing handoff across the last three cycles) was the right-sized work. Cold-read and cleared `ops/accept_image.py` (self-test 4/4, `--check` 89 cards + 114 zones/0 errors, both its gates already wired and passing) and `ops/stripe_fulfil.py` (its `DELIVERY` table cross-checked against `build_catalog.py`'s real product `kind` values and `stripe_catalog.py`'s own `SELLABLE` deliverable paths, all match; a dry run correctly refuses without a Stripe credential rather than guessing). Both recorded `clean` in `ops/cold-read-ledger.json`. **Also found, re-deriving `BLOCKER-001`'s deploy gap rather than assuming it closed:** production is 3 material commits behind repository HEAD (`ac1af6e7`, `ba73ec3c`, `223f5111`), and one of them, `ba73ec3c`, is the same-day fix for `render_room()` falsely claiming "not one product below carries a paying link" directly above real outbound retailer links on all 20 room pages with a kit; that false disclosure is still what production serves until the next redeploy. No sandboxed session here holds `VPS_DEPLOY_KEY`/`~/.ssh/6s_deploy`, the same standing structural limit; full account in `BLOCKER-001` below. No backlog row newly unblocked; this sits in epic 2 (broken or dishonest) and epic 6 (operational honesty), the ordering rule's own top classes after measurement.

**Prior (2026-09-25, scheduled operator cycle): `checks.yml` was genuinely, currently failing on main, every run since commit `41cce0e5`, and the cause was CI's own shallow checkout, not the code it tests.** Found by checking GitHub Actions directly rather than trusting this sandbox's own preflight (which runs against a full local clone and cannot reproduce a shallow one): `ops/affiliate_report.py`'s `inputs_date()`, itself a same-day fix for a different CI outage, stamps its reports with the newest commit date touching `ops/affiliate-accounts.json`/`ops/affiliate-catalogue.csv` via `git log -1 -- <path>`. Those two files last changed 2026-09-09, 2,539 commits back; `checks.yml`'s `actions/checkout@v4` had no `fetch-depth`, so CI runs on a depth-1 shallow clone where that commit is invisible, `git log` returns nothing, and the stamp silently falls back to the checked-out file's mtime, always "now". Every push regenerated a report that differed from the committed one, failing `gate_generator_ownership` and `test_affiliate_report_stamp_stable.py` on every single run: the identical "a date that moves when nothing moved" bug the function's own docstring says it exists to prevent, reappearing through checkout depth instead of the clock. `publish-image.yml` runs the identical `preflight.py --own` through the identical gate and was equally exposed, just triggered less often by its own `site/**`/`Dockerfile` path filter. Fixed by adding `fetch-depth: 0` to both workflows' checkout steps (a bounded depth, the 50 `hourly-brief.yml`/`roadmap-report.yml`/`status-email.yml` already use, would not be enough here, since these two inputs can go thousands of commits between real edits). New `gate_ci_checkout_full_history` in `preflight.py` requires both workflows that run `preflight.py` to declare full history, so this class cannot silently return; `ops/tests/test_gate_ci_checkout_full_history.py` (7 cases) proved fail-then-pass, including directly against the real pre-fix committed workflow files via `git stash`. Pushed as commit `1da2f69e`; the live Actions run this fix should turn green (run #1401 or later) was still in progress when this cycle ended and could not be watched to completion from this sandbox. 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`; 9 more `ops/*.py` files cold-read clean, no defect in any. No backlog row newly unblocked; this sits squarely in epic 2 (broken or dishonest), the highest-priority class this operator's own ordering recognises after measurement, and a currently-red main branch undermines confidence in every gate this repository relies on to catch the next real defect.

**Prior (2026-09-24, scheduled operator cycle): Built `.github/workflows/deploy.yml`, the automated-deploy workflow this repository has been missing since `ops/deploy.py` was written 2026-08-31.** `CHECKIN-LOG.md`'s last several straight hourly check-ins each independently concluded "production is behind the repository, deploy" and stopped there, because no sandboxed operator session has ever held `~/.ssh/6s_deploy` or egress to the VPS, and (until this cycle) nothing under `.github/workflows/` called `ops/deploy.py` either, so the gap only ever closed when a local session happened to run one by hand, in bursts, with days of drift between them. The new workflow triggers itself the moment `publish-image.yml` finishes publishing, installs a `VPS_DEPLOY_KEY` GitHub Actions secret as the deploy key, and runs `python ops/deploy.py` exactly as a local session does, including its own refusal to report success until production's `build-id.txt` matches the repository's. Verified rather than assumed to be inert today: read its own first step, confirmed it checks for the secret and exits 0 untouched when absent; `preflight.py` (which already reads every file under `.github/workflows/`) ran clean with the new file in place, no gate objected. `ARCHITECTURE.md`'s workflow count and named inventory corrected to 11 (`deploy.yml` added); `DEPLOYMENT.md`'s pipeline diagram and its own "no automated pipeline" correction updated to match; `OWNER-ACTIONS.md` "start here" gained item 0, ranked first: paste the existing key's contents into a GitHub secret named `VPS_DEPLOY_KEY`, one time, and every future push reaches a customer unattended. This does not deploy anything today by itself; it removes the only remaining reason a deploy needs a human once Phil makes that one paste. `preflight.py` reran clean after every edit (every gate passed, 23 warnings, none new, same standing sandbox set). 7 GitHub issues confirmed live via the API, unchanged (2 P0 `blocked-on-art`, 5 `decision`), none of them this. No backlog row newly unblocked by this; it is new infrastructure work, not a queued item, chosen because it is the single most repeated unresolved line across dozens of `NIGHTLY-LOG.md`/`CHECKIN-LOG.md` entries and sits squarely in epic 2 (broken or dishonest): undeployed fixes include a live broken mobile hamburger menu on 5 pages, per section 2 below.

**Older entries (63 of them, 2026-08 to 2026-09-26) live in `STATUS-ARCHIVE.md`.** Most recently added 2026-09-26, this cycle, moving the two oldest of what would otherwise have been a six-entry stack there (two new top entries landed from concurrent cycles in one push) to keep this stack at four (the same rotation practice this file has followed since 2026-09-17): the oldest archived each time a new one is added rather than left to grow unread. Nothing is deleted, and the gates that scan this file for stale claims (`gate_no_stale_session_label`, `gate_no_stale_checkout_count`, `gate_no_stale_listmonk_blocker`, `gate_corporate_buy_path_current`, `gate_critical_risks_escalated`) scan the archive too, so an archived claim is no less checked than a current one.
