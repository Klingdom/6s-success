...
**Updated By:** Claude, autonomous operator pass. Checkout arrived shallow again (issue #27); `git fetch --unshallow` then `git merge --ff-only` resolved it cleanly with no history discarded. `python ops/preflight.py` failed on arrival with 2 gates red, both real: `check_sellable` (149 SKUs "name a file that is not built," the gitignored `build/products/` catalogue, never generated before the check runs) and `affiliate` (`could not read` the free sample PDF, because `ops/affiliate.py`'s PDF reader needs `pymupdf`, which is not installed anywhere this environment's CI runs). Per Step 2, treated the fix as this run's work. Cross-checked against the live GitHub Actions history rather than assuming: `publish-image.yml` run #128, the most recent one, is genuinely FAILED on `main` right now with these exact two errors, meaning every deploy touching `site/**` since the affiliate gate was wired into CI (`bdccb815`, `689aaa7b`, both Phil's own direct commits, ~21:58-22:07 UTC today) has been blocked. Added `ops/requirements.txt` (pymupdf, the only third-party import either CI-run script needs) and wired both `publish-image.yml` and `fulfil-orders.yml` to `pip install -r` it before the steps that need it; added a "build the gitignored catalogue" step to `publish-image.yml` (`ops/build_catalog.py --build`) so the sellable gate has something to check, matching what `fulfil-orders.yml` already did. `preflight.py` now passes clean (1 evergreen warning) after both fixes. Also verified, not just noted: a separate affiliate-programme workstream (link layer, compliance gate, four launch-blocker fixes, ten-programme research dossier emailed to Phil) landed today entirely via Phil's own direct commits, none of it this operator's; nothing further is operator-actionable there, since every remaining step (opening an account) requires Phil's own legal/tax identity per `CLAUDE.md` Step 13. Same 10 open GitHub issues as the prior cycle, no new comments, no mail credentials, no egress to 6s-success.com, api.stripe.com, api.indexnow.org, cloud.umami.is or api.umami.is. **Confirmed, not assumed:** `publish-image.yml` run #129, on the fix commit `dfb59587` itself, completed `success` at 22:52 UTC, the first green run since the affiliate gate broke deploys. CI is deploying again. Full history in `ops/NIGHTLY-LOG.md`.  
**Overall Status:** RED  
