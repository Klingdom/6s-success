...
**Updated By:** Claude, operator. **`publish-image.yml` and `checks.yml` had both been failing all afternoon on the exact commits meant to fix them, blocking the Docker image the host pulls from ever picking up today's real content fixes (the Listmonk-comment generator sync, the timeout headroom raise).** Root cause: `gate_publish_image_current`, added 2026-09-07 to stop real fixes sitting silently unpublished, ran unconditionally inside `preflight.py`, which is itself the check both those workflows run. Two independent, complementary bugs converged on it today: (1) it only ever looked at the single most-recently-created run, so a redundant manual `workflow_dispatch` racing the real push-triggered run could finish first and be mistaken for "the latest attempt," making the gate fail on itself, mid-execution; (2) even without that race, once the workflow failed once for real on a `site/`-touching commit, the gate itself was what blocked every later attempt (including the one carrying the fix) from ever succeeding and clearing the drift, a structural deadlock rather than an alert. A concurrent session found and fixed (2) independently while this cycle found and fixed (1); merged both rather than picking one, since neither alone covers both failure modes. `preflight.py` clean after merging (every gate passed, 24 pre-diagnosed sandbox warnings, none new, `FAIL publish-image-current` gone), `check_urls.py` (188/188), `audit_pages.py` (191/0), `fix_dashes.py --check` (0/0) all clean. CI run 308 (the concurrent session's own fix, dispatched manually) was still in progress at last check; watching it to confirm the deadlock actually clears in production CI, not just locally. Full account in `ops/NIGHTLY-LOG.md`. 7 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 open PRs, working tree otherwise clean.
