...
**Last Updated:** 2026-09-04 (third cycle, corrected mid-cycle by Phil's own concurrent work)  
**Updated By:** Claude, autonomous operator pass. `preflight.py` FAILED cold on `gate_dashes` (Phil's new `REVENUE-REVIEW-2026-09-04.md`, 7 em dashes); fixed, plus two false positives in `gate_copy_vs_control` found while investigating it, proved both ways with a planted regression. Read the revenue review and, rather than leave it standalone, checked its "Corporate Lean 6S has no price and no buy path" claim against the live site and filed the two real decisions inside it as `OWNER-ACTIONS.md` item 14 and GitHub issue #30. Before that push landed, Phil pushed `9e7b1cd1` directly, answering both: a real `site/corporate.html` (deliberately no invented price) and 20 of 20 room pages now routing to a consult, overriding the roadmap's G2 gate by his own hand. Rebased, corrected `OWNER-ACTIONS.md`/`BACKLOG-2026-H2.md` to credit him, closed issue #30. Rebasing then surfaced a real P0: `preflight.py --own` (the CI flag, adds `gate_generator_ownership`) failed because `build/6S-Whole-House-Print-Pack.html`, the exact file `ops/stripe_fulfil.py` delivers to PACK-HOUSE buyers, still carried the pre-fix broken 152-page geometry despite Phil's commit message claiming it was rebuilt. Regenerated; a concurrent follow-up push from Phil (`f2885908`) fixed it independently too, converged before this push landed. Three commits pushed after two real rebases with conflicts, `git fetch` before each push, nothing overwritten. GitHub: 9 open issues, 0 PRs (issue #30 opened and closed same cycle). No mail credential. Full detail in `ops/NIGHTLY-LOG.md`.  
