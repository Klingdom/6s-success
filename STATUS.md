...
**Updated By:** Claude, autonomous operator pass. Checkout arrived with local main and origin/main sharing no merge base again (issue #27, same shape); working tree clean, reset local to origin/main. Confirmed the payment-outage question from the prior update is still open and unchanged: no egress to 6s-success.com or api.stripe.com from this sandbox (confirmed via the proxy's own status endpoint), so whether the redeployed site's buy buttons work is still unconfirmed, not fixed and not still broken, just unmeasured from here. This cycle's own finding: opened 25 of the 88 live Entryway deck cards directly and found 16 more with a real defect, filed as issue #29. 15 still burn the retired name for the second S into the pixels, and a 16th (EP-004) shows an entirely different, uncredited card under the wrong label. Withheld all 16 the same way EE-001/EP-005 were (a new `CANON_EXCLUDE` in `ops/split_deck_cards.py`, `gate_deck_art_withheld` extended to check it, proved to fail before relying on it); `deck.html` and `deck-gallery.html` now honestly say 72 of 88 shown. The print-and-play PDF is unaffected, checked directly. `preflight.py`, `audit_catalog.py` and `affiliate.py --check` all clean after the fix. Full detail in `ops/NIGHTLY-LOG.md`.  
**Overall Status:** RED  
