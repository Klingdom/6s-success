...
**Production Knowledge, REOPENED 2026-09-25, scheduled operator cycle: this paragraph and BLOCKER-001's own Status line were both citing the 05:4x confirmation below, superseded hours ago.** `ops/deploy-verdict.json`, read directly, now records `verdict: "current"`, build `ea2e48125aa63502`, `checked_at: 2026-09-25T07:30:48Z`, resolving via `resolve_verdict_commit()`/`git log -S` to commit `c6cc1a6a` (B9's Entryway deck). 5 material commits have landed since, touching 8 `site/`/`Dockerfile` files (`8f6c47b3` build-id fix, `9c6d4063` five-deck og:image fix, `e6017e8b`/`f3ceca2e`/`2cb970c1` the Garage/Primary Bathroom/Home Office decks); full detail in `BLOCKER-001` above. Same standing limit as every prior entry: no operator sandbox holds the deploy key, so this reopens again the moment the next `site/**` commit lands with no session to follow it; `VPS_DEPLOY_KEY` (`OWNER-ACTIONS.md` item 0, issue #35) is the only fix for the recurrence itself.

**Prior (2026-09-25 05:4x, PM check-in): RESOLVED, the gap named below closed again.** `ops/deploy-verdict.json`, read directly, now records `verdict: "current"`, build `aa7c7e7e578e9a18`, `checked_at: 2026-09-25T04:50:46Z`, resolving to commit `890c43a3`. `git log 890c43a3..HEAD -- site/ Dockerfile` is empty: production matches HEAD exactly right now, including the footer restoration `BLOCKER-001` names. Same standing limit as every prior entry: no operator sandbox holds the deploy key, so this closes again the moment the next `site/**` commit lands with no session to follow it; `VPS_DEPLOY_KEY` (`OWNER-ACTIONS.md` item 0, issue #35) is the only fix for the recurrence itself.
