...
**Prior (2026-09-17, local session): The `publish-image.yml`/`checks.yml` deadlock is resolved and confirmed live, not just locally.** Three distinct, converging bugs in `gate_publish_image_current` (added 2026-09-07, ran unconditionally inside `preflight.py`, which both workflows run) each blocked the fix meant to resolve the one before it: (1) the gate only looked at the single most-recently-created run, so a duplicate `workflow_dispatch` racing the real attempt could be mistaken for "the latest," failing the gate on itself mid-execution; (2) even without that race, the gate itself blocked the one workflow whose success would have cleared it, a structural deadlock; (3) the fix for (2) added an env-based branch whose own tests never isolated `GITHUB_WORKFLOW`, so they silently inherited CI's real ambient value and broke the moment they ran for real inside that job. All three fixed (two independently and convergently by a concurrent session, reconciled rather than duplicated), and CI run 309 (workflow_dispatch on `0cbca788`) confirmed `completed success` end to end, including the actual "Build and push" step, at 14:39:03Z. Local `preflight.py` afterward: every gate passed, 23 warnings, `gate_publish_image_current` no longer even fires, meaning `goods` has genuinely advanced past `d4ac5147` and today's real content fixes (the Listmonk-comment generator sync, the timeout raise, the duplicate-Stripe-name gate) are now in the image the host pulls. Production still needs Phil's own Hostinger redeploy click to actually serve it, an unchanged standing item in `OWNER-ACTIONS.md`. Full account in `ops/NIGHTLY-LOG.md`. 7 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 open PRs, working tree otherwise clean.

**Prior (2026-09-17, scheduled operator cycle):** `gate_status_currency` warned this file had gone 8 material commits stale since its last edit (`2ce28941`); closed that gap, per `CLAUDE.md` 0.2, rather than opening a fresh sweep. The eight: **the invest.html trust defect fixed and gated** (a fundraising page carried unsourced superlatives and described an unbuilt phone app/kit as buyable today; full account in `ops/NIGHTLY-LOG.md`); two stale live instructions found sweeping `BACKLOG-2026-H2.md` end to end and gated (a two-week-old published-video count, and `OWNER-ACTIONS.md`/`BACKLOG-2026-H2.md` still telling Phil to finish affiliate applications a later, settled plan already held); a live protection gap closed on `shop.html`'s prerendered product grid (could have gone stale against the catalogue unnoticed); a latent false-plural bug fixed in the retailer link verifier (no live link was actually wrong); a staleness gap closed on the phone app's store icons; and the Kitchen deck's 21 missing micro quests authored plus its related-card field printed on all 18 action cards. `preflight.py` clean (every gate passed, 23 pre-diagnosed sandbox warnings, none new beyond the transient staleness this update itself closes), 7 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 open PRs, working tree otherwise clean.

**What happened in the 9 unmentioned commits (2026-09-16, 10:20 to 18:54), most recent first:**

- **`9b8a41ea` (18:54):** a live overclaim fixed on `site/deck/entryway-print-and-play.html` (the retired-mockup redirect notice): it told visitors "88 cards, illustrated front and back" when `ops/card-hero-verdicts.json` shows 9 of 88 currently render text-only (no photograph). Rewritten to state the real split; new `gate_print_and_play_art_count_current` in `preflight.py` re-derives the count from the verdict file so this cannot drift silently again. **Deployed and verified live at `2026-09-16T13:20`, Phil's own local session (`80a2d801`): production moved `75f9963d239b9112` to `0bd535b9df92bdd5` and now matches the repository exactly, old wording confirmed gone on the live page.** `153b89f1` below (12:31) was already inside the prior confirmed build.
- **`f5e3206e` (18:50):** `OWNER-ACTIONS.md`'s header said "Last measured: 2026-09-15" while its own body (item 1f, the VPS disk finding) was dated 2026-09-16; `gate_owner-actions-last-measured-current` FAILed preflight over it. Fixed the header.
- **`b46aee74` (12:33):** logged the first direct production inspection and escalated the VPS disk finding (79% full, 46GB reclaimable, two Ledgerium containers crash-looping, not ours to fix per `CLAUDE.md` 36b) into `OWNER-ACTIONS.md` item 1f.
- **`153b89f1` (12:31):** `standards.html` gained a five-question FAQ block, answered in the page's own sentences. Included in the `15:06:13Z` confirmed-current production build.
- **`9bdc1303`/`c65d15be` (16:53/17:01):** preflight arrived red from a concurrent session's duplicate app-icons gate registration; reconciled, plus fixed three stale on-device-check-count docs and a test false positive on the new YouTube deep-link.
- **`4a5645ad`, `d142cad0`:** `ops/build_app_icons.py` (the phone app's store icons) written and registered with `gate_generator_ownership`'s protection.
- **`00d66af9`, `0589c508`, `4ed67396`, `6d270a24`, `c3005d49`, `e191bef0`:** phone-app store-submission prep: store listing text and a price decision, `gate_store_art`'s path-prefix bug fixed, the six required screenshots mapped to real on-device checks, the app's own zone video, and two store gates proven by sabotage (planted the real defect shape, watched them fail by name, confirmed the fix passes). All still gated on Apple/Play developer accounts only Phil can create (`BACKLOG-2026-09-07.md` section 6, item 5).

None of the above needed a fresh fix beyond what's listed; all were already shipped and gated by the commits themselves. This entry exists because the account of them was missing, not because anything was broken.

**Corrected this file's own stale claim rather than repeating it a further time: production is confirmed current, not behind.** Several of this file's own recent entries below still say "production remains on an older build than the repository." That was true when written; it stopped being true at `2026-09-16T15:06:13Z`, when a session with real VPS access ran `ops/deploy.py`, moved production to build `628eb4520b6e9ded`, and committed the confirmation to `ops/deploy-verdict.json` in `574adfa9` (already logged at the top of `ops/NIGHTLY-LOG.md` as "Log the Home Quest video and finish-picture release; commit its deploy verdict"). Verified directly rather than assumed: `ops/deploy-verdict.json`'s `build_id` (`628eb4520b6e9ded`) matches `site/build-id.txt` at HEAD exactly. The command deck (`EXECUTIVE-DASHBOARD-LIVE.md`, `ops/dashboard.html`, `ops/state.json`) had been regenerated locally with this state but never committed; shipped it this cycle. The dashboard's own constraint line has moved accordingly, from "production is serving an old build" to discovery/traffic: 158 of 159 catalogue items are buyable and live, and the gap is visitors, not what can be bought. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; section 6's owner gates (YouTube OAuth, Search Console, Gemini billing, KDP/Etsy accounts) remain the only real blockers. No mail credential, no egress from this sandbox, both unchecked not empty.

**Prior (2026-09-16, scheduled operator cycle):** Checkout arrived shallow and detached; step 0 (`git fetch --unshallow`, `merge --ff-only`) attached cleanly onto `origin/main` (`66e5535a`, 166 commits ahead), then fast-forwarded again mid-cycle onto a concurrent PM check-in's push, no conflict either time. `preflight.py` clean on arrival (every gate passed, 24 pre-diagnosed sandbox warnings, none new). 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; section 6's owner gates (YouTube OAuth, Search Console, Gemini billing, KDP/Etsy accounts) remain the only real blockers. No mail credential, no egress from this sandbox, both unchecked not empty.

**Closed the standing doc-sprawl backlog-consolidation handoff: its own premise was wrong, checked directly rather than trusted.** Three consecutive PM check-ins today recommended collapsing `BACKLOG.md`/`BACKLOG-2026-H2.md` to short historical pointers, each citing "the same posture already used for `ROADMAP.md`/`STRATEGY.md`/`GROWTH-PLAN.md`." Checked the cited precedent before doing anything to a 2,545-line file kept specifically for its acceptance-criteria detail: `ROADMAP.md` (2,039 lines) and `STRATEGY.md` (1,556 lines) both still carry their full original body under a banner. Neither was ever collapsed; nothing here has ever been collapsed to a stub. The real, actual precedent is "banner plus full body kept," and `BACKLOG.md`/`BACKLOG-2026-H2.md` already match it exactly, including the forward-pointing banner fix from 2026-09-08 (the one real defect in this chain, already closed). There is no outstanding structural defect to fix here, only an inherited misdescription of what "the same posture" means, repeated forward through three handoffs without either being re-checked. Also independently re-verified the most recent check-in's own gate-migration list (11 `preflight.py` gates it mapped to `BACKLOG-2026-H2.md` by nearest-preceding-`def`, explicitly flagged there as unverified): read all 11 function bodies directly rather than trust the grep-based mapping. Only 4 actually check live content inside the file (`gate_no_stale_session_label`, `gate_no_stale_card_deck_decision`, `gate_critical_risks_escalated`, `gate_image_prompts_tier0_count_honest`); the other 7 (`gate_corporate_buy_path_current`, `gate_etsy_pdfs_current`, `gate_experiment_owner_actions_surfaced`, `gate_goals_traffic_current`, `gate_indexable_pages_have_schema`, `gate_mobile_badge_contrast`, `gate_roadmap_photo_asset_caveat`) only mention the filename in a docstring's historical narration, nothing live. All 4 real ones check for a *stale/wrong* pattern reappearing, not for specific content to keep existing, so none would need migrating even if a collapse were the right call; `gate_critical_risks_escalated` was additionally already safe regardless, since `STATUS.md` independently names all three of today's CRITICAL/OPEN risk IDs on its own. Widened it anyway to also check `BACKLOG-2026-09-07.md` (the file that has actually carried live ordering since 2026-09-08), since the working-list check should cover the live queue, not only its superseded predecessor. New `gate_doc_supersession_chain_current` in `ops/preflight.py` protects the thing that actually was broken once (2026-09-08's backwards-only chain) from regressing in either this chain or the ROADMAP one, fail-then-pass proved against isolated copies of the real files (three planted regressions: a stripped banner, a successor file renamed away, a terminal file wrongly marked superseded; all three fail by name, the real files pass clean throughout). Full `preflight.py` run (not just fast) confirmed clean after. No site page, price, product or generator output touched; IndexNow not applicable. Full account in `ops/NIGHTLY-LOG.md`.

**Closed the standing doc-sprawl handoff's first concrete piece.** 18 root-level `.md` files (`AGENT-EVALUATIONS.md`, `AGENT-ROUTING.md`, `AUTONOMOUS-OPERATING-LOOP.md`, the `AUTONOMY-*` cluster, `COST-GOVERNANCE.md`, `LOOP.md`, `MISSION-CONTROL.md`, `OBSERVABILITY.md`, `OWNER-COMMAND-CENTER.md`, `OWNER-DIRECTIVES.md`, `SCHEDULER.md`, `SELF-IMPROVEMENT.md`, `SYSTEM-REGISTRY.md`) were all written 2026-08-16/17, day one or two of this repository, specifying a formal multi-agent orchestration layer (named roles, a scheduler, a memory architecture, formal handoffs). Verified directly rather than assumed from the dates alone: grepped `ops/*.py`, `CLAUDE.md`, `README.md` and the GitHub workflows for every one of the 18 filenames; none is referenced anywhere outside the cluster's own cross-references to itself. `MISSION-CONTROL.md` in particular styles itself "the concise, continuously maintained handoff between operating cycles" while its own body is an unfilled `UNKNOWN`-valued template, untouched in a month, while `GOALS.md`/`STATUS.md`/`BACKLOG-2026-09-07.md`/`ops/NIGHTLY-LOG.md` have been doing that job in practice the whole time. This matches a prior cycle's own finding (`STATUS.md`, 2026-09-16 PM check-in below): a background agent already read this same cluster end to end and found "11 of 12 genuinely aspirational/template specs with no live claim to correct," but stopped at fixing the one factual bug found (`EXECUTIVE-DASHBOARD.md`) without addressing the broader risk, that a template with no false claim in it can still read as live authority to a reader who does not know it was never adopted. Added the same supersession-style banner already proven on `BACKLOG.md`/`ROADMAP.md`/`STRATEGY.md`/`GROWTH-PLAN.md` (kept, not deleted, so the record shows what was designed) to all 18 files, pointing at the operating model that actually runs: `CLAUDE.md` section 0, `GOALS.md`, `BACKLOG-2026-09-07.md`, `ops/preflight.py`'s gates, `ops/NIGHTLY-LOG.md`. `CUSTOMER-JOURNEY.md`, `SPENDING.md` and `EXPERIMENT-PLAN.md` checked and deliberately left alone: the first still describes 6S Success's real content model, the other two are live, substantively-current policy documents, not part of the abandoned cluster (`SPENDING.md`/`EXPERIMENT-PLAN.md` were both edited within the last 5 days). Verified after: full `preflight.py` clean (every gate passed, 24 warnings, none new, `gate_root_docs_six_s_terms` still passing on the reworded files), `check_urls.py` (188/188), `audit_pages.py` (0/0), `affiliate.py --check` (162 documents), `fix_dashes.py --check` (0/0, the banner text itself carries none). No site page, price, product or generator output touched; IndexNow not applicable. The three-overlapping-backlogs and 89-file-count parts of the same doc-sprawl handoff are not yet worked, left for the next cycle rather than widened here. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, scheduled operator cycle):** Both the `ops/*.py` and root-doc cold-read lanes independently confirmed exhausted by many cycles today, so dispatched a background agent to cold-read the 9 lowest-mention root docs below that tier.

**Found and independently re-verified, not trusted from the agent's report: `GOALS.md`'s own O1 "Published videos" row had said "12 of 228" for six weeks, contradicting `MEDIA-OPERATIONS-PLAN.md` and `OWNER-ACTIONS.md`'s own "102 of 114 remaining," which was right the whole time.** 228 is a real count, just of the wrong thing: 114 zones rendered in two orientations (vertical and wide), not the YouTube publishing target. Confirmed directly against `ops/youtube_upload.py`'s own docstring ("the wide 16:9 file is the upload; Shorts are a separate distribution decision... not posted by this tool") and `build/video/youtube/`, the committed metadata directory that tool actually reads jobs from: exactly 114 files. The numerator (12) was never wrong. Fixed both rows in `GOALS.md` and the one live cross-reference in `PLAN-MICROZONES-DECKS-APP.md`'s M3 row. `gate_goals_published_videos_current` had "228" hardcoded as a regex literal, so it could never have caught its own denominator drifting; rewritten to derive the real total from `build/video/youtube/*.json` instead of a literal, with a dated correction in its own docstring. New `ops/tests/test_gate_goals_published_videos_current.py` (7 cases), fail-then-pass proved via `git stash` against the real pre-fix gate. Full `preflight.py` clean after (every gate passed, 24 warnings, none new), `check_urls.py` (188/188), `audit_pages.py` (0/0), `fix_dashes.py --check` (0/0). No price or product touched, no site page changed, IndexNow not applicable. Production remains on an older build than the repository; redeploy needs VPS access this sandbox does not hold, standing item in `OWNER-ACTIONS.md`. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in):** Checkout arrived shallow and detached; step 0 attached cleanly onto `origin/main` (`9727d295`), no conflict. Closed the standing handoff the prior PM check-in left open: run #1026 (Preflight on `c24c75ce`) confirmed `success` at 10:30:29Z via the Actions API, about 29 minutes after it started, longer than the 17-18 minute cluster cited but a real completion, not a hang. 8 GitHub issues confirmed live, unchanged, all `decision`/`blocked-on-art`. Did not complete a full local `preflight.py` run this pass (killed it partway to avoid colliding with `ops/ship.py`'s own run inside a 30-minute slot); relied on CI's own green run for gate coverage instead of an unfinished local one. Nothing else unblocked; handed the operator the standing doc-sprawl candidate (three overlapping backlog files) in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in):** This file was one cycle stale, last citing `3df5f0f0`; corrected rather than opening a fresh sweep. Since that entry, a scheduled operator cycle worked the handed-off root-doc cold-read tier: a background agent read all 12 previously-unswept root docs (`AGENT-EVALUATIONS.md` through `TESTING.md`, roughly 22.5k lines) end to end and verified every checkable claim, finding 11 of 12 genuinely aspirational/template specs with no live claim to correct. The one real finding, independently re-verified rather than trusted from the agent's report: `EXECUTIVE-DASHBOARD.md` section 104 still read `exists: UNKNOWN`/`last_verified: UNKNOWN` for the live dashboard itself, six lines below the same document's own banner already stating it exists and is regenerated by `ops/dashboard.py`; fixed, and a smaller misattribution in the same banner (crediting issue #8 with flagging the wrong pair of files as duplicates) corrected alongside it (`2c355b63`). Root-doc cold-read tier is now fully swept; no candidate files remain in that lane. Checkout arrived shallow and detached again this pass; standard step 0 handled it in seconds, ff-forwarded onto `origin/main` (`cfc2b516`), clean. `preflight.py` clean (every gate passed, 24 pre-diagnosed sandbox warnings, none new). 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 PRs. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. Working tree clean, main already pushed; `checks.yml` run 1023 on the merge commit preceding HEAD (`364d6a77`) still `in_progress` at the time of this check, ordinary timing, not yet confirmed. Production remains on an older build than the repository (last confirmed live `c3530a45f1e67279` at 2026-09-15T22:43:13Z; repository has since moved further ahead); redeploy needs VPS access this sandbox does not hold, standing item in `OWNER-ACTIONS.md`. No new defect found beyond this file's own one-cycle staleness. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in):** This file itself was 32 commits stale (last citing `7d3e68cc`, the RISK-0009/mirror-retirement cycle) and silent on two real pieces of work since; corrected rather than opening a fresh sweep. Since that entry: a PM check-in root-caused and fixed a real `checks.yml` flake in `test_ship_conflict_safety.py` (a detached `git gc --auto` child racing `TemporaryDirectory` cleanup), and CI run 1020 on that fix (`b276e85d`) was confirmed `success` via the Actions API by the following operator cycle, closing a handoff two cycles had left open. Most recently, a PM check-in tripped and fixed a real self-inflicted bug while verifying state: `test_audit_catalog.py`'s lock file has self-heal logic for an orphaned lockdir, gated on `STALE_AFTER = 900`, but its default wait `timeout` was `600`, so a waiter could never actually outlast the staleness threshold on its first attempt. Default timeout is now `STALE_AFTER + 120`; a new self-check proves the fix fail-then-pass. Checkout arrived shallow and detached this pass too; standard step 0 handled it in seconds, ff-forwarded onto `origin/main` (`3df5f0f0`), clean. `preflight.py` clean (every gate passed, 24 pre-diagnosed sandbox warnings, none new). 8 GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`, 0 PRs. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. Working tree clean, main already pushed. Production remains on an older build than the repository (last confirmed live `c3530a45f1e67279` at 2026-09-15T22:43:13Z; repository has since moved to `009f9d1dff3cc1e4`); redeploy needs VPS access this sandbox does not hold, standing item in `OWNER-ACTIONS.md`. No new defect found beyond this file's own staleness. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, scheduled operator cycle):** Checkout arrived shallow and detached; unshallowed, ff-forwarded cleanly onto `origin/main`, then merged one concurrent PM check-in mid-cycle (conflict only in the generated dashboard files and the newest-first nightly log; resolved by regenerating the dashboard and keeping both log entries). `preflight.py` clean throughout (every gate passed, 23 pre-diagnosed sandbox warnings, none new). 8 GitHub issues confirmed live via the API, unchanged (decision/blocked-on-art), 0 PRs. `inbox_agent.py --apply`: no mail credential, unchecked not empty. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; recomputed the `ops/*.py` cold-read mention floor (now 10+ for every file, that lane exhausted) and moved a tier over per step 5d: `build/listings/verify_zone_claims.py`, `mobile/quest-app/assets/zoneHeroes.js`, `mobile/quest-app/babel.config.js`, all clean (the first by actually running it against the real Etsy PDFs and hand-cross-checking every zone name against `MARKETPLACE-LISTINGS.md`'s copy). Then picked up a concurrent session's own handoff rather than relay it a third time: `content/book/6s-success-claude-files/` (79 files, `RISKS.md` RISK-0009's already-named "stale, untouched mirror" of the control layer) retired, checked first that no generator reads it and `ops/corpus_index.py` never scored any of its files postable. RISK-0009 closed: the control layer already carries a build-blocking gate for the six-s term (`gate_root_docs_six_s_terms`) and dashboard visibility for the dash rule, stronger protection than the risk's own mitigation had asked for. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in):** Checkout arrived shallow and detached; a real commit (`9e1c2a21`, the print manual font fix below) plus its own dashboard-regen commit landed mid-attach, so the first fetch's target went stale before the merge; re-fetched and ff-forwarded cleanly onto the true tip. Previous work confirmed finished: `preflight.py` clean, 23 pre-diagnosed warnings, none new; this file already correctly reflected the print-manual fix, no staleness found. Re-verified rather than cited: 8 GitHub issues unchanged (decision/blocked-on-art), 0 PRs, `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. Chased down an anomaly that looked like a path-filter regression (a third "Command deck regen only" commit triggered a Checks run despite touching only the three files the filter excludes) and traced it to the ordinary case: it rode in the same push as a real `content/**` change, which correctly matches. No defect found. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, scheduled operator cycle):** Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-only onto `origin/main` (`8c54da7b`), clean. `preflight.py` clean on arrival, 8 GitHub issues confirmed live and unchanged (decision/blocked-on-art), `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. Per step 5d cold-read eight low-mention `ops/*.py` files (all clean), then ran (not just read) `ops/build_manual_print.py`, which surfaced a real drift: commit `7e7d1db7` (2026-09-15, Phil) re-cut two font files roughly in half, but the committed print edition of the Micro Zone Manual (`content/manual/print/6S-Micro-Zone-Manual-PRINT-7x10.html`, last built 2026-08-21) still embedded the old, larger font bytes for both. No live customer affected: the $29 MZ-MANUAL product is fulfilled from a different file that links Google Fonts rather than embedding, and the print edition itself has no live destination yet (Amazon KDP account creation still owner-gated). Regenerated the file and added `gate_manual_print_fonts_current` to `ops/preflight.py` (same regenerate-and-diff posture as `gate_kdp_cover_current`/`gate_etsy_pdfs_current`), with `ops/tests/test_gate_manual_print_fonts_current.py` (5 cases) fail-then-pass proved both against an isolated fixture and against the real repository. Full `preflight.py` (every gate passed, 23 pre-diagnosed warnings, none new), all 159 non-slow test files, `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), `fix_dashes.py --check` (0/0) all clean after. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in):** Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-only onto `origin/main` (`8c54da7b`), clean. Previous work confirmed finished (`preflight.py` clean, 23 pre-diagnosed warnings, main already pushed, matching the prior PM check-in's own "fully closed" claim). Re-verified rather than cited every standing claim this slot depends on: 8 GitHub issues unchanged, read the two P0s (#2, #15) in full, both genuinely infrastructure/art decisions with the free mitigation already applied; recomputed the `ops/*.py` cold-read mention floor directly (now 10) and traced all 14 floor files through this log's own history, each already carrying a specific prior clean result. No new item survives independent re-check; nothing to fix, nothing to hand off beyond confirming CI goes green on `8c54da7b`. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, PM check-in, RETRO archive):** Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-only onto `origin/main` (`c1af20c3`), clean. Previous work confirmed finished (`preflight.py` clean, 23 pre-diagnosed warnings, main already pushed); picked up the prior PM check-in's own standing handoff (archive the 38 `RETRO-2026-08-*.md` cycle files into `retro/`) and closed it fully rather than partially: the move alone would have silently narrowed three real gates/metrics that scan root-level `*.md` files non-recursively (`gate_root_docs_six_s_terms`, the em/en-dash gate in `ops/fix_dashes.py`, and `ops/dashboard.py`'s `ctrl_em`/`ctrl_en` house-style count), so all three now also read `retro/*.md`, and 21 citations of a moved filename across five docs and four `.py` comments now say `retro/RETRO-...`. Fail-then-pass posture kept: confirmed the dash gate was clean before moving anything, reran it and the six-s-terms test (11/11) and full `preflight.py` (every gate passed, 23 warnings, none new) and `check_urls.py` (188/188) after. 8 GitHub issues unchanged (decision/blocked-on-art), 0 PRs, no mail credential. Full account in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-16, operator cycle):** Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-forwarded onto `origin/main` (`7895ecb8`), clean. Every unblocked `BACKLOG-2026-09-07.md` row again done or Phil-gated; cold-read `ops/build_standards.py` per step 5d and closed a real (currently latent) gap: `site/downloads/6S-Standards-Pack.html`, the free 20-sheet pack a visitor actually downloads, had no gate protecting it against drifting from `build/6S-Standards-Pack.html` once content changes and the manual copy step is skipped. `gate_downloads_noindex`'s own docstring had already named the copy step as unprotected for one failure mode (missing noindex/canonical tags) but nothing checked the content itself. New `gate_standards_pack_current`, fail-then-pass proved directly against the real committed files. No live defect: the two files were already byte-identical. A concurrent PM check-in landed on the same base (`7895ecb8`) mid-cycle; merged cleanly, no content conflict beyond this header. Full account in `ops/NIGHTLY-LOG.md`. 8 GitHub issues unchanged (decision/blocked-on-art), 0 PRs, no mail credential.

**Prior (2026-09-16, PM check-in):** Checkout arrived shallow and detached; unshallowed, `git fetch --unshallow` then ff-only onto `origin/main` (`7895ecb8`), clean.

**Did (2026-09-17 afternoon, local session, CEO cycle):** one URL per page, live and verified. Proxy logs read across rotated files (not just the current one) showed genuine Googlebot (66.249.x) fetched all 115 zone URLs twice on 23 to 27 Aug, once as `/zones/<slug>` and once as `.html`, then fell to a few content fetches a week; and 29 of 143 Googlebot fetches 10 to 17 Sept went to `www.`, which answered 200 with the whole site. `site/nginx/default.conf` now 301s `www.` to the apex and the `.html` twin of every zone/room/article to its extensionless canonical (index pages excluded: without that, `/zones/` 301'd to itself, caught in a VPS test container before commit). `/rooms/` and `/deck/` redirects made absolute https. Guarded by `ops/tests/test_nginx_www_redirect.py` (fail-then-pass). Production `6c155b8b48679497` to `128f51debc58e79e`; live checks: www and .html 301 in one hop, canonicals 200, sitemap 188/188, beacon 200, container healthy. Closed the www half of OWNER-ACTIONS 9, parked since 2026-09-03 as a proxy change it never needed. LRN-0013 corrected: its "Googlebot never read a content page" came from the unrotated log only; Google read everything once and did not come back, which points at page value/authority, testable only with Search Console (OWNER-ACTIONS 1a). Fresh Umami read the same hour, last 7 days: 18 visitors, 28 visits, 57 pageviews; Google referrals 6 all-time. **Operating finding for Phil:** 1,247 commits 10 to 17 Sept, about 74% check-in, dashboard-regen or merge commits, 118 mentioning duplicate/concurrent work, 13 "no new closeable item"; 70 touched `site/`.

**Did (2026-09-17, local session):** deployed 95 commits of finished, undeployed work and verified it live. Production had sat at `a055ace82941078e` while the repository moved to `6c155b8b48679497`, 10 `site/` files apart. Measured before deploying by diff-derived markers: **0 of 32** added lines live on `kitchen-deck.html`, **0 of 21** on `invest.html`, **0 of 1** on `book.html`. Two were P2 trust fixes nobody could see: `invest.html` still served unsourced superlatives and called unbuilt app/kit capability present fact, and `book.html` still did not disclose that its free HTML sample degrades 172 of 231 figures to text. After deploying, the same method reads 32/32, 21/21, 1/1. The Kitchen deck's micro quests are live: 7 "Ninety seconds, if that is what you have" headings, 21 items. Six paths 200 in 0.37-0.54s, container healthy, 0 restarts. I had built the same micro quests independently and dropped my commit: `37705880` landed first with a better implementation (deck-wide uniqueness assertion, a dedicated `gate_kitchen_micro_quests`, a better heading), checked rather than assumed before yielding. **Open finding for the next cycle, corrected:** the deploy gap is an operating-loop failure, not a missing check. The watchers exist and fired; `ops/state.json` read `deploy_verdict: stale` and the dashboard printed "Redeploy the site" for days while every session read past it. What is missing is routing that line to a session with deploy access. Needs no owner gate.

**Did (2026-09-16 late, local session):** two discovery fixes shipped and verified live at the constraint. `llms.txt` now names the Standards Pack (2,193 to 2,460 bytes live): the one page search has measurably sent anyone to was absent from the file that tells answer engines what exists, and it is now enforced by `LLMS_TXT_MUST_NAME` with the test at 9 cases, the new one proven by sabotage. `robots.txt` now carries `Disallow: /stats/`: of Googlebot's 129 fetches on this domain, 21 went to `/stats/api/send` and 6 to `/stats/script.js` while **none** went to a zone, room or article page. Production `0bd535b9df92bdd5` to `a055ace82941078e`, matching the repository; the beacon still returns 200 to a browser-shaped POST, so crawler directives did not touch real visitors. Four of my own readings were retracted this cycle (IndexNow wrong in both directions, the sitemap `lastmod` "defect" that is a documented trade, `/cart.html` which is a correct 404). Open question for the next cycle: Googlebot reads no content at all while Bing crawls pages freely, which looks like authority rather than crawl budget, and testing it needs Search Console (OWNER-ACTIONS 1a).

**Did (2026-09-16 evening, local session):** shipped and verified live two customer-facing changes. `standards.html`, the one page search actually sends anyone to, gained five questions answered in the page's own sentences, visible and in JSON-LD (26,576 to 30,170 bytes, 2 to 3 JSON-LD blocks, 0 to 5 Question nodes, verified on the live URL). It was the only page shape on the site with no FAQ; 115 of 140 pages already had one. Also deployed a concurrent session's overclaim fix that was committed and sitting undeployed: the print-and-play page told customers "88 cards, illustrated, front and back" when 9 of 88 carry no photograph, caught live one minute before the deploy and now corrected. Production `75f9963d239b9112` to `0bd535b9df92bdd5`, matching the repository exactly. First direct production inspection in days (this machine holds the deploy key, cloud sessions do not): analytics proven alive end to end after a 45-hour no-event gap, every buy-click ever recorded traced to Phil's own devices, and the VPS disk at 79% with 45.96 GB reclaimable build cache escalated as OWNER-ACTIONS 1f rather than pruned on a host shared with Ledgerium's live billing. `DECK-GAME-DESIGN.md` written against the deck brief: the 1-3 minute tier cannot be harvested (684 zone steps run 25-134 words, 23% open with a physical verb) and must be authored, 21 lines for the Kitchen only.

**Did (this pass): previous work confirmed finished; the ops-file cold-read lane confirmed exhausted (every one of 139 files now at 9+ mentions in `ops/NIGHTLY-LOG.md`); handed the operator a doc-sprawl cleanup instead of manufacturing a new gate.** `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, same count as last cycle, none new. 8 open GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`; `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. No egress from this sandbox (`curl` to `6s-success.com` resolves DNS but the connection itself fails), so the 2-day-stale traffic/affiliate carried-forward readings could not be refreshed here, consistent with every prior cloud session. Full account, including the doc-sprawl finding (120 root-level `.md` files, three overlapping backlogs, 38 unarchived `retro/RETRO-2026-08-*.md` cycle files), in `ops/NIGHTLY-LOG.md`.

**Did (prior pass): previous work confirmed finished, one real gap closed.** `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, none new. `checks.yml` run 1002 on `2525057e` confirmed `success`; the current head (`af218da2`) has its own run in progress, ordinary timing, not a stall. 7 open GitHub issues confirmed live via the API, unchanged except one new: `BACKLOG-2026-09-07.md`'s own newest finding (23 catalogue entries with no explaining page, logged 2026-09-15 evening) had never actually been added to that file, only to `ops/NIGHTLY-LOG.md`, the exact "measured but the document of record was never updated" gap section 7 names. Added section 1b to `BACKLOG-2026-09-07.md` and opened issue #32 (`decision`) with the two options and a recommendation, rather than leave it findable only by reading the log. Also corrected the prior PM check-in's own handoff: it recommended cold-reading `stripe_check.py`/`stripe_links.py` next, but `stripe_links.py` has already been cold-read clean at least six separate times across this log and `stripe_check.py` was confirmed clean again this same day; the operator cycle that picked up the handoff had already substituted a genuinely unread file (`send_brief.py`) and found a real defect there instead, so no work was lost, but the stale recommendation would have cost a future cycle's slot if left standing. `BACKLOG-2026-09-07.md` sections 2-6 otherwise again all done or Phil-gated.

**Did (this pass): this file itself was 17 commits stale (last citing `21f907a1`) and silent on three real pieces of work since; corrected rather than opening a fresh sweep.** Since the last entry: a local session shipped `8dfa3fce` (shop pack cards link the free page that explains them), whose first release `publish-image.yml` correctly refused (`gate_generator_ownership`: the new links used `.html`, not this site's extensionless canonical form); the fix landed twice, once as this session's own `1c93dc1d` and independently as a concurrent local session's identical commit (kept on branch `backup/pack-links-local-784c6eba`, not pushed, to avoid a duplicate). A PM check-in then confirmed both `checks.yml` and `publish-image.yml` green on `1c93dc1d` before starting new work, per its own explicit handoff. This session found `ops/revenue_model.py`'s `CHECKOUT_PAID, CHECKOUT_TOTAL = 1, 7` and `ROADMAP-2026-2029.md`'s identical citation both predated `LEARNINGS.md` LRN-0010 (2026-09-14, traced every one of those checkout sessions to the owner's own household) and never cited it, so a placeholder already labelled noisy read as possibly not describing a real customer at all; added the citation to both (numbers unchanged, still labelled placeholders) and a new `gate_revenue_model_checkout_caveat` so it cannot silently drop again (`e50c8048`). A concurrent push landed while this ran; merged cleanly (`8f31ded3`) and the command deck was regenerated fresh rather than hand-resolving its generated files (`46ade0c1`). **`checks.yml` run 1001 on `46ade0c1` is still `in_progress` as this is written** (Preflight step running; the prior same-workflow run on `1c93dc1d` took 29 minutes, so this is ordinary timing, not a stall); CI green on this exact head is NOT YET CONFIRMED, next check-in should confirm it before starting fresh work if this line is still here. `preflight.py` fast, local: every gate passed, 23 pre-diagnosed sandbox warnings, none new. 7 open GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; nothing newly unblocked this slot.

**Did (prior pass): previous work confirmed finished (preflight clean, 7 issues unchanged, main pushed); mid-cycle a local session with real VPS access pushed `19b1e298`, deploying the storage-after-Sort fix that had built green hours earlier but never gone live. Re-fetched, regenerated the command deck, and corrected section 30's production-knowledge paragraph, which still cited the prior build.** `ops/deploy-verdict.json` now reads build `c3d0d442441b24df` at `2026-09-15T21:13:31Z`, matching `site/build-id.txt`; the dashboard's headline constraint line has correctly dropped the deploy gap and reverted to discovery/traffic. `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, none new. 7 open GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`.

**Did (prior pass): previous work confirmed finished; section 30's production-knowledge paragraph corrected, a real cold-read of `preflight.py --deep` left running (handed off by the prior PM cycle, not yet completed by any cycle today).** `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, none new. Working tree clean, main already pushed before this pass touched anything. 7 open GitHub issues confirmed live via the API, unchanged, all `decision`/`blocked-on-art`; `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated. Found: section 30's "Production Knowledge" paragraph, written by the 11:20:39Z STATUS.md commit, said build `346c043b56385f64` was "not yet redeployed," citing `e42a98a4e7a2e266` as the last confirmed build. A later commit (`a064f2fa`, 11:49:23Z, a local session with real VPS access) had since committed an updated `ops/deploy-verdict.json` confirming `346c043b56385f64` itself went live at 11:17:08Z, but never touched this paragraph, the exact "corrected source, unrederived artifact" defect class `BACKLOG-2026-09-07.md` section 7 names as the dominant one this month. Corrected the paragraph and the "Immediate Focus" line's stale "redeploying is Phil's own action" clause to match; `site/build-id.txt` (`346c043b56385f64`) and `ops/deploy-verdict.json` now agree, both read fresh, not cited from memory.

**Did (this pass): previous work confirmed finished, this file's own currency corrected rather than another cold-read sweep opened.** `preflight.py` fast: every gate passed, 23 pre-diagnosed sandbox warnings, none new. Working tree clean, main already pushed before this pass touched anything. The most recent operator cycle (`042079dd`, logged in `ops/NIGHTLY-LOG.md` but never reflected here) closed GitHub issue #20 as superseded (the four-tier pricing ladder it asked Phil to confirm named a 46-card deck; the live catalogue has one free 88-card deck, no paid tier), corrected `PRICING.md` section 0.6 to match, and added `gate_pricing_deck_ladder_current` so it cannot drift silently again. Confirmed live via the GitHub API rather than trusted from the log: 7 open issues, not 8, all still `decision` or `blocked-on-art`. Section 30 below still named 8 open issues and cited production two builds behind the repository's current `346c043b56385f64` (`site/build-id.txt`); corrected both against `ops/deploy-verdict.json` (production last confirmed current at `e42a98a4e7a2e266`, 2026-09-15T10:37:19Z) and the live issue count. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; nothing else unblocked this slot.

**Did (prior pass):** previous work (last cycle's syntax-error fix and EP-001 root-cause, `cb00b337`) confirmed finished: `preflight.py` run unbounded, every gate passed, 23 pre-diagnosed warnings, none new; working tree clean, main pushed. `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; all 8 open GitHub issues carry `decision` or `blocked-on-art`; 0 open PRs. Section 30 below ("Current Overall Assessment") had gone stale since 2026-08-19/08-26: it still cited a superseded 60/161 traffic reading against GOALS.md's current 75/196/947 (2026-09-14 21:30), called the email list "unreadable" when it is readable and measured at 0, and dated production knowledge to 2026-08-19 when a real session confirmed it current on 2026-09-15T05:21:21Z. No gate covers this section (`gate_risks_traffic_citations_current` only checks `RISKS.md`). Rewrote it against GOALS.md, EXECUTIVE-DASHBOARD-LIVE.md and OWNER-ACTIONS.md, all read fresh this pass. Nothing else unblocked this slot; handed off to the operator in `ops/NIGHTLY-LOG.md`.

**Did (prior pass):** found RISKS.md's two most-read entries, RISK-0013 ("this register's own most consequential entry") and RISK-0005, still citing a superseded traffic reading (68 visitors/161 visits, 2026-09-11) a cycle after GOALS.md moved to 75/196/947 (2026-09-14 21:30), and RISK-0013 never mentioning LRN-0010 at all, so it still treated the 16 unpaid Stripe checkout sessions as an open, ambiguous lead that finding had already retired (0 stranger buy-clicks since 7 Sept, not "16 ambiguous"). Fixed both entries. New `gate_risks_traffic_citations_current` in `ops/preflight.py` requires GOALS.md's current baseline appear in any RISKS.md block that cites an actual visitor/visit figure; own substring-matching bug caught and fixed before shipping (see `ops/NIGHTLY-LOG.md`). A concurrent PM check-in pushed mid-cycle (dashboard-only regen after a stale commit gap); rebased cleanly, no content conflict. Full `preflight.py` clean after (0 gates failed, 23 pre-diagnosed warnings), `check_urls.py` (188/188), `affiliate.py --check` (162 documents). 8 GitHub issues unchanged (decision/blocked-on-art), no egress, no mail credential.

**Did (2026-09-15 afternoon, local session):** reviewed the morning's card work and found the shared checklist split (`video_zone.done_items()`, feeding video beats, caption sidecars and social cards) still broke noun lists sharing one qualifier ("Broom" / "Mop and dustpan hanging ...", "Diapers, wipes" / "Cream all touchable ...") and dropped the "and" inside lists; corrected, with a test that counts every word. Instagram captions had a four-item cap that dropped standards such as "The cabinet strapped to a wall stud" from 16 captions; removed. Card eyebrows now stay on one line. Regenerated 20 caption sidecars, 32 social captions and 62 cards (185 of 228 now carry a picture) (`7ef3a6fa`). OWNER-ACTIONS item 1 no longer tells Phil to use a YouTube replace-video option that keeps the URL: that option does not exist.

**Did (2026-09-15 late morning, local session):** the 228 Pinterest and Instagram cards now lead with each zone's approved picture (106 of 114 zones), fitted in the browser and read back, failing the build on overflow. Their checklist split was broken: it cut lists apart and dropped words ("one wallet and one phone per adult" became "One phone per adult"), and it now keeps every word; the same longer items had pushed 15 Pinterest descriptions into a cut that removed the zone link, now fixed. All 228 cards rebuilt and measured as fitting (114 of 114 zones, both surfaces). 179 carry their zone's picture, 8 of those leaving one item behind a '+ 1 more on the zone page' line. 49 are text only: 33 whose standard is too long to share the canvas, and 16 for the 8 zones with no approved picture. Cards average 148 KB (33 MB for 228, was 28 MB). Found, not fixed: 11 of the 12 published zone videos show and narrate the old broken checklist (BACKLOG 1b).

**Did (2026-09-15 mid-morning, local session):** product tiles on the shop, homepage, method and book pages serve responsive AVIF/WebP (`247dcef6`): first 8 shop tiles on desktop 1,632 KB to 96 KB, all 159 tiles 16.5 MB to 1.7 MB. Card images declare their true widths (150/400/760, `27b0ac2d`) and the free deck page's six card previews became pictures, the last bare JPEGs over 40 KB on the site. The two 400-weight body fonts were full variable fonts used at one weight; pinned, they drop from 352 KB to 152 KB with identical glyphs and pixel-identical zone and article renders, and nginx gzip moved from level 1 to 6 (quest-data.js 167 KB to about 135 KB) (`7e7d1db7`, live at build `346c043b56385f64`). `ops/deploy-verdict.json` had been one deploy behind all night because the session committed it before each deploy; it is now committed after deploys, and the OWNER-ACTIONS redeploy question it raised is resolved (no redeploy needed).

**Did (2026-09-15 morning, local session):** the large photos on index, about, method, consulting and book are responsive AVIF/WebP pictures (`082d0a73`): first-view image bytes across those pages 1,395 KB to 183 KB desktop, 65 KB phone. Its first image build failed on `test_gate_ops_python_syntax.py` (a fixture valid on Python 3.12+); fixed in `feb32974`. Open risk: `checks.yml` cancels in-progress runs per ref, so under concurrent pushes Checks rarely finishes on main (four cancelled in a row after 06:10); recommended fix recorded in the nightly log, not made.

**Did (2026-09-15 early morning, local session):** all 20 room pages show each zone's approved illustration as a thumbnail (106 thumbnails, 8 empty slots for rejected zones, the Quest's own honest note); the 11 room pages that had no image at all now carry pictures. gate_pages_missing_art now keys room pages on the chapter lead figure so the thumbnails cannot mask the 11 unillustrated chapters. Release `e170c110` live and verified (build `dc6db21934bf8a6e`, ALL PASS). Site-wide visual audits at 390 and 1280px: 193 pages, 0 findings.

**Did (2026-09-15 late night, local session):** card art release verified live (build `4e533c792222ee54`, served deck PDF byte-identical; 9 cards without a photo, not 12). Quest app: six-pass spine and 44px footer links (`0d0f5b1b`), then each card shows its zone's approved picture as a bundled 74x56 thumbnail (`f8d687ce`, 106 pictures, 1.4 MB, gated for drift). App changes verified by tests, gates and a 390px web render, NOT on a device and NOT by CI (checks.yml does not watch mobile/**).

**Did (2026-09-15 night, local session):** homepage hero badge now names the pass the Quest opens on (Sort), gated; method page course grid balanced 3 + 3; release `04985c27` live and verified (build `15cb07c13abaadca`, ALL PASS). Three of the twelve placeholder Entryway cards now carry locally generated, reviewed photographs (EP-008, ET-004, EU-011); free deck PDF rebuilt, 12 placeholders to 9, live check pending. The eight image-less zone pages stay text-only: 0 of 11 zones produced an acceptable local image (LRN-0012).

**Did (2026-09-15, supplies and shop):** zone and room page supplies are cards with a quieter disclosure and 44px tap targets, live at `2f183516`. Micro Zone Packs show their own zone pictures in the shop, live and verified. Known limit: several approved zone pictures show a general room view rather than the exact micro zone.

**Did (2026-09-15, local session):** home hero shows a real Quest card (gated), deck hero shows real card fronts, a 114-page heading cascade bug fixed, card template improved (objective panels, whole taglines, pinned footers) with the deck PDF rebuilt, and the app bundles the site's fonts. Release `0b4c4fa4` is pushed; production stays on `ce8d3507` until its image builds and the live check passes. The first attempt, `1dea8896`, failed publish and never deployed.

**Did (late evening, same local session):** reduced-motion accessibility fix deployed and verified on production (build `ce8d3507`; the live stylesheet carries `.js .reveal{...transition:none}`), with CI Checks on `8c139586` green. Rebuilt three stale Etsy packs (Kitchen, Moving-In, Holiday Hosting). No listing is live, so no buyer had the old copy. **CORRECTED 2026-09-15: wrong. The rebuild used stale untracked intermediates on this workstation and reintroduced old copy. `0374e09c` fixed the generator and restored the current text (identical to pre-rebuild). No buyer affected. See LRN-0011.** Open: CI has no browser, so browser tests there report NOT VERIFIED rather than pass, and commits touching only `site/` get no Checks run.
...
**Prior pass, for continuity: the executive dashboard's own headline sentence was silently dropping a known, still-open production outage on every cloud run.** Every unblocked row in `BACKLOG-2026-09-07.md` sections 2-4 was again done or Phil-gated (Pinterest/Instagram crops, named there as unblocked prep work, already exist on disk: 114 each), so per step 5d this pass checked the dashboard's own generation logic rather than another `ops/*.py` file. `ops/dashboard.py`'s "constraint" sentence, the single most prominent line on the page, checked `S["deploy"]["verdict"]` (this run's own unmeasured probe, always `"unknown"` without egress, which is every cloud sandbox) instead of `S["deploy_verdict"]` (`resolve_deploy_verdict()`'s already-correct carried-forward value). Proved live before fixing: the committed `ops/state.json` read `deploy_verdict: "stale"` while `constraint` said "Discovery... is the constraint now," meaning only a session with real egress could ever re-trigger the correct "PRODUCTION IS SERVING AN OLD BUILD" framing, and every other run since would silently understate a real, still-open deploy outage in the one sentence Phil reads first. Fixed the field read; regenerated, `constraint` now correctly names the outage while the carried verdict is `stale`. New `gate_dashboard_constraint_reflects_carried_deploy` in `preflight.py`, fail-then-pass proved directly against the real committed file (planted the old comparison, watched it fail by name citing the exact line, reverted, reran clean).

**Verified:** `preflight.py` fast clean before and after (0 gates failed, 23 warnings, all previously diagnosed sandbox limits), `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), mobile `npm test` (4 suites, all pass). 8 GitHub issues unchanged (decision/blocked-on-art), 0 PRs, no mail credential, no egress (confirmed directly: `curl` to `6s-success.com` returned exit 56). Full account in `ops/NIGHTLY-LOG.md`.

**Prior pass, for continuity: production was unpublished for four real commits; found and fixed the actual cause.** The prior check-in closed CI (`checks.yml` green) and read that as previous work being finished, but `publish-image.yml`, the workflow that actually builds and pushes the image the host pulls, had failed on the last two pushes. `gate_publish_image_current` (visible only on a full, non-fast local run) confirmed HEAD's `site/` had not been published since `60b4b99`; four real merged commits had never shipped. Root cause, from the CI job log: `gate_generator_ownership` failing with "9 file(s) differ," two independent misses. D7's zone-title fix regenerated three generators but not `ops/build_social_captions.py`, leaving 8 Pinterest/Instagram caption files with the pre-fix doubled title ("guest bathroom guest vanity counter"). Separately, D11's spelling fix regenerated every HTML page but not `site/sitemap.xml`, which still carried a machine-read `<image:caption>` saying "organiser." Fixed by rerunning the real 21-generator ownership chain end to end; confirmed exactly 9 files move and no others, matching CI's own count. Full account in `ops/NIGHTLY-LOG.md`.

**Handing to the operator:** confirm the next `publish-image.yml` run is green on this fix, the only real evidence production has caught up.

**Prior pass, for continuity: a week-old commercial review's own named findings, re-verified and three of them fixed.** `REVIEW-COMMERCE-2026-09-07.md` ("This file is a report. It changes nothing else.") had sat with nothing acted on for a week; a prior same-day check-in named it and two other cold-read candidates as the remaining fresh lane once the backlog and `ops/*.py` were confirmed exhausted again. Checked each finding directly against live code rather than trust the report. Fixed: (1) the homepage's only sentence for a B2B reader linked `consulting.html` (the household service) instead of `corporate.html`; (2) `how-we-make-money.html` said "Almost all of our revenue comes from products we made", true of one $19 sale but implying a revenue stream that does not exist (`CLAUDE.md` 8); (3) `ops/build_zone_pages.py` and `ops/build_articles.py` hardcoded "19 dollars"/"250 dollars" as string literals across 134+ pages instead of reading `PACK-HOUSE`/`CN-VIRTUAL` from `data.js` live, the same defect class that already produced a live $18 charge on a $9.99 page once. Both generators now read price live; regenerated pages are byte-identical (no price has changed), and both already sit in `gate_generator_ownership`'s regenerate-and-diff chain, so a future reprice without regenerating now fails that gate automatically. Fail-then-pass proved in an isolated worktree for `build_articles.py` (`build_zone_pages.py`'s own half of that gate is skipped in this sandbox, `build/heroes/` absent). Annotated the review file with what is fixed and what remains genuinely open. Full account in `ops/NIGHTLY-LOG.md`.

**Verified:** `preflight.py` clean before and after (0 gates failed, 21-22 warnings, all previously diagnosed), `check_urls.py` (188/188), `audit_pages.py` (0/0 duplicates), `affiliate.py --check` (162 documents). 8 GitHub issues unchanged since 2026-09-11, all `decision`/`blocked-on-art`; 0 PRs.

**Did (this pass): privacy.html cold-read closed, consuming the standing handoff.** privacy.html and accessibility.html were the last two hand-maintained pages without an individual content-honesty read (CLAUDE.md 8-10). accessibility.html checked out clean. privacy.html's "two people have ever finished a card" (added 2026-09-07, `94a36cc6`) is a live Umami count with no date attached; no sandbox this week and no credentialed CI job can re-derive it, so it could only ever drift silently while reading as current. Reworded to name the date it was checked. New `gate_quest_finisher_count_dated` in `preflight.py`, `ops/tests/test_gate_quest_finisher_count_dated.py` (7 cases) fail-then-pass proved directly against the real file. Full `preflight.py` (0 failed, 23 warnings, all previously diagnosed), `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), mobile `npm test` (4 suites) all clean. 8 GitHub issues unchanged since 2026-09-11, all decision/blocked-on-art; 0 PRs. One spurious `fingerprints` FAIL from an accidental duplicate `preflight.py` process (this session's own background-tooling mistake) caught, root-caused to resource contention rather than trusted, killed the duplicate, reran clean; no code defect. No further hand-maintained page remains unread in this lane.

**Prior pass, for continuity: today's whole-day CI outage is closed, verified against a real green CI run, not assumed from a local pass.** The prior pass's own fix (a `timeout 700` bound wrapped around checks.yml's "ops test suite" shell loop) shipped and, the very next real run (917), FAILED by name at exactly 700s on `test_generator_ownership.py`: not hung, not a real regression. That file drives a full `preflight.py --own` in a throwaway git worktree (up to 1800s) unless it sees `SIXS_UNDER_PREFLIGHT` in its environment, the exact variable `gate_tests()` in `preflight.py` already sets before running these same files, for the identical recursion reason; the shell loop mirrored `gate_tests()` in every other respect but never set this one variable. Fixed by exporting it before the loop (`30d1a7bc`); new `gate_ops_test_suite_matches_gate_tests` in `preflight.py` statically checks the step keeps exporting it, `ops/tests/test_gate_ops_test_suite_env.py` (4 cases) fail-then-pass proved directly. Watched the resulting run (918) to a real, verified conclusion via the Actions API: every step's own `conclusion` field read `success`, Preflight 16m45s, the ops test suite 10m47s (down from 22.5 minutes before failing, because the fixed file now skips in under a second). Full account of all three fixes (unbound loop, mismatched timeout, mismatched env var) in `ops/NIGHTLY-LOG.md`. No backlog row was open past this: `BACKLOG-2026-09-07.md` sections 2-6 remain done or Phil-gated.

**Prior pass, for continuity:** today's whole cycle history (dozens of runs, this operator and several concurrent sessions) chased a real CI outage in `.github/workflows/checks.yml`: a chain of `subprocess.TimeoutExpired` crashes and Chrome resource contention on GitHub's shared runner, root-caused and fixed in stages (a bare `--no-sandbox` condition, `c1e31bfe`'s checks.yml trigger loop on the command deck's own generated files, `6b737dca`'s uncaught outer timeout in `gate_tests()`, `69779797`'s second instance in `gate_mobile_js_tests()`, `ef47f8cd`'s retry-count guard). The latest real CI run (912) FAILED, not crashed, with one concrete line: `test_gate_etsy_pdfs_current.py`'s own diagnostic fallback (added earlier today to work around CI's 90-character truncation of failure summaries) hit the exact "no PDF produced for" exhausted-Chrome-retries shape that `gate_etsy_pdfs_current()` itself already treats as UNCHECKED/warn, not a defect, but the diagnostic block had no such handling and unconditionally counted it as a test failure. Fixed by mirroring the production gate's own already-proven posture (the same string match `gate_etsy_pdfs_current`'s `exhausted_retries` branch uses, and the same shape case 7 in the same test file already proves deterministically with a mock): a diagnostic re-run that hits this exact shape is now printed as INCONCLUSIVE and not counted as a failure, while any other diagnostic failure shape still fails the test normally. Verified: local `preflight.py` fully clean (0 gates failed, real Chrome renders fine locally, so this branch was never hit here), the full test file passes 7/7, and a standalone proof (a fake `build_etsy_assets.py` that deterministically prints the real CI failure line) confirmed the old code would have counted it as a failure and the new code correctly does not.

**Handing to the operator:** watch the next real CI run to confirm this closes today's outage for good; if CI is still not green, the next failure is a genuinely new shape, not a repeat of anything fixed today.

**Prior pass, for continuity:** picked up the prior cycle's own handoff rather than a fresh sweep. `build/listings/build_etsy_assets.py`'s five Etsy PDFs were unprotected against source drift, the same shape `gate_kdp_cover_current` closed for the KDP cover. Checked with pymupdf before acting: `build/6S-Whole-House-Print-Pack.html` was substantively regenerated 2026-09-07 (the Sustain rewrite) and the Kitchen pack's zones carry the same rewrite, while all five delivered Etsy PDFs were still exactly what was rendered 2026-09-03. `check_etsy.py` only compares page/card counts, which matched (no card added or removed, only its text improved), so the stale listings passed every existing check; a real buyer would have received the pre-rewrite text. Fixed, not just flagged: widened `find_browser()` to also find this sandbox's own Chromium (the script was hardcoded to a Windows-only Edge path), regenerated all five listings, verified with `check_etsy.py` and by reading the extracted text directly. New `gate_etsy_pdfs_current` (normalized-text diff, not raw bytes, since two real renders of identical source proved byte-different but text-identical), `ops/tests/test_gate_etsy_pdfs_current.py` (5 cases, real isolated fixtures through the real browser), fail-then-pass proved. Full account in `ops/NIGHTLY-LOG.md`.

**Handing to the operator:** `check_etsy.py`/`verify_epub.py` still not cold-read for their own sake (only exercised as part of this fix). The same "renders a customer deliverable outside `ops/`" shape may exist elsewhere; not yet swept. Standing 8 decision/art issues and `OWNER-ACTIONS.md`, unchanged.

**Prior pass, for continuity:** cold-read `mobile/quest-app/lib/importProgress.js` and found `mergeDone()` did not fully deliver the self-contained guarantee its own docstring promises. When both the on-device and incoming timestamp for the same card are invalid, the merge wrote `undefined`; `JSON.stringify` drops that key on persist, and `pickCard.js` already treats any truthy `done[id]` (even corrupted) as done, so a card the phone had already hidden as finished would silently reappear as not-done on the next reload. Unreachable through today's one real caller (`parseBackup` drops bad incoming entries first), but exactly the shape the docstring says cannot happen. Fixed by falling back to the existing value when both sides are invalid, rather than `undefined`. New test proves it, fail-then-pass via `git stash`. No new preflight gate needed: `gate_mobile_js_tests` already runs every `lib/*.test.js` file directly and already caught the pre-fix case by name. Verified: mobile `npm test` (4 suites), `preflight.py` clean after (one self-inflicted `stray-probe-files` FAIL from this session's own killed concurrent preflight run, self-healed, reran clean). Full account in `ops/NIGHTLY-LOG.md`. No price, product or page touched.

**Verified, no defect found:** the `ops/*.py` cold-read lane is confirmed still fully exhausted (every file at 8+ log mentions), so per step 5d moved one tier over to `build/listings/*.py` (2-5 mentions each): `amazon_nodes.py` and `amazon_suggest.py` read cold, both correctly scoped (Amazon's real browse tree and autocomplete, explicitly not presented as volume data) and both need live Amazon egress this sandbox does not have, so neither could be run, only read. `etsy_economics.py` read cold and is sound: it deliberately takes Etsy's fee rates as CLI arguments rather than hardcoding a number nobody has verified since Etsy started 403-ing automated fetches. `build_etsy_assets.py` read cold, sound, but Windows-only (shells out to a literal `msedge.exe` path), cannot run here. `build_kdp_cover.py` (the one file in this tier that is plain Pillow, no browser) actually run rather than only read: installed Pillow fresh in this sandbox, regenerated `build/listings/kdp/cover-kdp.jpg` from `build/cover.png`, and pixel-diffed the fresh output against the committed file before letting the regen overwrite anything (`ImageChops.difference(...).getbbox()` returned `None`, exact pixel match). Confirms the KDP cover asset is current, not stale, closing this tier's one file that could actually be exercised here. No defect found in any of the five.

**Prior pass, for continuity: per step 5d, moved to the `build/listings/*.py` tier (`amazon_nodes.py`, `amazon_suggest.py`, `build_kdp_cover.py`, each at 1 mention). `build_kdp_cover.py` regenerated byte-identical to the committed KDP cover once Pillow was installed in this sandbox (not present at start; installs cleanly, taking `preflight.py`'s "cover geometry: Pillow not installed" line from unchecked to checked for this run only, no repo change), confirming it is current, not stale. The real find was a step away, in `.github/workflows/status-email.yml`: its cron minute moved from `:10` to `:23` on 2026-08-31 (`f879787d`, dodging a congested minute, the same fix `hourly-brief.yml`/`fulfil-orders.yml` got that day), and the line's own inline comment was updated to "23 past," but the block comment three lines above still read "Ten past" and nothing had ever compared the two: the same "source corrected, artifact never re-derived" class this file's own history names as dominant, this time inside a workflow's schedule comment rather than a page or a doc. Corrected the comment. New `check_schedule_past_comments()`/`gate_schedule_comment_minute_current()` in `ops/preflight.py` scans every file in `.github/workflows/` (not a fixed list) for any "NN past" claim, digit or spelled out (reusing `_spelled_number()`, the same cardinal-word parser `gate_deck_count` already proved), and fails if it disagrees with the file's own real cron minute(s).

**Verified:** `ops/tests/test_gate_schedule_comment_minute.py` (6 cases) fail-then-pass proved directly: the real pre-fix text failed by name citing "Ten past" against the real minute 23, the fixed text and every other real committed workflow file (including the multi-minute `fulfil-orders.yml` and the no-"past"-phrase `roadmap-report.yml`) came back clean. Full `preflight.py` clean after (every gate passed, 21 warnings, one fewer only because Pillow is now present in this sandbox), all 118 test files individually, `check_urls.py` (188/188), `audit_pages.py` (191/0, 0 duplicate titles/descriptions), `affiliate.py --check` (162 documents), mobile `npm test` (4 suites) all clean. No price/product touched, no site page changed, IndexNow not applicable (a workflow file, not a page).

**Prior pass, for continuity:** `ops/social_drafts.py` and its sibling `ops/linkedin_drafts.py` (the real daily `--send` email Phil actually reads) both reported a static "N usable posts remain" figure that never decreased as posts were actually served, proved by replaying multiple simulated days. Fixed both to subtract the rotation's served set; new tests fail-then-pass proved against the pre-fix code. Full `preflight.py`, all 117 test files, `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (162 documents), mobile `npm test` (4 suites) all clean after.
