...
| Production traceability | Tracked, 20 commits/29 site files behind as of this check, GHCR image already current (`publish-image.yml` run 399) | No sandboxed session holds the deploy key, so this session cannot verify directly; `deploy.yml` fired automatically off that build and its own job log shows the Deploy step `skipped` (no `VPS_DEPLOY_KEY` secret yet). `ops/deploy-verdict.json` (last confirmed `2026-09-24T18:47:16Z`) is the live-tracked answer, not unknown by design. See `BLOCKER-001` above for the current gap |
