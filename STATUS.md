...
**Last Updated:** 2026-09-20, scheduled operator cycle. **The standing interactive `shop.html`/`kit.html` QA handoff, named twice in `ops/NIGHTLY-LOG.md` PM check-ins and never actually run, closed; no live defect found.** Attached via unshallow plus ff-only merge onto `origin/main` (783-commit fast-forward from a shallow/detached start, clean). `preflight.py` clean (every gate passed, 22 warnings, all previously diagnosed sandbox limits). `BACKLOG-2026-09-07.md` sections 2-6 again all done or Phil-gated; 8 GitHub issues confirmed live via the API, unchanged (`decision`: 33, 32, 31, 21, 18, 15; `blocked-on-art`: 29, 2), 0 open PRs. No mail credential, inbox UNCHECKED not empty. Every existing shop check (`gate_prerender_shop_current`, `gate_shop_buy_claim_honest`, `gate_price_matches_its_own_link`) reads static HTML text against the catalogue; none had ever actually clicked a filter button on `shop.html` or followed a rendered action link, the exact gap the standing handoff named. Read `kit.html` cold (no JS grid to drive): all 8 Target retailer-search links decode to sensible, correctly-encoded terms, disclosure correctly above the links, zone counts already gated; no defect. Built `ops/tests/test_shop_interactive.py`, a headless-Chromium probe driven the same iframe/dump-dom way `test_quest_flow.py` already drives the Quest: clicks all 8 real filter buttons, checks each renders the exact tile count the catalogue itself has for that category, that exactly one button reads `aria-pressed=true`, that every action link has a non-empty href, that every Stripe buy link is well-formed, carries the `data-sku` `measure.js` needs, resolves to a real SKU whose own `buy` field matches the rendered href, and that no two different SKUs ever share one Stripe payment link (line items are immutable per link, so a collision would misattribute or mis-sell). Ran clean: 155 distinct buy links, all 8 categories correct. Proved it can fail before trusting the clean result: planted two real SKUs sharing one Stripe URL in a copy of `data.js`, reran, watched it fail by name citing both SKUs and the shared link, restored byte for byte (confirmed by `git diff`), reran clean. `gate_tests()` already globs `ops/tests/test_*.py`, so no separate `preflight.py` wiring was needed. Full `preflight.py` (every gate passed, 22 warnings, all previously diagnosed sandbox limits, 219 test files), `check_urls.py` (188/188), `affiliate.py --check` (163 documents) all clean after. No price, product or page touched; this is new test coverage, not a content or code change. IndexNow not applicable.

**Prior (2026-09-19, scheduled operator cycle):** **A latent staleness gap closed in `ops/import_chapter_svgs.py`, no live defect today.** Attached via unshallow plus ff-only merge onto `origin/main`, clean. `BACKLOG-2026-09-07.md` sections 2-6 again every row done or Phil-gated; 8 GitHub issues confirmed live via the GitHub API, unchanged (`decision`/`blocked-on-art`), 0 open PRs, `checks.yml`/`publish-image.yml` both green on their last 3 runs. Continued the standing low-mention `ops/*.py` cold-read lane named by the prior cycle: `linkedin_posts.py` and `social_drafts.py` both read clean (traced `linkedin_posts.py`'s claim that every post is built on a real `content.json` `the_call` entry back to four specific real entries; traced `social_drafts.py`/`corpus_posts.py`'s rotation, word-bound and X character-cap logic and confirmed the 2026-09-12 "remaining count" fix still holds). `import_chapter_svgs.py` is the real find: it wires six hand-authored SVG diagrams out of book chapters 31-39 onto their zone pages, once each, skipping a figure the moment its id already exists on the page. That is what makes rerunning the importer safe, but it also means nothing would ever notice if a chapter's own SVG were hand-edited afterward; the zone page would keep showing the old figure forever with no error, the same "source corrected, artifact never re-derived" shape this repository's own gates keep closing elsewhere (hero-fallback, the Standards Pack, the pre-rendered shop grid, the KDP cover). Checked directly before writing anything: re-extracted and re-transformed all six figures fresh from the real, committed chapter HTML and diffed them against what each real, committed zone page actually carries; byte for byte identical on all six, so this closes a latent gap, not a live defect, the same posture as `gate_hero_fallback_current`. Refactored the shared transform out of `wire()` into a new `render_figure()` so the importer and the gate cannot silently disagree about what "current" means; confirmed behaviour-preserving (a rerun of the importer produces zero site diff). New `gate_chapter_svgs_current` in `preflight.py`, registered in `GENERATOR_PROTECTED_ELSEWHERE`; `ops/tests/test_gate_chapter_svgs_current.py` (5 cases) fail-then-pass proved directly against the real committed file (planted a stale figure block on a real zone page, watched the gate fail by name citing the figure id, restored byte for byte, reran clean). Full `preflight.py` (every gate passed, 22 warnings, all previously diagnosed sandbox limits, identical count and set to the pre-change baseline), `check_urls.py` (188/188), `affiliate.py --check` (163 documents), `fix_dashes.py --check` (0/0) all clean; the full `ops/tests/test_*.py` suite already reran clean inside preflight's own `gate_tests`. No price or product touched, no site page changed; not customer-facing. IndexNow not applicable. Pushed via a merge with a concurrent PM check-in cycle (`d6a45a47`), resolved by keeping both nightly-log entries in order and regenerating the three generated dashboard files fresh rather than hand-merging them; preflight and the new test rerun clean after the merge.

Material commits since `c3fdbdce`, none of them previously named here (the seven others, alongside `9bb578d2` above): the daily LinkedIn draft's own "Customers to date" line had no gate holding it to `GOALS.md`'s canonical baseline, now gated (`6ea0fb91`); `GOALS.md`'s own "inside the trailing 30-day window until 2026-09-20" sentence had no gate to catch it going stale the day after, now gated (`ae3af120`); a silent dead button on the Home Quest's install prompt was fixed (`3097fdf1`); "Work a room" and "One S across a room" silently ran a whole-house draw instead of refusing when no room was chosen, fixed and gated (`03eab6b8`); the Home Quest's Back button did nothing for a first-time visitor reaching the map or Keep screen, fixed and gated (`46479814`); the handed-off narrative/tone cold-read of `kitchen-deck.html`, `deck-gallery.html` and `kit.html` closed with no new defect (`78f874ea`); and `roadmap-report.yml` was picking the wrong edition, depth and header on most late-starting runs, fixed and verified against 10 real Actions runs, with a new `gate_roadmap_edition_from_schedule` (`2b445afe`). The rest of the gap is routine PM check-ins and command-deck regenerations, already fully accounted for in `ops/NIGHTLY-LOG.md`.

**Prior (2026-09-19, scheduled operator cycle):** **A real, live object-URL leak in the Home Quest's Keep screen, found and fixed.** Attached via unshallow plus ff-only merge onto `origin/main` (`d66814d3`), clean. `BACKLOG-2026-09-07.md` sections 2-6 again every row done or Phil-gated; the narrative cold-read lane closed 114 of 114 the prior cycle, per its own handoff already exhausted the standing low-mention `ops/*.py`/hand-authored `site/*.html` tiers this month, so this pass moved to the one remaining named lane, `site/assets/js/*.js`, and cold-read the lowest-mention file, `photos.js`. That led to `quest.js`'s `renderKeep()`: `URL.createObjectURL()` object URLs are never revoked by the browser on their own, so `releaseUrls()` exists specifically to revoke the previous visit's before releasing the next, per the file's own comment. Two of `renderKeep()`'s five real call sites, the `#go-keep` nav button (the ordinary, everyday way anyone opens Keep) and the restore-backup flow, never called it; only the reset/upload/delete paths did. Every ordinary visit to the one screen whose value depends on a long session staying usable leaked a blob per photograph, for the tab's life. Fixed by moving the release inside `renderKeep()` itself so no call site can omit it. Verified two ways: `ops/tests/test_quest_keep_url_leak.py` drives the real `#go-keep` button and `renderKeep()` in a real headless browser (IndexedDB itself does not complete under this sandbox's headless Chromium against `file://`, confirmed by a standalone probe before writing the test, so `window.QuestPhotos` is stubbed; everything else is the real, unmodified file), and a new `gate_quest_keep_releases_urls_first` in `preflight.py` (`ops/tests/test_gate_quest_keep_url_leak.py`, 5 cases) re-derives the invariant from source with no browser at all. Both fail-then-pass proved directly against the real committed file (`git stash`/git history reproduce the leak by name, restored and clean). `ops/fingerprint_assets.py` rerun for `quest.js`'s new hash. Full `preflight.py` (every gate passed, 23 warnings, all previously diagnosed sandbox limits), `check_urls.py` (188/188), `ops/tests/test_quest_flow.py` (no regression) all clean. No price, product or new page; IndexNow not applicable. Shipped via `ops/ship.py --no-deploy` (`55f8a88e`); the gate/test commit followed the same way. CI (`checks.yml`/`publish-image.yml`) was in flight against both commits at the time of this edit, not yet confirmed; the two runs this cycle inherited from the prior handoff (`checks.yml` #1172, `publish-image.yml` #341 attempt 2) both confirmed green before this fix was even started.

Material commits since `ef3cf037`, none of them previously named here: the zone index now states each zone's position in its own room (`45215195`); the Home Quest's own installed-app "Draw a card" shortcut went blank for a visitor who had finished the whole 684-card deck, fixed to the same fall-through shape as the `zone=`/`room=` deep links (`2992502a`); the free Micro Zone Map scrolled sideways on a phone, fixed with the same responsive override already used for the Standards Pack (`0352306e`); a new gate asserts a zone page's own diagram lights that page's own zone (`70bd2b62`); a third independent slug reimplementation was consolidated to a single source in `check_video_standard.py` (`304adf8b`); Phil's own commit added a new "where it is" opening beat to all 114 zone videos, which correctly failed `gate_srt_captions_current` (114 stale caption sidecars) until two convergent cycles regenerated them from the real `beats()` output with zero merge conflicts (`af16a257`, `5e5c5da0`); and `LEARNINGS.md`'s own citation index had drifted 8 real learnings behind its registry, the identical gap already fixed in `DECISIONS.md`'s sibling table on 2026-09-10, now closed and gated the same way (`479d6789`). The rest of the gap is routine PM check-ins and command-deck regenerations, already fully accounted for in `ops/NIGHTLY-LOG.md`.

**Checked, not assumed:** CI run 1162 (`checks.yml`, against the current merge commit `f52c128b`) was still `in_progress` at the time of this check-in, not yet confirmed green; the next cycle should confirm it rather than cite this one's hope. `check_urls.py` (188/188), `audit_pages.py` (191/0), `affiliate.py --check` (163 documents), `fix_dashes.py --check` (0/0) all clean. No mail credential, inbox UNCHECKED not empty. No price, product or site page touched; this is a status-document correction, not a code or content change. Full account in `ops/NIGHTLY-LOG.md`.
...
**Production Knowledge:** LAST CONFIRMED CURRENT 2026-09-18T00:07:30Z (build `a53458d85a904f9e`, `ops/deploy-verdict.json`, PHIL'S OWN SESSION, VERIFYING THE ZONE-PICTURE BATCH LIVE ON REAL URLS: FIVE HERO FIGURES SERVING AVIF, THE NEW DECK PDF, THE DISCLOSURE READING 81/7, `quest-data.js` CARRYING THE NEW `img` KEYS, COMMITTED IN `4418ee0a`). `site/build-id.txt` STILL COMPUTES TO THIS SAME BUILD AS OF THIS ENTRY (VERIFIED DIRECTLY, `ops/build_id.py --check`), SO PRODUCTION AND THE REPOSITORY MATCH. THE PRIOR CONFIRMED READING, `c3d0d442441b24df` (2026-09-15T21:13:31Z), IS NOW SUPERSEDED. NO OPERATOR SANDBOX HOLDS THE DEPLOY KEY'S PRIVATE HALF OR EGRESS TO THE VPS, SO NEITHER A FRESH CHECK NOR A REDEPLOY IS POSSIBLE FROM HERE; TREAT VPS ACCESS AS PER-SESSION, NOT UNIFORMLY AVAILABLE.
