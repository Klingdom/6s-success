...
**Updated By:** Claude, autonomous operator pass. Checkout arrived detached with local `main` sharing no common ancestor with `origin/main` (issue #27's usual shape); reset onto `origin/main`. Preflight FAILED at the start on a real, live defect (`checks.yml` was already red on GitHub at HEAD, confirmed via the Actions API, not assumed): STATUS.md carried 3 em dashes from a prior cycle's own write-up. Per CLAUDE.md STEP 2, fixing that was this run's work before anything else.

**This pass's real finding:** two things, one a live CI break and one a stale backlog claim. First, `ops/fix_dashes.py`'s own same-line em-dash-pair handling mishandled a THIRD spaced dash later on one line (a separate aside earlier in the same sentence): the pair regex only ever looks at the first two dashes it finds, and the leftover third fell through to a bare `.replace("--", ", ")` that left both of its real spaces in place ("`opacity` -- now" became "`opacity` ,  now"), the exact defect that had just shipped in this file's own prior write-up. Phil independently fixed the visible symptom by hand (commit `93c07d94`) while this cycle was running; this commit fixes the tool's own root cause so the next paragraph with three dashes on one line does not reproduce it. Fixed the fallback to consume surrounding whitespace like every sibling path in the file already does; new case in `ops/tests/test_fix_dashes.py` (10 of 10 pass, merged with Phil's own same-cycle improvement to that file's failure reporting), proved to fail against the pre-fix logic in isolation and pass restored. Second, read `BACKLOG-2026-09-07.md` cold rather than assuming its own priority order was still accurate: A4 ("rebalance the zone page against its own query") was still listed open, but `short_answer()` (commit `ccb8fdbc`, landed the same day the backlog was written, several hours later) already renders the ~100-word "how to organize" answer second on every zone page, ahead of the 471-word supply list. Checked live, not assumed: all 114 `site/zones/*.html` pages carry it there. Marked A4 done with the evidence; new `gate_zone_short_answer_above_fold` in `preflight.py` (`ops/tests/test_gate_zone_short_answer.py`, 5 cases) so a future edit cannot silently move the answer back behind the supply list, proved fail-then-pass on two planted regressions. Deliberately did not move the supply list or cleaning-procedure sections: `zone_page()`'s own comment explains why they sit after the six passes (a reader who meets the kit list after starting has already made the trip they were trying to avoid), and doing so would have undone that to chase a problem already fixed.

Preflight fast clean (0 gates failed, 14 warnings, all sandbox-environment limitations). Page count 190. GitHub: 9 open issues unchanged (all art-blocked or decision-labelled), 0 open PRs, CI confirmed green again at the new HEAD via the GitHub Actions API after pushing this fix (was red at the prior HEAD, see above).  
