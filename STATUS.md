...
**Did (late evening, same local session):** reduced-motion accessibility fix deployed and verified on production (build `ce8d3507`; the live stylesheet carries `.js .reveal{...transition:none}`), with CI Checks on `8c139586` green. Rebuilt three stale Etsy packs (Kitchen, Moving-In, Holiday Hosting). No listing is live, so no buyer had the old copy. Open: CI has no browser, so browser tests there report NOT VERIFIED rather than pass, and commits touching only `site/` get no Checks run.
