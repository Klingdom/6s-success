...
| **M6** | Author `diagnosis` for the remaining 102 zones | completes the layer | 2 | 4.0 | All 114 pass M2. **Gated, condition REPLACED 2026-09-21, see `DECISIONS.md` D-021.** The original condition ("M4 live 21 days and a report on whether the 12 pilot pages moved on impressions or entrances") **cannot be answered and should not be waited on**: impressions need Search Console, which is verified to nothing, and entrances do not exist to compare. Measured 2026-09-21 from the Umami database: since M4 shipped, all 114 zone pages together have 247 pageviews from **5 distinct visitors** across 115 paths, which is a sweep rather than reading; verified search engines fetched the 12 pilot pages 9 times in the same period against 98 for the other 102. Nothing can be concluded from that. **New condition:** M6 opens when zone pages have genuine human readers (entrances from more than a handful of sessions in a 30-day window) or when Search Console produces a real impressions read. Note also that M6 must not be justified by an expected search gain: `LEARNINGS.md` LRN-0013 now rules out on-page depth, uniqueness, structure and metadata as causes. Its real case is product coherence (`CLAUDE.md` section 6, diagnose before prescribing). | content-editor |
...
| **S5** | ~~Say so out loud where a household will see it~~ | the drift signal is useless if it is only on a page | 2 | 0.25 | **Prep done 2026-09-13, operator; paste itself Phil-gated, see `OWNER-ACTIONS.md` item 20.** `description_for()` in `ops/build_youtube_metadata.py` now writes a "KEEP IT THIS WAY" block linking that zone's own `#sustain` anchor for every zone with a real Sustain pass, verified across all 114 generated descriptions, not just the 12 published. `gate_youtube_sustain_anchor` in `preflight.py` fails if a future edit drops or mis-slugs the link; `ops/tests/test_gate_youtube_sustain_anchor.py` (6 cases) fail-then-pass proved, both the missing-anchor and wrong-slug shapes. No operator credential can write to an already-published video's own description (confirmed: uploading needs OAuth this sandbox does not hold, per item 1), so the 12 live videos still need Phil to paste the block by hand; the exact link for each is in `OWNER-ACTIONS.md` item 20. | product-manager, then **Phil** |
...
| **DECISION POINT** | ~~read M4's 21-day measurement~~ **unanswerable, see D-021** | 0 | Replaced: the measurement needs readers the site does not have (5 visitors across 115 zone pages since M4) and impressions it cannot see (no Search Console). M6 now opens on real zone entrances or a Search Console read |
