...
- **A victory condition for that card.** Right now 570 of 684 cards print a
  stop condition that card cannot reach. A person who does the Sort card
  properly is shown a description of the finished zone and told, in effect,
  that they failed.
...
| **M3** | Author `diagnosis` for the 12 pilot zones: Entryway 5 + Kitchen 7 | these are the only 12 zones with a published video, i.e. the only ones with any distribution surface (`GOALS.md`, 12 of 228 published) | 2 | 1.5 | 12 zones pass M2's validator. The 7 Kitchen zones reuse the 21 frictions already written in `kitchen-deck.json` character-for-character, and a gate asserts it. No friction sentence claims a customer said anything. **Kitchen half done 2026-09-07, operator.** All 7 Kitchen zones now pass `ops/diagnosis.py` (0 problems); every string copied verbatim from `kitchen-deck.json`'s own FRICTION and 15-minute ACTION cards, nothing invented. Diff purely additive (420 insertions), confirmed against a plain round-trip of the untouched file first. Entryway 5 still open: those zones have no equivalent card deck to reuse from, so it needs new authorship rather than a mechanical mapping, better done as its own pass. | content-editor, Kitchen half done by operator |
...
| **M5** | Kill the identical 19-link block on the remaining 102 pages | 110 of 114 pages currently carry byte-identical related reading | 2 | 0.5 | No two zone pages share an identical related-reading set. Every article is linked from at least 3 zone pages and no article is linked from more than 30. | seo-aeo |
...
| **M7** | Per-card victory conditions for all 684 cards | fixes the 570-card defect in 1.1 | 2 | 1.5 | Every card in `quest-data.js` carries its own `victory`; `#c-done-look` reads it; the whole-zone `done_looks_like` appears only on the Sustain card and on the zone page. A reviewer can confirm each victory is achievable by that pass alone. | content-editor + software-engineer |
...
| **The Entryway deck has three published sizes** | 46 in `ops/build_printpack.py` and `ops/build_standards.py`, 88 on the site, 89 in `build/entryway-cardtext.json`. Carried from `DECK-SYSTEM.md` 10.3, re-checked today. |
...
| # | Item | Why | Tier | Days | Acceptance criteria | Owner |
|---|---|---|---|---|---|---|
| **K0** | Fix the Entryway deck's card count everywhere, and re-verify the 404 tile against the live site | we cannot ship a second deck while the first has three sizes and a broken shop image | 2 | 0.5 | One number appears in the scripts, the site, the PDF and the corpus. The tile is fetched from production and its status recorded; if it is a 404 it is fixed and re-fetched, and the fix is staged per `CLAUDE.md` 0.3. | content-editor + commerce-manager |
...
| # | Item | Why | Tier | Days | Acceptance criteria | Owner |
|---|---|---|---|---|---|---|
| **A1** | Per-card victory conditions live in the app | this is item M7 landing on the surface; it fixes a contradiction on 570 cards | 2 | see M7 | On any non-Sustain card, "You can stop when" describes a state reachable by that pass alone. Verified by reading 20 sampled cards across 20 rooms. | software-engineer |
| **A2** | Move the session length off the first card | "45 to 75 minutes" is the first number a first-timer reads | 7 | 0.25 | The whole-zone session appears on the zone page and on the finish screen, not on card one of a first run. | ux-frontend |
...
| **A5** | Instrument the funnel so the next cycle can read it | today we emit `quest-start`, `quest-card-done`, `quest-zone-held`, `quest-first-start`, `quest-offer-shown`. We cannot tell whether people bounce at the ask or at the work. | 2 | 0.5 | New events: `quest-symptom-picked` (which symptom), `quest-cause-shown`, `quest-victory-confirmed`, `quest-card-abandoned` (pass, elapsed seconds), `quest-return` (days since last). No personal data, no free text, consistent with `CLAUDE.md` section 47. All five readable in the analytics database within 24 hours of deploy. | software-engineer + analytics-intelligence |
