...
8. **Deploy.** Push to `main`; the image builds automatically (about 19 minutes). What happens next depends on what the session holds, and the two cases were collapsed into one here until 2026-09-20, which risked the worse outcome of the two: a session that could deploy reading that it could not, and leaving the work built but never served. **With `~/.ssh/6s_deploy`** (interactive sessions on Phil's machine): run `python ops/deploy.py`, which pulls the new image on the VPS, recreates the container, diffs the production compose file against this repository, and verifies the live build id afterwards. That is a real deploy, no click needed. **Without the key** (the hourly cloud cycle, which holds no key and usually no egress either, issue #22): the change is built and not live, and the log must say exactly that rather than implying otherwise.
